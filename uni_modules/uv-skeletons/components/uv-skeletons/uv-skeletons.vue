  ...
  <view class="uv-skeleton">
    <view v-if="loading">
      <view v-for="(item, index) in elements" :key="index">
        ...
        >
          ...
          >
            <view v-for="(item3, index3) in item2.elements" :key="index3">
              <!-- 垂直高度站位 -->
              <view
                :style="{ height: $uv.addUnit(item3.height, 'rpx') }"
                v-if="item3.type == 'gap'"
              ></view>
              <view
                :class="[item3.clas, roundClass, animateClass]"
                :style="[style(item3)]"
                v-else
                ref="skeleton"
              ></view>
        ...
        <!-- 垂直高度站位 -->
        <view
          :style="{ height: $uv.addUnit(item.height, 'rpx') }"
          v-else-if="item.type == 'gap'"
        ></view>
        <!-- 其他基本单位 line avatar 等 -->
        <view
          :class="[item.clas, roundClass, animateClass]"
          :style="[style(item)]"
          v-else
          ref="skeleton"
        ></view>
    ...
    getElCounts(el) {
      ...
      if (this.$uv.test.number(el)) {
        ...
        for (let i = 0; i < child.length; i++) {
          children[i] = {
            clas:
              child[i].type.indexOf("avatar") > -1 ||
              child[i].type.indexOf("custom") > -1
                ? ""

<template>
  <view class="uv-waterfall">
    ...
    <view class="uv-waterfall__gap_right" :style="[gapRightStyle]"> </view>
    <!-- #endif -->
    <!-- #ifdef APP-NVUE -->
...
export default {
  ...
  computed: {
    ...
    nvueWaterfallStyle() {
      ...
      if (!style.width)
        style.width = this.$uv.addUnit(this.$uv.sys().windowWidth, "px");
      if (!style.height)
        style.height = this.$uv.addUnit(this.$uv.sys().windowHeight, "px");
    ...
    copyValue(nVal, oVal) {
      ...
      if (nVal.length != 0) {
        ...
        this.tempList = this.tempList.concat(
          this.$uv.deepClone(nVal.slice(startIndex)),
        );
        this.splitData();
      }
      // #endif

...
function drawToolTipBridge(opts, config, context, process, eachSpacing, xAxisPoints) {
    ...
    if (
        ...
        opts.tooltip &&
        process === 1 &&
        (opts.type == 'line' || opts.type == 'area' || opts.type == 'column' || opts.type == 'mount' || opts.type == 'candle' || opts.type == 'mix')
    ) {
            ...
            newCategories.forEach(function (item, index) {
                ...
                if (truePoints - Math.abs(_scrollDistance_) >= opts.area[3] - 1 && truePoints - Math.abs(_scrollDistance_) <= opts.width - opts.area[1] + 1) {
                    ...
                    context.fillText(
                        ...
                        startY + opts.xAxis.marginTop * opts.pix + ((opts.xAxis.lineHeight - opts.xAxis.fontSize) * opts.pix) / 2 + opts.xAxis.fontSize * opts.pix
                    );
                    context.closePath();
                    context.stroke();
                }
...
uCharts.prototype.dobuleZoom = function (e) {
    ...
    if (!this.scrollOption.moveCount) {
        let cts0 = {
            changedTouches: [{ x: tcs[0].x, y: this.opts.area[0] / this.opts.pix + 2 }]
        };
        let cts1 = {
            changedTouches: [{ x: tcs[1].x, y: this.opts.area[0] / this.opts.pix + 2 }]
        };
        if (this.opts.rotate) {
            cts0 = {
                changedTouches: [

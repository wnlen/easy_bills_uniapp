<template>
    <view class="uv-rate" :id="elId" ref="uv-rate" :style="[$uv.addStyle(customStyle)]">
        <view class="uv-rate__content" @touchmove.stop="touchMove" @touchend.stop="touchEnd">
            <view class="uv-rate__content__item" v-for="(item, index) in Number(count)" :key="index" :class="[elClass]">
                <view class="uv-rate__content__item__icon-wrap" ref="uv-rate__content__item__icon-wrap" @tap.stop="clickHandler($event, index + 1)">
                    <uv-icon
...
export default {
    ...
    methods: {
        ...
        getRateIconWrapRect() {
            ...
            // #endif
            // #ifdef APP-NVUE
            dom.getComponentRect(this.$refs['uv-rate__content__item__icon-wrap'][0], (res) => {

...
function createDom(nodes, parent, namespace) {
  const _loop = function _loop(i) {
    ...
    if (!node.type || node.type == "node") {
      ...
      if (!namespace) ele = document.createElement(name);
      else ele = document.createElementNS(namespace, name); // 设置属性
      ...
      if (name == "img") {
        ...
        ele.addEventListener(
          ...
          function (e) {
            ...
            let offset;
            if (href && href[0] == "#")
              offset = (document.getElementById(href.substr(1)) || {})
                .offsetTop;
            uni.postMessage({
              data: {
                action: "onLinkTap",
        ...
        name == "table" &&
        options[4] &&
        !ele.style.cssText.includes("inline")
      ) {

...
export default {
  ...
  methods: {
    ...
    setBarScrollTop() {
      ...
      let barScrollTop =
        tabRect.rect.height / 2 + offsetTop - scrollViewHeight / 2;
      // 先给一点随机值，避免出现不能滚动的BUG
      barScrollTop = Math.min(
        barScrollTop,
        this.scrollViewHeight - this.tabsRect.height,
      );
    ...
    async clickHandler(currentIndex) {
      ...
      if (this.chain) {
        ...
        this.contentScrollTop =
          this.children[currentIndex].top -
          this.$uv.getPx(this.hdHeight) -
          Math.random() * 4 -
          4;
        // #ifndef MP-WEIXIN
    ...
    getHeight() {
      ...
      if (isEmpty || this.height == "auto")
        height = this.$uv.addUnit(this.$uv.sys().windowHeight);

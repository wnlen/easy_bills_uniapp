<template>
    <view class="uv-switch" :class="[disabled && 'uv-switch--disabled']" :style="[switchStyle, $uv.addStyle(customStyle)]" @tap="clickHandler">
        <view class="uv-switch__bg" :style="[bgStyle]"></view>
        <view class="uv-switch__node" :class="[innerValue && 'uv-switch__node--on']" :style="[nodeStyle]" ref="uv-switch__node">
            <uv-loading-icon :show="loading" mode="circle" timingFunction="linear" :color="innerValue ? activeColor : '#AAABAD'" :size="size * 0.6" />
        </view>
    </view>
</template>
<script>

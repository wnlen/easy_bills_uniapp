...
export default {
    ...
    if (!this.useNative) {
      ...
      this.$uGetRect(".u-slider__base").then((rect) => {
        ...
        if (this.sliderRect.width == 0) {
          console.info(
            "如在弹窗等元素中使用，请使用v-if来显示滑块，否则无法计算长度。",
          );
        }
        this.init();
      });
  ...
  methods: {
    ...
    onTouchStart(event, index = 1) {
      ...
      this.newValue =
        (this.distanceX / this.sliderRect.width) * (this.max - this.min) +
        parseFloat(this.min);
      this.status = "moving";
      // 发出moving事件
    ...
    onClick(event) {
      ...
      this.newValue =
        (clientX / this.sliderRect.width) * (this.max - this.min) +
        parseFloat(this.min);
    ...
    updateValue(value, drag, index = 1) {
      ...
      let width = Math.min(
        ((valueFormat - this.min) / (this.max - this.min)) *
            ...
            return (
              Math.round(
                Math.max(
                  ...
                  Math.min(
                    value,
                    this.rangeValue[1] - parseInt(this.step),
                    this.max,
                  ),
                ) / parseInt(this.step),
              ) * parseInt(this.step)
          ...
          case 1:
            return (
              Math.round(
                Math.max(
                  this.min,
                  this.rangeValue[0] + parseInt(this.step),

<template>
  <view
    class="u-dragsort"
    :class="direction == 'horizontal' ? 'u-dragsort--horizontal' : ''"
  >
    <view
  ...
  methods: {
    ...
    handleDragMove(event) {
      ...
      if (this.direction === "horizontal") {
        ...
        if (
          closestIndex !== this.closestIndex &&
          closestIndex !== this.dragIndex
        ) {
    ...
    calculateTargetPosition(currentY) {
      ...
      // 获取拖动项的高度
      const dragHeight =
        this.itemRects[this.dragIndex]?.height || this.itemHeight;
    ...
    swapItems(targetIndex, event) {
      ...
      const originalTransform =
        this.$refs[`u-dragsort-item-${originalIndex}`]?.style.transform;

<template>
  <view class="u-picker-wraper">
    ...
    >
      <slot :value="inputLabel"> </slot>
      <slot name="trigger" :value="inputLabel"> </slot>
      <up-input
        ...
        if (!o || n.join("/") != o.join("/")) {
          ...
          if (n != null) {
            n.forEach((element, index) => {
              ...
              if (
                currentCols &&
                Object.prototype.toString.call(currentCols) ===
                  "[object Object]"
              ) {
  ...
  computed: {
    ...
    inputLabel() {
      ...
      if (
        ...
        let res = this.innerColumns[0].filter((item) =>
    ...
    //已选，待确认的值
    inputValue() {
      let items = this.innerColumns.map(
  ...
  methods: {
    ...
    setDefault() {
      ...
      if (this.lastIndex.length == 0) {
        ...
        if (
          Array.isArray(this.defaultIndex) &&
          this.defaultIndex.length == this.innerColumns.length
        ) {

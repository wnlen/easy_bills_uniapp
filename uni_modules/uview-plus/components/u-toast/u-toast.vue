<template>
    <view class="u-toast">
        <u-overlay :show="isShow" :zIndex="tmpConfig.overlay ? tmpConfig.zIndex : -1" :custom-style="overlayStyle">
            <view
                class="u-toast__content"
                :style="[contentStyle]"
                :class="['u-type-' + tmpConfig.type, tmpConfig.type === 'loading' || tmpConfig.loading ? 'u-toast__content--loading' : '']"
            >
                <up-loading-icon v-if="tmpConfig.type === 'loading'" mode="circle" color="rgb(255, 255, 255)" inactiveColor="rgb(120, 120, 120)" size="25"></up-loading-icon>
                <u-icon v-else-if="tmpConfig.type !== 'defalut' && iconName" :name="iconName" size="17" :color="tmpConfig.type" :customStyle="iconStyle"></u-icon>
                <u-gap v-if="tmpConfig.type === 'loading' || tmpConfig.loading" height="12" bgColor="transparent"></u-gap>
                <text class="u-toast__content__text" :class="['u-toast__content__text--' + tmpConfig.type]" style="max-width: 400rpx">{{ tmpConfig.message }}</text>
            </view>
        </u-overlay>
    </view>
</template>

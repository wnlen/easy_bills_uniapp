...
export default {
  ...
  computed: {
    ...
    indicatorTop() {
      ...
      return Math.floor(
        top - height / 2 + itemHeight * this.activeIndex + itemHeight - 70 / 2,
      );
    },
  },
  watch: {
  ...
  methods: {
    ...
    init() {
      ...
      this.getIndexListRect().then(async (sizeScroll) => {
        this.scrollViewHeight = sizeScroll.height
          ? sizeScroll.height
          : this.sys.windowHeight - customNavHeight;
    ...
    getIndexListLetter(pageY) {
      this.pageY = pageY;
      let { top, height, itemHeight } = this.letterInfo;

              ...
              const captureFirstFrame = () => {
                ...
                wx.canvasToTempFilePath({
                  ...
                  success: (result) => {
                    ...
                    this.fileList["currentItemIndex"].thumb =
                      result.tempFilePath;
                  },
                  fail: (err) => {
      ...
      const lists = fileList.map((item) => {
        ...
        return Object.assign(Object.assign({}, item), {
          ...
          isImage: item.name
            ...
            : this.accept === "image" || test.image(name),
          isVideo: item.name
            ? test.video(item.name)
            : this.accept === "video" || test.video(name),
          deletable:
              ...
              var uploadTask = uni.uploadFile({
                ...
                success: (uploadFileRes) => {
                  ...
                  if (test.promise(afterPromise)) {
                    afterPromise.then((data) =>
                      that.succcessUpload(len + j, data.url, data.thumb),
                    );
                  } else {
                    result = res0.data.params.host + "/" + res0.data.params.key;
                    if (that.accept === "video" || test.video(result)) {
                      thumb =
                        result + "?x-oss-process=video/snapshot,t_10000,m_fast";
                    }
            ...
            default:
              ...
              var uploadTask = uni.uploadFile({
                ...
                success: (uploadFileRes) => {
                  ...
                  if (test.promise(afterPromise)) {
                    afterPromise.then((data) =>
                      that.succcessUpload(len + j, data.url),
                    );
                  } else {
                    if (res0.code != 200) {
                      uni.showToast({

<template>
    <view class="uv-index-list">
        ...
        >
            ...
            >
                <text class="uv-index-list__letter__item__index" :style="{ color: activeIndex === index ? '#fff' : inactiveColor }">{{ item }}</text>
            </view>
        </view>
        <uv-transition
            mode="fade"
    ...
    methods: {
        ...
        async scrollHandler(e) {
            ...
            for (let i = 0; i < len; i++) {
                ...
                if (scrollTop + customNavHeight <= children[0].top || scrollTop >= children[len - 1].top + children[len - 1].height) {
                    this.activeIndex = -1;
                    break;
                } else if (!nextItem) {

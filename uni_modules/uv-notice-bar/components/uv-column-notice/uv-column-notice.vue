<template>
    <view class="uv-notice" @tap="clickHandler">
        ...
        <swiper
            ...
            @change="noticeChange"
        >
            <swiper-item v-for="(item, index) in text" :key="index" class="uv-notice__swiper__item">
                <text class="uv-notice__swiper__item__text uv-line-1" :style="[textStyle]">{{ item }}</text>
            </swiper-item>
        </swiper>
        <view class="uv-notice__right-icon" v-if="['link', 'closable'].includes(mode)">
            ...
            <uv-icon v-if="mode === 'closable'" name="close" :size="16" :color="color" @click="close"></uv-icon>
        </view>
    </view>
</template>

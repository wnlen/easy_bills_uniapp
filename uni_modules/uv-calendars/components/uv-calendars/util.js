...
class Calendar {
    ...
    if (this.range && status == "init") {
      ...
      if (Array.isArray(date)) {
        ...
        if (
          ...
          this.dateCompare(this.rangeStatus.before, this.rangeStatus.after)
        ) {
          this.rangeStatus.data = this.geDateAll(
            this.rangeStatus.before,
            this.rangeStatus.after,
          );
        }
    ...
    for (let i = 1; i <= dateData; i++) {
      ...
      let data = {
        ...
        dateEqual:
          this.range &&
          checked &&
          this.dateEqual(this.rangeStatus.before, this.rangeStatus.after),
        month: full.month,
  ...
  setMultiple(fullDate) {
    ...
    let multiples = this.multipleStatus.data;
    const findIndex = multiples.findIndex((item) =>
      this.dateEqual(fullDate, item),
    );
    if (findIndex < 0) {

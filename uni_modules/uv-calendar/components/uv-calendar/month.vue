<template>
  <view class="uv-calendar-month-wrapper" ref="uv-calendar-month-wrapper">
    <view
      ...
      :key="index"
      :class="[`uv-calendar-month-${index}`]"
      :ref="`uv-calendar-month-${index}`"
      :id="`month-${index}`"
    >
      <text v-if="index !== 0" class="uv-calendar-month__title"
        >{{ item.year }}年{{ item.month }}月</text
      >
      <view class="uv-calendar-month__days">
        <view
          v-if="showMark"
          class="uv-calendar-month__days__month-mark-wrapper"
        >
          <text class="uv-calendar-month__days__month-mark-wrapper__text">{{
            item.month
          }}</text>
        </view>
        <view
          class="uv-calendar-month__days__day"
          v-for="(item1, index1) in item.date"
          :key="index1"
          :style="[dayStyle(index, index1, item1)]"
          @tap="clickHandler(index, index1, item1)"
          :class="[
            item1.selected && 'uv-calendar-month__days__day__select--selected',
          ]"
        >
          <view
            class="uv-calendar-month__days__day__select"
            :style="[daySelectStyle(index, index1, item1)]"
          >
            <text
              v-if="getTopInfo(index, index1, item1)"
              class="uv-calendar-month__days__day__select__top-info"
              :class="[
                item1.disabled &&
                  'uv-calendar-month__days__day__select__top-info--disabled',
              ]"
              :style="[textStyle(item1)]"
              >{{ getTopInfo(index, index1, item1) }}</text
            >
            <text
              class="uv-calendar-month__days__day__select__info"
              :class="[
                item1.disabled &&
                  'uv-calendar-month__days__day__select__info--disabled',
              ]"
              :style="[textStyle(item1)]"
              >{{ item1.day }}</text
            >
            <text
              ...
              :class="[
                item1.disabled &&
                  'uv-calendar-month__days__day__select__buttom-info--disabled',
              ]"
              :style="[textStyle(item1)]"
            ...
            <text
              v-if="item1.dot"
              class="uv-calendar-month__days__day__select__dot"
            ></text>
          </view>
        </view>
      </view>
  ...
  methods: {
    ...
    getMonthRect() {
      ...
      const promiseAllArr = this.months.map((item, index) =>
        this.getMonthRectByPromise(`uv-calendar-month-${index}`),
      );
      // 一次性返回

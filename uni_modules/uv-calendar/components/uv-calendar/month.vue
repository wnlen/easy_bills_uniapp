<template>
    <view class="uv-calendar-month-wrapper" ref="uv-calendar-month-wrapper">
        <view v-for="(item, index) in months" :key="index" :class="[`uv-calendar-month-${index}`]" :ref="`uv-calendar-month-${index}`" :id="`month-${index}`">
...
export default {
    ...
    methods: {
        ...
        getMonthRect() {
            ...
            const promiseAllArr = this.months.map((item, index) => this.getMonthRectByPromise(`uv-calendar-month-${index}`));
            // 一次性返回

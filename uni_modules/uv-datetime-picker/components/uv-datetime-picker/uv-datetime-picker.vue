  ...
  methods: {
    ...
    updateIndexs(value) {
      ...
      if (this.mode === "time") {
        ...
        if (this.mode === "datetime") {
          // 数组的push方法，可以写入多个参数
          values.push(
            formatter("day", this.$uv.padZero(dayjs(value).date())),
            formatter("hour", this.$uv.padZero(dayjs(value).hour())),
            formatter("minute", this.$uv.padZero(dayjs(value).minute())),
          );
        }
      }
    ...
    correctValue(value) {
      ...
      if (!isDateMode) {
        ...
        hour = this.$uv.padZero(
          this.$uv.range(this.minHour, this.maxHour, Number(hour)),
        );
        minute = this.$uv.padZero(
          this.$uv.range(this.minMinute, this.maxMinute, Number(minute)),
        );
        return `${hour}:${minute}`;
      } else {

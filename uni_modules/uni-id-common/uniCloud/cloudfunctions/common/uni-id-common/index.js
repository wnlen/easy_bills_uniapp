...
var e,
  t =
    (e = require("crypto")) && "object" == typeof e && "default" in e
      ? e.default
      : e;
const n = {
...
function a(e) {
  return (
    "object" ===
      ((i = e), Object.prototype.toString.call(i).slice(8, -1).toLowerCase()) &&
    e.errCode &&
...
function f(e) {
  return l(
    ((t = JSON.stringify(e)), Buffer.from(t, "utf-8").toString("base64")),
  );
  var t;
}
function p(e, n) {
...
class E {
  ...
  checkConfig() {
    ...
    if (t >= e)
      throw new Error(
        "Config error, tokenExpiresThreshold should be less than tokenExpiresIn",
      );
    t > e / 2 &&
      console.warn(
  ...
  async getUserRecord() {
    ...
    if (((this.userRecord = e.data[0]), !this.userRecord))
      throw { errCode: n.ACCOUNT_NOT_EXISTS };
  ...
  async getUserPermission() {
    ...
    if (e.includes("admin"))
      return (
        (this.userPermission = { role: e, permission: [] }),
        this.userPermission
      );
    const t = await T.where({ role_id: I.in(e) }).get(),
      n =
        ((i = t.data.reduce(
          (e, t) => (t.permission && e.push(...t.permission), e),
          [],
        )),
        Array.from(new Set(i)));
    var i;
    return (
      (this.userPermission = { role: e, permission: n }),
      this.userPermission
    );
  }
  ...
  async refreshToken({ token: e } = {}) {
    if (!e)
      throw { errCode: n.PARAM_REQUIRED, errMsgValue: { param: "token" } };
    this.oldToken = e;
    const t = this._checkToken(e);
    ((this.uid = t.uid), (this.oldTokenPayload = t));
    const { uid: i } = t,
      { role: r, permission: o } = await this.getUserPermission(),
      { token: s, tokenExpired: c } = await this._createToken({
        uid: i,
        role: r,
        permission: o,
      });
...
var m = Object.freeze({
  ...
  createToken: async function ({ uid: e, role: t, permission: n } = {}) {
    return new E({ uniId: this }).createToken({
      uid: e,
      role: t,
      permission: n,
    });
...
class x {
  constructor({ context: e, clientInfo: t, config: n } = {}) {
    ((this._clientInfo = e
      ...
      w.hasFile("custom-token.js") &&
        this.setInterceptor(
          "customToken",
          require(w.resolve("custom-token.js")),
        ));
  ...
  _getAppConfig() {
    ...
    return Array.isArray(e)
      ? e.find((e) => e.dcloudAppid === this._clientInfo.appId) ||
          e.find((e) => e.isDefaultConfig)
      : e;
  }
  _getPlatformConfig() {
    ...
    let t;
    switch (
      (["app-plus", "app-android", "app-ios"].indexOf(
        this._clientInfo.platform,
      ) > -1 && (this._clientInfo.platform = "app"),
    ...
    return (
      ["tokenSecret", "tokenExpiresIn"].forEach((e) => {
        if (!i || !i[e])
          throw new Error(`Config parameter missing, ${e} is required`);
      }),
      i
    );
  }

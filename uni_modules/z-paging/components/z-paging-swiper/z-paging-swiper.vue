...
<template>
  <view
    :class="
      fixed
        ? 'zp-swiper-container zp-swiper-container-fixed'
        : 'zp-swiper-container'
    "
  ...
  >
    <!-- #ifndef APP-PLUS -->
    <view
      v-if="cssSafeAreaInsetBottom === -1"
      class="zp-safe-area-inset-bottom"
    ></view>
    <!-- #endif -->
    ...
    <view class="zp-swiper-super">
      <view
        v-if="zSlots.left"
        :class="{ 'zp-swiper-left': true, 'zp-absoulte': isOldWebView }"
      >
        <slot name="left" />
      </view>
      <view
      ...
      <view
        v-if="zSlots.right"
        :class="{
          'zp-swiper-right': true,
          'zp-absoulte zp-right': isOldWebView,
        }"
      >
        <slot name="right" />
      </view>
...
export default {
  ...
  methods: {
    ...
    updateLeftAndRightWidth() {
      ...
      this.$nextTick(() =>
        this._updateLeftAndRightWidth(this.swiperContentStyle, "zp-swiper"),
      );
    },
  },
};

...
export default {
    ...
    realTotalData(newVal) {
      ...
      if (this.finalUseVirtualList) {
        ...
        this.$nextTick(() => {
          ...
          newVal.length &&
            this.cellHeightMode === Enum.CellHeightMode.Fixed &&
            this.isFirstPage &&
  ...
  computed: {
    ...
    finalUseInnerList() {
      return (
        this.useInnerList ||
        (this.finalUseVirtualList && !this.forceCloseInnerList)
      );
    },
    finalCellKeyName() {
      // #ifdef APP-NVUE
      if (this.finalUseVirtualList && !this.cellKeyName.length) {
        u.consoleErr(
          "在nvue中开启use-virtual-list必须设置cell-key-name，否则将可能导致列表渲染错误！",
        );
      }
      // #endif
    ...
    finalVirtualPageHeight() {
      return this.virtualPageHeight > 0
        ? this.virtualPageHeight
        : this.windowHeight;
    },
  ...
  methods: {
    ...
    doInsertVirtualListItem(item, index) {
      ...
      item[c.listCellIndexUniqueKey] =
        `${this.virtualListKey}-${item[cellIndexKey]}`;
      this.$nextTick(async () => {
        let retryCount = 0;
        while (retryCount <= 10) {
          await u.wait(c.delayTime);

          const cellNode = await this._getNodeClientRect(
            `#zp-id-${item[cellIndexKey]}`,
          ...
          const lastTotalHeight = lastHeightCache
            ? lastHeightCache.totalHeight
            : 0;
    ...
    didUpdateVirtualListCell(index) {
      ...
      this.$nextTick(() => {
        this._getNodeClientRect(`#zp-id-${index}`, this.finalUseInnerList).then(
          (cellNode) => {
            // 更新当前cell的高度
            ...
            currentNode.totalHeight =
              currentNode.lastTotalHeight + cellNodeHeight;

            // 从当前index起后续的cell缓存高度的lastTotalHeight和totalHeight需要加上当前cell变化的高度
            for (
              let i = index + 1;
              i < this.virtualHeightCacheList.length;
              i++
            ) {
              const thisNode = this.virtualHeightCacheList[i];
        ...
        u.delay(
          async () => {
            for (let i = 0; i < list.length; i++) {
              const cellNode = await this._getNodeClientRect(
                `#zp-id-${list[i][this.virtualCellIndexKey]}`,
                this.finalUseInnerList,
              );
              const currentHeight = cellNode ? cellNode[0].height : 0;
              if (!cellNode) {
              ...
              const lastHeightCache = currentCacheList.length
                ? currentCacheList.slice(-1)[0]
                : null;
              const lastTotalHeight = lastHeightCache
                ? lastHeightCache.totalHeight
                : 0;
            ...
            if (dataFromTop && list.length) {
              ...
              this.virtualHeightCacheList =
                currentCacheList.concat(heightCacheList);
            }
            this._updateVirtualScroll(this.oldScrollTop);
          },
          c.delayTime,
    ...
    _setCellIndex(list, dataFrom = "bottom") {
      ...
      const cellIndexKey = this.virtualCellIndexKey;
      [Enum.QueryFrom.Refresh, Enum.QueryFrom.Reload].indexOf(this.queryFrom) >=
        0 && this._resetDynamicListState();
      if (this.totalData.length) {
        if (dataFrom === "bottom") {
          currentItemIndex = this.realTotalData.length;
          const lastItem = this.realTotalData.length
            ? this.realTotalData.slice(-1)[0]
            : null;
          if (lastItem && lastItem[cellIndexKey] !== undefined) {
            currentItemIndex = lastItem[cellIndexKey] + 1;
          }
        } else if (dataFrom === "top") {
          const firstItem = this.realTotalData.length
            ? this.realTotalData[0]
            : null;
      ...
      this.cellHeightMode === Enum.CellHeightMode.Dynamic &&
        this._updateDynamicCellHeight(list, dataFrom);
    },
    ...
    _updateVirtualScroll(scrollTop, scrollDiff = 0) {
      ...
      if (
        ...
        currentTimeStamp - this.virtualScrollTimeStamp <=
          this.virtualScrollDisTimeStamp
      ) {
        return;
      }
      ...
      if (cellHeightMode === Enum.CellHeightMode.Fixed) {
        ...
        const bottomRangePageOffset =
          scrollTop + this.finalVirtualPageHeight + rangePageHeight;

        let virtualBottomRangeIndex = 0;
        let virtualPlaceholderBottomHeight = 0;
        let reachedLimitBottom = false;
        const heightCacheList = this.virtualHeightCacheList;
        const lastHeightCache = !!heightCacheList
          ? heightCacheList.slice(-1)[0]
          : null;
        ...
        if (scrollDirection === "bottom") {
          ...
          for (let i = startTopRangeIndex; i < heightCacheList.length; i++) {
            ...
            if (
              heightCacheItem &&
              heightCacheItem.totalHeight > topRangePageOffset
            ) {
              // 记录顶部视图边缘cell的index并更新顶部占位区域的高度并停止继续查找
              this.virtualTopRangeIndex = i;
              this.virtualPlaceholderTopHeight =
                heightCacheItem.lastTotalHeight;
              break;
            }
          }
        } else {
        ...
        // 从顶部视图边缘的cell的位置开始向后查找
        for (
          let i = this.virtualTopRangeIndex;
          i < heightCacheList.length;
          i++
        ) {
          const heightCacheItem = heightCacheList[i];
          ...
          if (
            heightCacheItem &&
            heightCacheItem.totalHeight > bottomRangePageOffset
          ) {
            // 记录底部视图边缘cell的index并更新底部占位区域的高度并停止继续查找
            virtualBottomRangeIndex = i;
            virtualPlaceholderBottomHeight =
              lastHeightCache.totalHeight - heightCacheItem.totalHeight;
    ...
    _updateFixedTopRangeIndex(scrollIndex) {
      let virtualTopRangeIndex =
        this.virtualCellHeight === 0
          ? 0
          : scrollIndex -
            (parseInt(this.finalVirtualPageHeight / this.virtualCellHeight) ||
              1) *
              this.preloadPage;
    ...
    _updateFixedBottomRangeIndex(scrollIndex) {
      ...
      virtualBottomRangeIndex = Math.min(
        this.realTotalData.length,
        virtualBottomRangeIndex,
      );
      this.virtualBottomRangeIndex = virtualBottomRangeIndex;
      this.virtualPlaceholderBottomHeight =
        ((this.realTotalData.length - virtualBottomRangeIndex) *
          this.virtualCellHeight) /
        this.virtualListCol;
      this._updateVirtualList();
    },
    // 更新virtualList

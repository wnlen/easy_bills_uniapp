...
export default {
  ...
  computed: {
    ...
    isOldWebView() {
      ...
      try {
        ...
        if (
          (deviceType === "iOS" && version <= 10) ||
          (deviceType === "Android" && version <= 6)
        ) {
          return true;
        }
      } catch (e) {
  ...
  methods: {
    ...
    _getNodeClientRect(select, inDom = true, scrollOffset = false) {
      ...
      //#endif
      let res = !!inDom
        ? uni.createSelectorQuery().in(inDom === true ? this : inDom)
      ...
      return new Promise((resolve, reject) => {
        res.exec((data) => {
          resolve(
            data && data != "" && data != undefined && data.length
              ? data
              : false,
          );
        });
      ...
      this.$nextTick(() => {
        ...
        setTimeout(() => {
          ["left", "right"].map((position) => {
            this._getNodeClientRect(`.${parentNodePrefix}-${position}`).then(
              (res) => {
                this.$set(
                  targetStyle,
                  position,
                  res ? res[0].width + "px" : "0px",
                );
              },
            );
          });

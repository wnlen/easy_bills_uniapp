...
export default {
  ...
  computed: {
    finalLanguage() {
      try {
        ...
        return local === "auto"
          ? interceptor._handleLanguage2Local(
              language,
              this._language2Local(language),
            )
          : local;
      } catch (e) {
  ...
  methods: {
    ...
    _language2Local(language) {
      const formatedLanguage = language
        .toLowerCase()
        .replace(new RegExp("_", ""), "-");
      if (formatedLanguage.indexOf("zh") !== -1) {
        if (
          formatedLanguage === "zh" ||
          formatedLanguage === "zh-cn" ||
          formatedLanguage.indexOf("zh-hans") !== -1
        ) {
          return "zh-Hans";
        }
        return "zh-Hant";

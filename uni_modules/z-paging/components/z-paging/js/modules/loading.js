...
export default {
  ...
  watch: {
    ...
    loadingStatus(newVal) {
      ...
      if (this.useChatRecordMode) {
        if (
          this.isFirstPage &&
          (newVal === Enum.More.NoMore || newVal === Enum.More.Fail)
        ) {
  ...
  computed: {
    ...
    showLoading() {
      ...
      return this.autoHideLoadingAfterFirstLoaded
        ? this.fromEmptyViewReload
          ? true
          : !this.pagingLoaded
  ...
  methods: {
    ...
    _startLoading(isReload = false) {
      if (
        (this.showLoadingMoreWhenReload && !this.isUserPullDown) ||
        !isReload
      ) {

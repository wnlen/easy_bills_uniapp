...
export default {
  ...
  watch: {
    ...
    refresherStatus(newVal) {
      ...
      this.refresherVibrate &&
        (newVal === Enum.Refresher.ReleaseToRefresh ||
          newVal === Enum.Refresher.GoF2) &&
        this._doVibrateShort();
  ...
  computed: {
    ...
    finalRefresherThreshold() {
      ...
      return (
        u.convertToPx(refresherThreshold) +
        this.finalRefresherThresholdPlaceholder
      );
    },
    ...
    finalRefresherThemeStyle() {
      return this.refresherThemeStyle.length
        ? this.refresherThemeStyle
    ...
    finalShowRefresherWhenReload() {
      return (
        this.showRefresherWhenReload || this.privateShowRefresherWhenReload
      );
    },
  ...
  methods: {
    ...
    _refresherTouchmove(e) {
      ...
      if (this.watchTouchDirectionChange) {
        ...
        const direction =
          refresherTouchmoveY > this.oldRefresherTouchmoveY ? "top" : "bottom";
        // 只有在方向改变的时候才emit相关事件
        if (
          direction === this.oldTouchDirection &&
          direction !== this.oldEmitedTouchDirection
        ) {
      ...
      if (
        this.pullDownTimeStamp &&
        currentTimeStamp - this.pullDownTimeStamp <= this.pullDownDisTimeStamp
      )
        return;
      // 如果不允许下拉，则return
      if (this._touchDisabled()) return;
      ...
      // 对下拉刷新的角度进行限制
      if (
        this.refresherMaxAngle >= 0 &&
        this.refresherMaxAngle <= 90 &&
        this.lastRefresherTouchmove &&
        this.lastRefresherTouchmove.touchY <= refresherTouchmoveY
      ) {
        if (
          !moveDis &&
          !this.refresherAngleEnableChangeContinued &&
          this.moveDis < 1 &&
          !this.refresherReachMaxAngle
        )
          return;
        const x = Math.abs(touch.touchX - this.lastRefresherTouchmove.touchX);
        const y = Math.abs(
          refresherTouchmoveY - this.lastRefresherTouchmove.touchY,
        );
    ...
    _handleRefresherTouchmove(moveDis, touch) {
      ...
      if (moveDis >= this.finalRefresherThreshold) {
        ...
        this.refresherStatus =
          this.refresherF2Enabled && moveDis >= this.finalRefresherF2Threshold
            ? Enum.Refresher.GoF2
            : Enum.Refresher.ReleaseToRefresh;
      } else {
        // 下拉刷新距离未超过阈值，显示默认状态
    ...
    _handleRefresherTouchend(moveDis) {
      ...
      if (
        ...
        (this.refresherStatus === Enum.Refresher.ReleaseToRefresh ||
          this.refresherStatus === Enum.Refresher.GoF2)
      ) {
        // 如果是松手进入二楼状态，则触发进入二楼
        if (this.refresherStatus === Enum.Refresher.GoF2) {
          ...
          u.delay(() => {
            this._emitTouchmove({
              ...
              dy: this.moveDis - refresherThreshold,
            });
          }, 0.1);
    ...
    // 下拉刷新结束
    _refresherEnd(
      shouldEndLoadingDelay = true,
      fromAddData = false,
      isUserPullDown = false,
      setLoading = true,
    ) {
      if (this.loadingType === Enum.LoadingType.Refresher) {
        const refresherCompleteDelay =
          fromAddData && (isUserPullDown || this.showRefresherWhenReload)
            ? this.refresherCompleteDelay
            : 0;
        const refresherStatus =
          refresherCompleteDelay > 0
            ? Enum.Refresher.Complete
            : Enum.Refresher.Default;
        ...
        this.refresherCompleteTimeout = u.delay(() => {
          ...
          const animateType =
            this.refresherEndBounceEnabled && fromAddData
              ? "cubic-bezier(0.19,1.64,0.42,0.72)"
              : "linear";
          if (fromAddData) {
            animateDuration = this.refresherEndBounceEnabled
              ? this.refresherCompleteDuration / 1000
    ...
    _doRefresherRefreshAnimate() {
      ...
      // #ifndef APP-NVUE
      const doRefreshAnimateAfter =
        !this.doRefreshAnimateAfter &&
        this.finalShowRefresherWhenReload &&
        this.customRefresherHeight === -1 &&
        this.refresherThreshold === u.addUnit(80, this.unit);
    ...
    _cleanRefresherCompleteTimeout() {
      this.refresherCompleteTimeout = this._cleanTimeout(
        this.refresherCompleteTimeout,
      );
      // #ifdef APP-NVUE

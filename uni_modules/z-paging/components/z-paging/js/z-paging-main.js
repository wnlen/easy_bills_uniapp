...
export default {
  ...
  methods: {
    ...
    _doVibrateShort() {
      ...
      // #ifdef APP-PLUS
      if (this.isIos) {
        const UISelectionFeedbackGenerator = plus.ios.importClass(
    ...
    async _setAutoHeight(shouldFullHeight = true, scrollViewNode = null) {
      ...
      try {
        if (shouldFullHeight) {
          ...
          let finalScrollViewNode =
            scrollViewNode ||
            (await this._getNodeClientRect(".zp-scroll-view"));
          let finalScrollBottomNode =
            await this._getNodeClientRect(".zp-page-bottom");
          if (finalScrollViewNode) {
            const scrollViewTop = finalScrollViewNode[0].top;
            let scrollViewHeight = this.windowHeight - scrollViewTop;
            scrollViewHeight -= finalScrollBottomNode
              ? finalScrollBottomNode[0].height
              : 0;
    ...
    _handleUnmounted() {
      ...
      this.useChatRecordMode &&
        uni.offKeyboardHeightChange(this._handleKeyboardHeightChange);
      // #endif
    },
    // 触发更新是否超出页面状态
    _updateInsideOfPaging() {
      this.insideMore &&
        this.insideOfPaging === true &&
        setTimeout(this.doLoadMore, 200);
    },
    // 清除timeout

<template>
  <view
    class="Index ft24"
    :class="vuex_userRole === 'D' ? 'Indexbg1' : 'Indexbg2'"
  >
    <u-navbar
      title=""
      :placeholder="true"
    ...
    <view class="flex-row justify-between items-end ml30 mr30">
      <view>
        <view class="title mb12" @click="isLogin ? '' : goToLogin()">{{
          isLogin ? "易单据" : "请登录~"
        }}</view>
        <view class="ft28">送货单轻松签收</view>
      </view>
      <view
        class="flex-row justify-between butBox text-center ft28 ft-bold"
        id="box"
      >
        <view
          ...
          :class="vuex_userRole === item.value ? 'activeBtn' : 'flex-1'"
        >
          {{ item.name }}
        </view>
      </view>
    ...
    <view class="bg-white radius12 mt60 ml30 mr30">
      <view class="mb54 flex-row justify-between">
        <view class="charts-box">
          ...
          <view class="text-center ft-bold">
            ...
            <text
              v-for="(item, index) in allprice.toFixed(2).toString().split('.')"
              :class="index == 0 ? 'ft32' : 'ft24'"
              :key="index"
            >
              {{ item }}
              <text
                v-if="
                  index === 0 &&
                  allprice.toFixed(2).toString().split('.').length > 1
                "
                >.</text
              >
            </text>
        ...
        <view class="flex-col justify-around mr32 items-end pt30">
          <view
            ...
            @click="goList(ite.type)"
            v-for="(ite, ind) in chartsDataPie2.series[0].data"
            :key="ind"
          >
            <view
              class="text-center ft-bold"
              :class="
                ind == 0 ? 'ft-orange' : ind == 1 ? 'ft-blue' : 'ft-green'
              "
            >
              <text class="ft20">￥</text>
              <text
                v-for="(item, index) in ite.value
                  .toFixed(2)
                  .toString()
                  .split('.')"
                :class="index == 0 ? 'ft28' : 'ft20'"
                :key="index"
              >
                {{ item }}
                <text
                  v-if="
                    index === 0 &&
                    ite.value.toFixed(2).toString().split('.').length > 1
                  "
                  >.</text
                >
              </text>
      ...
      <view class="flex-row justify-between pall30">
        <view
          ...
          :id="index == 0 ? 'box2' : ''"
          :class="
            vuex_userRole === 'D' && index === 0
              ? 'indexbox1 indexbox'
              : vuex_userRole === 'R' && index === 0
                ? 'indexbox2 indexbox'
                : 'indexbox3 indexbox'
          "
    ...
    <view class="bg-white radius12 mt30 ml30 mr30">
      <view class="flex-row flex-wrap">
        <view
          class="flex-col width25_ items-center"
          @click="goPath(listItem.path)"
...
export default {
  ...
  onLoad() {
    ...
    wx.loadFontFace({
      family: "阿里妈妈数黑体 Bold",
      source:
        'url("https://res-oss.elist.com.cn/wxImg/fonts/aliBold/Xizk3ttTNIiT.woff2")',
    });
    // #endif
  },
  computed: {
  ...
  methods: {
    ...
    guideCourse() {
      if (
        this.vuex_userRole == "D" &&
        this.$u.getPinia("guide.guidanceD") != 1
      ) {
        this.$refs.FunctionGuide.init();
      } else if (
        this.vuex_userRole == "R" &&
        this.$u.getPinia("guide.guidanceR") != 1
      ) {
        this.$refs.FunctionGuide.init();
      }
    },
...
.Indexbg1 {
  background: url("https://res-oss.elist.com.cn/wxImg/index/indexbg.png")
    no-repeat left top #f6f7f7;
  background-size: 100% auto;
}

.Indexbg2 {
  background: url("https://res-oss.elist.com.cn/wxImg/index/indexR.png")
    no-repeat left top #f6f7f7;
  background-size: 100% auto;
}
/* #ifndef MP-WEIXIN */
@font-face {
  ...
  src:
    url("https://res-oss.elist.com.cn/wxImg/fonts/aliBold/Xizk3ttTNIiT.woff2")
      format("woff2"),
    url("https://res-oss.elist.com.cn/wxImg/fonts/aliBold/O28WvFtJWcqI.woff")
      format("woff");

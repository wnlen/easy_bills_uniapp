  ...
  <view class="Index" @click="closeRole">
    <view
      ...
      :style="{
        backgroundImage:
          vuex_userRole != 'R'
            ? 'url(https://res-oss.elist.com.cn/wxImg/index/indexbg.png)'
            : 'url(https://res-oss.elist.com.cn/wxImg/index/indexR.png)',
        backgroundSize: 'cover',
      }"
    >
      <view
        class="flex-row justify-left items-center"
        style="
          width: 100%;
          height: 70%;
          background-color: transparent;
      ...
      >
        <view class="mt16 ml14 mr15">
          ...
          <u-avatar
            ...
            @click="goPath('')"
            :src="
              vuex_user.data.headPortrait
                ? vuex_user.data.headPortrait
                : ImgUrl + '/wxImg/index/mr.svg'
            "
          ></u-avatar>
          <!-- #endif -->
          <!-- #ifdef APP -->
          <u-avatar
            ...
            :src="
              vuex_user.data.headPortrait
                ? vuex_user.data.headPortrait
                : 'https://res-oss.elist.com.cn/wxImg/index/mr.svg'
            "
          ></u-avatar>
          <!-- #endif -->
        </view>
        <view
          class="flex-col justify-center items-start"
          style="width: 50%"
          @click="goPath('')"
        >
          <view v-if="isLogin">
    ...
    <view
      class="home-main pl0 pr0 width100 mt10"
    ...
    >
      <view class="home-box">
        <view class="" v-if="vuex_userRole == 'R'">
          <u-notice-bar
            ...
            speed="80"
            :list="[
              vuex_user.chickenBloodTip ||
                '欢迎使用易单据切换角色选择发货模式可使用～',
            ]"
          ></u-notice-bar>
        </view>
        <view
          v-if="vuex_userRole == 'D'"
          class="home-data cardShowPlus"
          style="background-color: #ffffff"
        >
          <view class="">
            <view class="flex-col height80 justify-left mt15 ml12">
              <view
                class="flex-row items-center justify-left pb10 width100"
                style="
                  display: flex;
              ...
              >
                <view class="flex-row" style="width: 300rpx; z-index: 999">
                  <text class="mr5 ft-bold ft32" style="letter-spacing: 2rpx"
                    >销售额(今年)</text
                  >
                  <view
                    @click="isLook = !isLook"
            ...
            <view class="view" @click="goList(2)">
              ...
              <view class="flex-row items-center justify-center mt8">
                ...
                <view class="" v-if="isLook">
                  ...
                  ></u-count-to>
                  <text
                    style="font-size: 29.68rpx; font-weight: bold"
                    v-if="ifBig(priceObj[2])"
                  >
                    {{ BigSub(priceObj[2]) }}
                  </text>
      ...
      <view
        class="towCard flex-col justify-center height300 ml24 mr24"
        style="background-color: transparent"
      >
        <view
          class="home-menu-wrap flex-row justify-center width100"
          style="background-color: transparent"
        >
          <view
            v-if="vuex_userRole == 'R'"
          ...
          >
            <view class="flex-col">
              ...
              <text class="ft24 ft-lighgray mt5" style="color: #e19306"
                >快速查询单据</text
              >
            </view>
          </view>
          <view
            ...
            :style="{
              backgroundImage:
                'url(https://res-oss.elist.com.cn/wxImg/index/yjkd.png)',
            }"
          ...
          >
            <view class="flex-col">
              ...
              <text class="ft24 ft-lighgray mt5 ml10" style="color: #d46d45"
                >便捷高效创建</text
              >
            </view>
          </view>
          <view
            ...
            :style="{
              backgroundImage:
                'url(https://res-oss.elist.com.cn/wxImg/index/ddtj.png)',
            }"
            @click="goPath('/pages/subStatistics/statistics')"
          >
            <view class="flex-col">
              <text class="ft36 ml10" style="color: #1d5594">订单统计</text>
              <text class="ft24 ft-lighgray mt5 ml10" style="color: #1d5594"
                >下载统计PDF</text
              >
            </view>
          </view>
      ...
      <view
        class="sudoku u-margin-top-30 ml24 mr24 pb48 cardShowPlus"
        v-if="vuex_userRole != 'R'"
        style="border-radius: 12rpx"
      >
        <view class="grid-container">
          <view
            class="grid-item mt10 items-center justify-center"
            @click="goList(item.type)"
            v-for="(item, index) in Sudoku.notLogD[0]"
            :key="index"
          >
        ...
        <view class="grid-container">
          <view
            ...
            v-for="(item, index) in Sudoku.notLogD[1]"
            :key="index"
          >
            <u-icon size="60" :name="item.icon"></u-icon>
            <view class="grid-text">{{ item.text }}</view>
        ...
        <view class="grid-container pb6">
          <view
            class="grid-item mt10 relation"
            @click="goPathIndex(item)"
            v-for="(item, index) in Sudoku.notLogD[2]"
            :key="index"
          >
            <view class="justify-center" v-if="!item.chat">
              ...
              <u-badge
                ...
                :count="item.count"
                class="mr30"
                style="position: absolute; top: 0rpx; right: 0rpx"
              ></u-badge>
            </view>
            <view v-else>
              <u-button
      ...
      <view
        class="sudoku u-margin-top-30 ml24 mr24 pl10 pr10 pb48 cardShowPlus"
        v-if="vuex_userRole == 'R'"
        style="border-radius: 12rpx"
      >
        <view class="grid-container">
          <view
            class="grid-item mt10"
            @click="goList(item.type)"
            v-for="(item, index) in Sudoku.notLogR[0]"
            :id="index == 0 ? 'box33' : ''"
            :key="index"
          >
            <u-icon size="60" :name="item.icon"></u-icon>
        ...
        <view class="grid-container">
          <view
            class="grid-item mt10"
            @click="goPath(item.path)"
            v-for="(item, index) in Sudoku.notLogR[1]"
            :key="index"
          >
            <u-icon size="60" :name="item.icon"></u-icon>
            <view class="grid-text">{{ item.text }}</view>
        ...
        <view class="grid-container pb6">
          <view
            class="grid-item mt10"
            @click="goPathIndex(item)"
            v-for="(item, index) in Sudoku.notLogR[2]"
            :key="index"
          >
    ...
    <up-overlay :show="expireShow">
      ...
      >
        ...
        >
          ...
          <!-- #ifdef MP-WEIXIN -->
          <text style="font-size: 32rpx; color: #999999"
            >该{{
              vuex_user.workData.identity == 3 ? "财务" : "分管"
            }}人员权限已到期,请联系</text
          >
          <!-- #endif -->
          <!-- #ifdef APP -->
          ...
          <view
            class="absolute flex-col justify-center items-center"
            style="bottom: 0; height: 160rpx; width: 100%"
            @click="exit"
          >
            <view
              class=""
...
export default {
  ...
  computed: {
    ...
    username() {
      ...
      return (
        this.vuex_user?.data?.name ||
        this.vuex_user?.data?.phoneNumber ||
        "用户"
      );
    },
  },
  ...
  onShow() {
    ...
    if (
      this.vuex_user.phone == undefined ||
      this.vuex_user.phone == "10000000000" ||
      this.vuex_user.phone == null
    ) {
  ...
  methods: {
        ...
        this.getElementData("#box", (res) => {
          this.setFunctionGuideData({
            ...
            tipsPosition: {
              ...
              right: "0",
              backgroundImage:
                "url(https://res-oss.elist.com.cn/wxImg/handbook/guide/stepD1.png)",
            },
            img: {
              url: "",
            },
        ...
        this.getElementData("#box22", (res) => {
          this.setFunctionGuideData({
            ...
            tipsPosition: {
              ...
              left: "0",
              backgroundImage:
                "url(https://res-oss.elist.com.cn/wxImg/handbook/guide/stepR22.png)",
            },
            position: {
              top: res.top + "px",
              width: `${res.width}px`,
        ...
        this.getElementData("#box33", (res) => {
          this.setFunctionGuideData({
            ...
            tipsPosition: {
              ...
              left: "0",
              backgroundImage:
                "url(https://res-oss.elist.com.cn/wxImg/handbook/guide/stepR33.png)",
            },
            position: {
              top: res.top + "px",
        ...
        this.getElementData("#box2", (res) => {
          this.setFunctionGuideData({
            ...
            tipsPosition: {
              ...
              left: "0",
              backgroundImage:
                "url(https://res-oss.elist.com.cn/wxImg/handbook/guide/stepD2.png)",
            },
            position: {
              top: res.top + "px",
              width: `${res.width}px`,
        ...
        this.getElementData("#box6", (res) => {
          this.setFunctionGuideData({
            ...
            tipsPosition: {
              ...
              left: "-40rpx",
              backgroundImage:
                "url(https://res-oss.elist.com.cn/wxImg/handbook/guide/stepD3.png)",
            },
            position: {
              bottom: setting
                ? uni.getSystemInfoSync().statusBarHeight * 0.1 + "px"
    ...
    BigSub(price) {
      if (typeof price !== "number" || price < 1000000) {
        throw new Error(
          "Price must be a number greater than or equal to 10000000.",
        );
      }
    ...
    getData() {
      ...
      realTimeSel.startDate = this.$u.timeFormat(

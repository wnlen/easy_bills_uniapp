<template>
	<view class="page-container">
		...
		<view class="top-img">
			<image class="banner" src="https://res-oss.elist.com.cn/advertising/wenjuan_background.png" mode="widthFix">
			</image>
		</view>
		<view class="page-list">
			<view class="list-item" v-for="(q_item,q_index) in survey_data.questions" :key="q_index">
				...
				<text class="item-title"><text v-show="q_index<9">0</text>{{ q_index+1 }} {{ q_item.title }}</text>
				<text class="item-title-type"
					v-if="q_item.type == 'single' || q_item.type == 'multi'">({{getStatusText(q_item.type)}})</text>
				<view class="list-item-radio" v-if="q_item.type=='single'">
					<u-radio-group
						v-model="FormData[q_item.id].o_id"
						active-color="#ff9900"
						shape="square"
						:wrap="true"
						size="17"
					...
					>
						<u-radio
							...
							@change="radioChange({'q_id':q_item.id,'o_id':o_item.id,'content':o_item.content})"
						>
							{{ o_item.content }}
						</u-radio>
					</u-radio-group>
				...
				<view class="list-item-radio" v-if="q_item.type=='multi'">
					<uv-checkbox-group v-model="FormData[q_item.id].o_ids" activeColor="#ff9900" shape="square"
						size="17" placement="column" @change="(val) => onCheckboxGroupChange(q_item.id, val)">
						<uv-checkbox :customStyle="{marginBottom: '16rpx'}" v-for="(o_item, o_index) in q_item.options"
							:key="o_item.id" :label="o_item.content" :name="o_item.id" labelSize="15" labelColor="#606266"></uv-checkbox>
					</uv-checkbox-group>
			...
			<view class="Note">
				注：仅可领取一次，主体下多成员的累积领取至多四次
			</view>
			<view class="btn_submit">
				<u-button type="warning" shape="circle" size="default" @click="submit" :custom-style="btn_submitStyle">
					提交
				</u-button>
			</view>
			<u-toast ref="uToast1" />

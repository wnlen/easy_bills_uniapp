<template>
  <view class="page-container">
    ...
    <view class="top-img">
      <image
        class="banner"
        src="https://res-oss.elist.com.cn/advertising/wenjuan_background.png"
        mode="widthFix"
      >
      </image>
    </view>
    <view class="page-list">
      <view
        class="list-item"
        v-for="(q_item, q_index) in survey_data.questions"
        :key="q_index"
      >
        <text v-if="q_item.isRequired" class="item-title-required">*</text>
        <text class="item-title"
          ><text v-show="q_index < 9">0</text>{{ q_index + 1 }}
          {{ q_item.title }}</text
        >
        <text
          class="item-title-type"
          v-if="q_item.type == 'single' || q_item.type == 'multi'"
          >({{ getStatusText(q_item.type) }})</text
        >
  ...
  methods: {
    ...
    submit() {
      ...
      for (const [q_id, value] of Object.entries(this.FormData)) {
        const isEmptySelect =
          !value.o_id && (!value.o_ids || value.o_ids.length === 0);

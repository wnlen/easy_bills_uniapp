<template>
	<view style="position: relative;height: 100vh;">
		<view class="ml20 mr48 pd30 bg-white mt10">
			...
			<view class="flex-row pt30 pb30 u-border-bottom items-center justify-between">
				...
				<view class="flex-row justify-end items-center flex-1">
					<view class="flex-row justify-center items-center">
						<view class="flex-col justify-center items-center" @click="check(1)"
					...
					<view class="flex-row justify-center items-center ml24">
						<view class="flex-col justify-center items-center" @click="check(2)"
							:style="{backgroundColor:!type?'#EA7183':'#ffffff'}"
							style="border-radius: 50px;height: 15px;width: 15px;border: 1px solid #AAAAAA;">
							<u-icon name="checkbox-mark" color="#ffffff" size="14"></u-icon>
						</view>
			...
			<view class="flex-row pt30 pb30 u-border-bottom items-center justify-between">
				<text style="color: red;">*<text class="ft-gray ml5">发票抬头</text></text>
				<view class="flex-row justify-end items-center flex-1">
					<input v-model="invoice.name" type="text" maxlength="50" class="text-right ft29"
						placeholder="请输入发票抬头 " />
				</view>
			</view>
			<view v-if="type" class="flex-row pt30 pb30 u-border-bottom items-center justify-between">
				<text style="color: red;">*<text class="ft-gray ml5">税号</text></text>
				<view class="flex-row justify-end items-center flex-1">
					<input v-model="invoice.taxpayerNumber" type="text" maxlength="50" class="text-right ft29"
			...
			<view v-if="type" class="flex-row pt30 pb30 u-border-bottom items-center justify-between">
				...
				<view class="flex-row justify-end items-center flex-1">
					<input v-model="invoice.openingBank" type="text" maxlength="50" class="text-right ft29"
						placeholder="选填" />
				</view>
			</view>
			<view v-if="type" class="flex-row pt30 pb30 u-border-bottom items-center justify-between">
			...
			<view v-if="type" class="flex-row pt30 pb30 u-border-bottom items-center justify-between">
				...
				<view class="flex-row justify-end items-center flex-1">
					<input v-model="invoice.phone" type="number" maxlength="11" class="text-right ft29"
						placeholder="选填" />
				</view>
			</view>
		</view>
		...
		<view class="flex-col justify-center pl60 pr60 pb60 pt60 vw100" style="position: absolute;bottom: 0;">
			...
			<u-button hover-class="none" :custom-style="{backgroundColor:'#47506C'}" shape="circle" type="primary"
				@click="installInvoice">提交申请</u-button>
		</view>

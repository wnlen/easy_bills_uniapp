<template>
  <view class="content pd15 bg-gray">
    <view
      class="vw100 relative flex-col justify-center items-center"
      style="height: 30vh; margin-top: 60%"
      v-if="!allShow && !defShow"
    >
      <u-empty
        icon="https://res-oss.elist.com.cn/wxImg/vip/print.svg"
        iconSize="400rpx"
        text="暂无打印机~"
        mode="search"
        margin-top="-200"
      ...
      <u-button
        ...
        size="medium"
        hover-class="none"
        :custom-style="{ backgroundColor: '#47506C' }"
        shape="circle"
        type="primary"
        @click="buy"
      >
    ...
    <view
      v-if="defShow"
      class="cardPrint flex-col justify-center items-start relative"
      v-for="(item, index) in def"
      :key="index"
    >
      <view class="ml15 flex-row justify-center items-center">
        <u-icon
          name="https://res-oss.elist.com.cn/wxImg/print/print-icon.svg"
          size="80"
        ></u-icon>
        <u-text class="ml15" style="color: #666666; font-size: 14px"
          >默认设备名称：{{ item.name }}</u-text
        >
        <view
          class="ml15"
          :style="{ backgroundColor: item.online ? '#01BB74' : '#F76565' }"
          style="width: 5px; height: 5px; border-radius: 45%"
        ></view>
      </view>
      <view
        ...
        @click="edit(item)"
      >
        <u-text class="mr20" style="color: #01bb74" v-show="item.online"
          >在线</u-text
        >
        <u-text class="mr20" style="color: #f76565" v-show="!item.online"
          >离线</u-text
        >
    ...
    <view
      v-if="allShow"
      class="cardPrintList flex-col justify-start items-start relative mt40"
    >
      <view class="ml15 mt25">其他打印机</view>
      <view
        class="cardPrint flex-col justify-center items-start relative mt20"
        @click="defUpdate(item)"
        v-for="(item, index) in all"
        :key="index"
      >
        <view class="ml15 flex-row justify-center items-center">
          <u-text class="ml15" style="color: #666666; font-size: 14px"
            >设备名称：{{ item.name }}</u-text
          >
        </view>
        <view
          class="absolute flex-row justify-center items-center"

<template>
	<view class="content pd15 bg-gray">
		...
		<view class="vw100 relative flex-col justify-center items-center" style="height: 30vh;margin-top: 60%;"
			v-if="!allShow&&!defShow">
			<u-empty src="https://res-oss.elist.com.cn/wxImg/vip/print.svg" icon-size="400" text="暂无打印机~" mode="search"
				margin-top="-200"></u-empty>

			<u-button class="absolute" style="bottom: 0;" size="medium" hover-class="none"
				:custom-style="{backgroundColor:'#47506C'}" shape="circle" type="primary" @click="buy">去购买</u-button>
		</view>




		<view v-if="defShow" class="cardPrint flex-col justify-center items-start relative" v-for="(item,index) in def"
			:key="index">
			<view class="ml15 flex-row justify-center items-center">
				...
				<view class="ml15" :style="{backgroundColor:item.online?'#01BB74':'#F76565'}"
					style="width: 5px;height: 5px;border-radius: 45%;"></view>
			</view>
		...
		<view v-if="allShow" class="cardPrintList flex-col justify-start items-start relative mt40">
			<view class="ml15 mt25">
				其他打印机
			</view>
			<view class="cardPrint flex-col justify-center items-start relative mt20" @click="defUpdate(item)"
				v-for="(item,index) in all" :key="index">
				<view class="ml15 flex-row justify-center items-center">

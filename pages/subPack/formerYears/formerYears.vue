    ...
    >
      <template #top>
        ...
        <view class="relative" style="width: 100%; height: 5vh">
          ...
          >
            ...
            >
              <view id="data-select1" class="btn-info flex-row justify-center">
                ...
                <text style="color: #965510" v-if="dropdownName != '请选择'"
                  >年</text
                >
              </view>
            </liu-data-select>
          ...
          <view
            class="absolute flex-row justify-end items-center"
      ...
      <view
        class="order-simple-list pl30 pr30"
        v-show="current == 0 && !moneyCALL"
      >
        <div
          class="bg-white pd20 mt20 radius flex-col justify-center items-center cardShow"
        >
          <view class="flex-col" style="width: 100%">
            <text class="ft30 ft-gray mb18" style="color: #999999"
              >累计金额</text
            >
            <view class="">
              <text class="ft42 ft-bold">￥</text>
              <u-count-to
                :end-val="OrderQuantitySum"
                separator=","
                color="#000000"
                font-size="40rpx"
                decimals="2"
          ...
          <view
            class="flex-row justify-center items-center mt20"
            style="width: 100%"
          >
            <view
          ...
          <div
            class="flex-row items-center radius pr20 mr10 mt20"
            style="height: 5vh; background-color: #f9f9f9; width: 100%"
          >
            <div
              class="bg-white flex-row items-center justify-left radius"
              style="width: 100%; height: 5vh; background-color: #f9f9f9"
            >
              <text class="ft11 ft-gray ml36" @click="CustomerGet">{{
      ...
      <view
        class="contentView flex-col justify-center items-center"
        style="height: 600rpx"
        v-show="current == 1"
      >
        <!-- 正在开发中~ -->
      ...
      <view
        class="order-list ml24 mr24 pt10 mt24"
        v-show="current == 0 && !moneyCALL"
      >
        <view
        ...
        >
          <view
            class="u-skeleton-fillet mb10 flex-row"
            @tap.stop
            style="width: 100%"
          >
            <view class="flex-row justify-left items-center" style="width: 70%">
              <view>
                ...
                <view class="flex-row justify-center items-center">
                  ...
                  <view
                    class="ft30 ft-lighgray pr30 ml24"
                    style="color: #666666"
                  >
                    订单编号
                    <text class="ml15" style="color: #f76565">
            ...
            <u-row gutter="16" justify="between">
              <u-col span="20">
                <view class="flex-col items-center text-left">
                  ...
                  >
                    {{ item.organizationE || item.bossNumberE
                  ...
                  <text
                    ...
                    style="font-weight: 500"
                  >
                    {{ item.enterpriseS || item.bossNumberS
                    }}{{ ifZX(item.bossNumberS) ? "(已注销)" : "" }}
                  </text>
                </view>
              </u-col>
            </u-row>
          ...
          <view class="u-skeleton-fillet ft30 line25 ft-lighgray">
            <text
              >日期：{{ $u.timeFormat(item.creationTime, "yyyy-mm-dd") }}</text
            >
          </view>
          <view
            class="ft-lighgray mt10 line25 flex-row items-center justify-end"
          >
            <text>
              <text>订单金额：</text>
      ...
      >
        ...
        >
          <view
            class=""
            style="text-align: left; font-size: 24rpx; color: #965510"
            :disabled="false"
          >
          ...
          >
            <u-icon
              name="https://res-oss.elist.com.cn/wxImg/statistics/down.svg"
              size="40"
            ></u-icon>
            下载PDF
          </view>
      ...
      <view slot="empty" style="padding-bottom: 200rpx">
        <u-icon
          margin-top="22rpx"
          labelPos="bottom"
          :name="ImgUrl + '/wxImg/list/empty.svg'"
          label-color="#AAAAAA"
          label="暂无记录"
          size="180"
        ></u-icon>
      ...
      <view class="flex-col pd30 justify-between height100 pt100">
        <view>
          <view class="flex-col mt40">
            ...
            <view
              class="flex-row items-center justify-between mt10"
              style="width: 100%"
            >
        ...
        <view class="flex-row justify-end mt25 vw100" style="">
          ...
          <view class="mr48" style="float: right">
            ...
            <u-button
              ...
              shape="circle"
              size="medium"
              :custom-style="{ backgroundColor: '#4E5777' }"
              >确定</u-button
            >
          </view>
        </view>
        ...
        <uv-calendars
          mode="range"
          :startDate="limitingTimeO"
          :endDate="limitingTimeT"
          ref="calendars"
          @confirm="date1Change"
        />
      </view>
...
export default {
  ...
  methods: {
    ...
    queryList(pageNo, pageSize) {
      ...
      if (port) {
        var DP =
          this.vuex_user.jurisdiction["B1-" + this.year] ||
          this.vuex_user.jurisdiction["B1"];
        ...
        var RP =
          this.vuex_user.jurisdiction["B2-" + this.year] ||
          this.vuex_user.jurisdiction["B2"];
    ...
    setGD() {
      ...
      var b1y = this.vuex_user.jurisdiction.hasOwnProperty(
        "B1-" + this.dropdownName,
      );
      var b2y = this.vuex_user.jurisdiction.hasOwnProperty(
        "B2-" + this.dropdownName,
      );
      var ifWorkPort = this.vuex_userRole == "R";

      if (ifWorkPort) {
        if (b2) {
          this.uNoticeBarlist.push(
            "往年数据收货端免费截至" +
              this.$u.timeFormat(
                this.vuex_user.jurisdiction["B2"],
                "yyyy-mm-dd",
        ...
        if (b2y) {
          this.uNoticeBarlist.push(
            "往年数据收货端免费截至" +
              this.$u.timeFormat(
                this.vuex_user.jurisdiction["B2-" + this.dropdownName],
                "yyyy-mm-dd",
              ) +
              "到期",
          );
        ...
        if (b1y) {
          this.uNoticeBarlist.push(
            "往年数据发货端免费截至" +
              this.$u.timeFormat(
                this.vuex_user.jurisdiction["B1-" + this.dropdownName],
                "yyyy-mm-dd",
              ) +
              "到期",
          );
    ...
    getInIt() {
      ...
      var timeEmp =
        this.realTimeSel.startDate == "" || this.realTimeSel.endDate == "";
      var ifWorkPort = this.vuex_userRole == "R";
    ...
    getAllYear() {
      ...
      this.$api.order.getOldOrders(dx).then((res) => {
        ...
        if (this.dataList.length > 0) {
          ...
          var timeO = (this.realTimeSel.startDate =
            this.dropdownName + "-01-01");
          var timeT = (this.realTimeSel.endDate = this.getLastMillisecondOfYear(
            this.dropdownName,
          ));
    ...
    dropdown(e) {
      ...
      if (role) {
        var b1 =
          this.vuex_user.jurisdiction["B1-" + e] ||
          this.vuex_user.jurisdiction["B1"];
        console.log("B1：", b1);
        if (b1 != undefined) {
          //时间对比
          ...
          if (!ifbig) {
            ...
            var timeO = (this.realTimeSel.startDate =
              this.dropdownName + "-01-01");
            var timeT = (this.realTimeSel.endDate =
              this.getLastMillisecondOfYear(this.dropdownName));
            this.date1 = this.dropdownName + "-01-01";
            this.date2 = this.getLastMillisecondOfYear(this.dropdownName);
            this.limitingTimeO = this.dropdownName + "-01-01";
            this.limitingTimeT = this.getLastMillisecondOfYear(
              this.dropdownName,
            );
            this.moneyCALL = false;
            this.$refs.paging.refresh();
    ...
    jumpnView() {
      ...
      if (this.dropdownName == "" || this.dropdownName == "请选择") {
        ...
        if (this.jurisdiction && this.orderList.length > 0) {
          uni.navigateTo({
            url:
              "/pages/subPack/formerYears/dataAnalysis?year=" +
              this.dropdownName,
          });
        } else {
          this.$u.toast(
            this.jurisdiction ? "当前无往年数据" : "请购买相关权限",
          );
        }
      }
    },
      ...
      if (this.refresh && timeO && timeT) {
        ...
        this.$api.order
          ...
          .then((res) => {
            ...
            this.totalMoney = orderList.reduce(
    ...
    filterReset() {
      ...
      this.realTimeSel.endDate = this.getLastMillisecondOfYear(
        this.dropdownName,
      );
    ...
    getData() {
      ...
      var phone =
        ...
        this.vuex_user.workData.identity != "4"
          ? this.vuex_user.workData.bossNumber
          : this.vuex_user.phone;
      let zd = false;
      try {
        ...
        zd = false;
        if (
          this.vuex_user.workData == null ||
          this.vuex_user.workData == undefined ||
          this.vuex_user.workData == ""
        ) {
          zd = true;
          console.log("没有工作");
        }
    ...
    loadData() {
      this.totalMoney = this.orderList.reduce(
        (total, obj) => total + obj.price,
        0,
      );
    },

<template>
	<view class="vh100 vw100 fixed-bar-height" style="background-color: #FFFBF1;">
		...
		<z-paging ref="paging" use-virtual-list :force-close-inner-list="true" :hide-empty-view="hideEmptyView"
			...
			<view slot="top">
				<u-navbar :border-bottom="false" :titleBold="true" title-color="#000000" title="往年数据" title-size="34"
					bgColor="transparent" :background="{background:'transparent'}"></u-navbar>
				...
				<view class="relative" style="width: 100%;height: 5vh;">
					<view class="absolute flex-col justify-center items-center" @click="selectionIconClick"
						style="width: 30%;left: 0;height: 100%;">
						<liu-data-select elementId="data-select1" :dataList="dataList" @change="dropdown"
							color="#965510">
							<view id="data-select1" class="btn-info flex-row justify-center">
								<u-icon class="mr10" v-show="!selectionIcon" name="arrow-down-fill" color=" #965510"
									size="15"></u-icon>
								<u-icon class="mr10" v-show="selectionIcon" name="arrow-up-fill" color=" #965510"
									size="15"></u-icon>
					...
					<view class="absolute flex-row justify-end items-center"
						style="width: 70%;right: 0;text-align: right;height: 100%;">
				...
				<div class="bg-white pd20 mt20 radius flex-col justify-center items-center cardShow">
					...
					<view class="flex-row justify-center items-center mt20" style="width: 100%;">
						<view class="flex-row justify-center items-center mr24" @click="choiceGET(0)"
							:style="{backgroundColor:'#ffffff',color:showOrderTage=='0'?'#965510':'#999999',border:'1px solid '+(showOrderTage=='0'?'#965510':'#999999')}"
							style="color: #ffffff;background-color: #965510;width: 20%;height: 27px;border-radius: 6px;height: 27.42px;">
							全部
						</view>
						<view class="flex-row justify-center items-center mr24" @click="choiceGET(1)"
							:style="{backgroundColor:'#ffffff',color:showOrderTage=='1'?'#965510':'#999999',border:'1px solid '+(showOrderTage=='1'?'#965510':'#999999')}"
							style="color: #965510;background-color: #F2FBF8;width: 20%;height: 27px;border-radius: 6px;height: 27.42px;">
							{{vuex_userRole=='R'?'待确收':'待签收'}}

						</view>
						<view class="flex-row justify-center items-center " @click="choiceGET(2)"
							:style="{backgroundColor:'#ffffff',color:showOrderTage=='2'?'#965510':'#999999',border:'1px solid '+(showOrderTage=='2'?'#965510':'#999999')}"
							style="color: #965510;background-color: #F2FBF8;width: 20%;height: 27px;border-radius: 6px;height: 27.42px;">
							已签收
						</view>
						<view class="flex-row justify-center items-center ml20" @click="choiceGET(3)"
							:style="{backgroundColor:'#ffffff',color:showOrderTage=='3'?'#965510':'#999999',border:'1px solid '+(showOrderTage=='3'?'#965510':'#999999')}"
							style="color: #965510;background-color: #F2FBF8;width: 20%;height: 27px;border-radius: 6px;height: 27.42px;">
							{{vuex_userRole=='R'?'已付款':'已收款'}}
						</view>
					</view>

					<div class="flex-row items-center radius pr20 mr10 mt20"
						style="height: 5vh;background-color:#F9F9F9;width: 100%;">
						<div class="bg-white flex-row items-center justify-left radius"
							style="width: 100%;height: 5vh;background-color:#F9F9F9;">
							<text class="ft11 ft-gray ml36"
								@click="CustomerGet">{{vuex_userRole=='R'?'供应商选择':'客户选择'}}</text>
							<u-input class="ml24" style="width: 100%;" @input="changeCustomer" v-model="customer"
					...
					<div class="flex-row items-center radius pr20 mr10 mt20"
						...
						<div class="bg-white flex-row items-center justify-left radius"
							...
							<u-input v-if="showTage!='1'" class="ml24  my-input" style="width: 100%;" v-model="field"
			...
			<view class="flex-col justify-center items-center" style="height: 80vh;" v-show="current==0&&moneyCALL">
				<view class="vw100" style="height: 100px;">
					<u-empty src="https://ydj-lsy.oss-cn-shanghai.aliyuncs.com/applet-img/img/role/empty.svg"
						icon-size="400" text="暂无数据~" mode="search" margin-top="-200"></u-empty>
				</view>
			...
			<view class="contentView flex-col justify-center items-center" style="height:300px;" v-show="current==1">
				<!-- 正在开发中~ -->
							...
							<view>
								...
								<view class="flex-row justify-center items-center">
									<view class="flex-col justify-center items-center"
										...
										:style="{backgroundColor:item.check?'#965510':'#ffffff'}"
			...
			<view v-show="!(current==0&&moneyCALL)" class="pt48" slot="bottom"
				style="background-color: #FFFFFF;box-shadow: 0px 2px 3px 0px rgba(51, 51, 51, 0.2);bottom: 0;height: 10vh;">
				<view class="items-center flex-row justify-center"
					style="width: 92%;display: flex;justify-content: space-between;">
					<view class="" style="text-align: left;font-size: 12px;color: #965510;" :disabled="false">
						<view class="ml24" style="">
							<view class="">
								<view class="flex-row justify-center items-center">
									<view class="flex-col justify-center items-center" @click="checkedAll"
										:style="{backgroundColor:checked?'#965510':'#ffffff'}"
										style="border-radius: 50px;height: 20px;width: 20px;border: 1px solid #AAAAAA;">
										<u-icon name="checkbox-mark" color="#ffffff" size="28"></u-icon>
									</view>
					...
					<view class="mr20 flex-row" style="text-align: left;font-size: 12px;width: 30%;color: #666666;">
						统计：<view style="color: #965510;">{{OrderQuantity}}个订单</view>
					</view>
					<view :disabled="!hasCheck" @click="dlPdf" class="flex-row justify-center items-center" style="width: 140px;height: 40px;border-radius: 45px;opacity: 1;background: linear-gradient(270deg, #464B63 0%, #56638A 100%);color: white;float: right;
							font-weight: 600;">
						<u-icon name="https://res-oss.elist.com.cn/wxImg/statistics/down.svg" size="40"></u-icon>下载PDF
					</view>
				</view>
			</view>
			...
			<view class="flex-col pd30 justify-between height100 pt100">
					...
					<view class="flex-col mt40">
						...
						<!-- 		<view class="flex-row items-center justify-between mt10 vw100">
							<view class="flex-row items-center">
								...
								<input @click="calendar1Show=true" disabled class="ml24"
									style="box-sizing: border-box;border: 0.5px solid #999999;width: 20%;border-radius: 3px;height: 30px;align-items: center;text-align: center;"
						...
						<view class="flex-row items-center justify-between mt10" style="width: 100%;">
							<view class="flex-row items-center" style="width: 50%;">
								<text class="mr10" style="color: #999999;">开始日期</text>
								<u-icon name="arrow-down-fill" size="10"></u-icon>
								<view @click="calendar1Show=true"
									class="ml24 text-center flex-row justify-center items-center"
									style="border-box;border: 0.5px solid #999999;padding: 12rpx;border-radius: 3px;min-width: 90px;height: 28px;">
									{{date1==""?"开始日期":date1}}
								</view>
							</view>
							<view class="flex-row items-center" style="width: 50%;">
						...
						<view class="mt40">
							...
							<view class="flex-row mt20" style="width: 100%;">
								<view class="text-center items-center mr24 pt5" @click="Filtrate('0')"
									:style="{backgroundColor:showTage=='0'?'#4E5777':'#FFFBF1',color:showTage=='0'?'#ffffff':'#965510'}"
									style="color: #ffffff;background-color: #4E5777;width: 30%;height: 27px;border-radius: 6px;height: 27.42px;">
									联系人
								</view>
								<view class="text-center items-center mr24 pt5" @click="Filtrate('1')"
									:style="{backgroundColor:showTage=='1'?'#4E5777':'#FFFBF1',color:showTage=='1'?'#ffffff':'#965510'}"
									style="color: #4E5777;background-color: #4E5777;width: 30%;height: 27px;border-radius: 6px;height: 27.42px;">
									联系号码
								</view>
								<view class="text-center items-center pt5" @click="Filtrate('2')"
									:style="{backgroundColor:showTage=='2'?'#4E5777':'#FFFBF1',color:showTage=='2'?'#ffffff':'#965510'}"
									style="color: #4E5777;background-color: #F2FBF8;width: 30%;height: 27px;border-radius: 6px;height: 27.42px;">
									{{(vuex_userRole=='R'?'收货地址':'收货地址')}}
								</view>
							</view>
							<view class="flex-row mt20" style="width: 100%;">
								<view class="text-center items-center pt5" @click="Filtrate('3')"
									:style="{backgroundColor:showTage=='3'?'#4E5777':'#FFFBF1',color:showTage=='3'?'#ffffff':'#965510'}"
									style="color: #4E5777;background-color: #F2FBF8;width: 30%;height: 27px;border-radius: 6px;height: 27.42px;">
									产品名称
								</view>
							</view>
				...
				<view class="flex-row justify-end mt25 vw100" style="">
					...
					<view class="mr48" style="float: right;">
						<u-button @click="filterReset" shape="circle" size="medium"
							:custom-style="{'marginRight':'20rpx'}" plain>重置</u-button>
						<u-button class="ml20" type="success" @click="filterSubmit" shape="circle" size="medium"
							:custom-style="{backgroundColor:'#4E5777'}">确定</u-button>
					</view>
				</view>
			</view>
		</u-popup>
		...
		<u-calendar v-model="calendar1Show" btn-type="success" active-bg-color="#01BB74" range-bg-color="#DFF9EF"
			range-color="#333333" mode="date" :min-date="limitingTimeO" :max-date="limitingTimeT"
			@change="date1Change"></u-calendar>
		<u-calendar v-model="calendar2Show" btn-type="success" active-bg-color="#01BB74" range-bg-color="#DFF9EF"
			range-color="#333333" mode="date" :min-date="limitingTimeO" :max-date="limitingTimeT"
	...
	export default {
		...
		methods: {
			...
			setGD() {
				...
				if (ifWorkPort) {
					if (b2) {
						this.uNoticeBarlist.push("往年数据收货端免费截至" + this.$u.timeFormat(this.vuex_user.jurisdiction["B2"],
							'yyyy-mm-dd') + "到期")
					}
					if (b2y) {
						this.uNoticeBarlist.push("往年数据收货端免费截至" + this.$u.timeFormat(this.vuex_user.jurisdiction["B2-" +
							this.dropdownName], 'yyyy-mm-dd') + "到期")
					}
				} else {
					if (b1) {
						this.uNoticeBarlist.push("往年数据发货端免费截至" + this.$u.timeFormat(this.vuex_user.jurisdiction["B1"],
							'yyyy-mm-dd') + "到期")
					}
			...
			dlPdf() {
				if (this.checked) {
					...
					this.$u.post('/edo/order/getFilter', dx)
						.then(res => {
			...
			downPdf(okDown) {
				...
				var condition = this.showTage == 0 ? "联系人" : (this.showTage == 1 ? "联系号码" : (this.showTage == 2 ? "收货地址" :
					(this.showTage == 3 ? "产品名称" : (""))))
				if (this.field == "") {
					...
					uni.downloadFile({
						...
						success: res => {
							...
							uni.getFileSystemManager().saveFile({
								...
								filePath: wx.env.USER_DATA_PATH + '/' + "YDJ-" + name + "-" +
									timeNowGet + '.pdf', //自定义文件名
								success(res) {
									uni.openDocument({
			...
			getData() {
				let that = this;
				var phone = this.vuex_user.data.work == "1" && this.vuex_user.workData.identity != "4" ? this.vuex_user
					.workData.bossNumber : this.vuex_user.phone;
				let zd = false;
				try {
					...
					zd = false;
					if (this.vuex_user.workData == null || this.vuex_user.workData == undefined || this.vuex_user
						.workData == "") {
						zd = true;
						console.log("没有工作");
					}

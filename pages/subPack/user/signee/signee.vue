<template>
	<view class="vh100 vw100 fixed-bar-height bg-gray fixed-bar-height">
		<view class="flex-col pt100" v-if="orderList.length==0&&!identity">
			<u-empty :show="isEmpty" src="https://ydj-lsy.oss-cn-shanghai.aliyuncs.com/applet-img/img/role/empty.svg"
				icon-size="400" text="暂无签收人~" mode="search" margin-top="200"></u-empty>
			<view class="mt25 vw100 text-center" style="font-size: 14px;">
				<u-button @click="goPath('/pages/subPack/user/signee/add')" hover-class="none"
					:custom-style="{backgroundColor:'#01BB74'}" :disabled="orderList.length>=1" type="primary" class=""
					shape="circle" size="medium">新增签收人</u-button>
			</view>
		</view>
		...
		<view class="order-simple-list  pt24" v-if="!identity">
			<view v-for="(item, index) in orderList" :key="index" class="flex-col pd24 ml24 mr24 mb24 bg-white radius">
				<view class="flex-row items-center justify-between">
					...
					<!-- #ifdef MP-WEIXIN -->
					<u-icon class="mr20" name="edit-pen" color="#aaa" size="34"
						@click.stop="goDetails(item,1)"></u-icon>
					<u-icon class="ml30 mr20" name="trash" color="#aaa" size="34"
						@click="goDetails(item.id,2)"></u-icon>
					<!-- #endif -->
					<!-- #ifdef APP -->
					<u-icon class="mr20" name="edit-pen" color="#aaa" size="34"
						@click="goDetails(item,1)"></u-icon>
					<u-icon class="ml30 mr20" name="trash" color="#aaa" size="34"
						@click="goDetails(item.id,2)"></u-icon>
					<!-- #endif -->
				</view>
				<view class="u-border-top pt30 mt30 flex-row items-center justify-between">
					<u-icon v-if="item.ifDefault!='Y'" label-size="24" name="checkmark-circle-fill" color="#01BB74"
						size="38" label="默认签收人"></u-icon>
					<u-icon @click="defaultItem(item.id)" v-else name="minus-circle" label-size="24" color="#ccc"
						size="38" label="默认签收人"></u-icon>
					<!-- 			<text class="ft-gray ft24" @click="deleteItem(item.id)">删除</text> -->
				</view>
				<u-popup v-model="show" mode="center" width="500rpx" height="400px" close-icon-size="30"
					:closeable="true" close-icon-color="#000000" close-icon-pos="top-right">
					<u-image style="transform: rotate(90deg)" width="500rpx" height="400px"
						:src="item.signatureImg"></u-image>
				</u-popup>
					...
					<u-popup class="flex-col justify-center items-center" border-radius="15" mode="center"
						...
						<view class="flex-col justify-center items-center relative" style="height: 100%;width: 100%;">
							<view class="absolute pt20" style="width: 100%;top: 0;height: 75%;">
								...
								<view class="flex-col items-center justify-center mt20"
									style="width: 100%;height: 35%;">
									<u-message-input :bold="false" @change="change" @finish="finish" :dot-fill="true"

  ...
  <view class="vh100 vw100 fixed-bar-height bg-gray fixed-bar-height">
    ...
    <view class="order-simple-list pt24" v-if="!identity">
      <view
        ...
        :key="index"
        class="flex-col pd24 ml24 mr24 mb24 bg-white radius"
      >
        <view class="flex-row items-center justify-between">
          <view class="mr20">
            <u-icon
              name="https://res-oss.elist.com.cn/wxImg/user/qm.svg"
              size="80rpx"
            ></u-icon>
          </view>
        ...
        <view
          class="u-border-top pt30 mt30 flex-row items-center justify-between"
        >
          <u-icon
            v-if="item.ifDefault != 'Y'"
            label-size="12"
        ...
        <up-overlay :show="showMask" @click="showMask = false">
          ...
          >
            ...
            >
              <view
                class="absolute pt20"
                style="width: 100%; top: 0; height: 75%"
              >
    ...
    finish(e) {
      ...
      if (pas) {
        if (type == 1) {
          uni.navigateTo({
            url:
              "/pages/subPack/user/signee/add?item=" + JSON.stringify(this.val),
          });
        }
        if (type == 2) {

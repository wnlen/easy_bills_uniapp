...
<!-- 			<u-form-item label="其他备注" prop="remark" label-width="170">
			...
			<u-form-item label="手写签名" required prop="signeeImage" label-position="top" label-width="170">
				<view v-if="formData.signeeImage" class="text-center width100" style="height:50vw;border: 2px dashed #eee;">
					<image @click="goPath('/pages/subSignature/signature')" style="transform: rotate(-90deg);height:50vw;width: 100%;" :src="formData.signeeImage" mode="heightFix"></image>
				</view>
			...
			submit() {
				...
				this.$refs.uForm.validate(valid => {
					if (valid) {
						...
						this.$u.post('/edo/signature/add',dx).then(res => {
							this.$u.toast("保存成功～");
							setTimeout(()=>{
								this.loadData()
								uni.navigateBack({

<template>
  <view class="vw100 vh100 bg-gray" style="overflow: hidden">
    <view
      v-if="vuex_work == 'Y'"
      class="ml24 mr24 pd24 mt24 bg-white"
    ...
    <view
      v-if="vuex_work == 'N'"
      class="ml24 mr24 pt25 pd24 bg-white mt30"
      @click="clear()"
      style="height: auto; border-radius: 12px"
    >
      <view class="flex-row justify-left" style="height: 10%; width: 100%">
      ...
      <view
        class="flex-row justify-left"
        v-if="userUrl['1'].length == 0"
        style="height: 90%; width: 100%"
      >
        <view class="flex-row justify-center items-center" style="width: 10%">
          <u-icon
            class="mt20"
            name="https://res-oss.elist.com.cn/wxImg/vip/init.svg"
            :label="item.name"
            shape="circle"
            size="80"
            @click=""
      ...
      <view
        class="flex-col justify-left pt20 pb20"
        v-for="(item, index) in userUrl['1']"
        :key="index"
        style="height: 90%; width: 100%"
      >
        <view class="flex-row justify-left items-center mt20">
          ...
          >
            ...
            <text
              class="ml15 jzzh"
              :style="{ color: expireDate(item.endTime) ? '' : '#FA5151' }"
              >账号截止到{{ $u.timeFormat(item.endTime, "yyyy-mm-dd") }}</text
            >
            <view
              v-if="item.staffNumber == 0"
              @click="RenewPeople(item, 1)"
              class="xf flex-col justify-center items-center ml24"
              style=""
              >续费</view
            >
          </view>
          ...
          >
            ...
            <view class="flex-col ml20">
              <view class="flex-row items-center">
                ...
                <view
                  @click="RenewPeople(item, 1)"
                  class="ml24 xf flex-col justify-center items-center"
                  >续费</view
                >
              </view>
              <text
                class="ml15 jzzh mt20"
                :style="{ color: expireDate(item.endTime) ? '' : '#FA5151' }"
              >
          ...
          <view class="flex-row justify-end items-center" style="width: 95%">
            ...
            <u-icon
              @click="changeUser(1)"
              name=""
              :custom-style="{ margin: '0 0 0 30rpx' }"
              size="38"
              class="mr30"
            ></u-icon>
          </view>
    ...
    >
      ...
      <view
        class="flex-col justify-left mt14 pt20 pb20"
        v-for="(item, index) in userUrl['4']"
        :key="index"
        style="height: 90%; width: 100%"
      >
        <view class="flex-row justify-left items-center mt20">
...
export default {
  ...
  methods: {
    ...
    CrearOrderBuy(json, index) {
      ...
      productCode.CODE =
        index == 3 ? ["C1-1"] : index == 1 ? ["C3-1"] : ["C2-1"];
      ...
      if (index == 4) {
        if (!this.system) {
          this.$refs.popRenew.src =
            this.imgListIOS.yggm + "?time=" + this.getTimestamp();
        } else {
          this.$refs.popRenew.src =
            this.imgList.yggm + "?time=" + this.getTimestamp();
        }
      } else if (index == 3) {
        if (!this.system) {
          this.$refs.popRenew.src =
            this.imgListIOS.cwgm + "?time=" + this.getTimestamp();
        } else {
          this.$refs.popRenew.src =
            this.imgList.cwgm + "?time=" + this.getTimestamp();
        }
      } else if (index == 1) {
        if (!this.system) {
          this.$refs.popRenew.src =
            this.imgListIOS.hhrgm + "?time=" + this.getTimestamp();
        } else {
          this.$refs.popRenew.src =
            this.imgList.hhrgm + "?time=" + this.getTimestamp();
        }
      }
    ...
    CrearOrder(json, index) {
      ...
      if (index == 4) {
        if (!this.system) {
          this.$refs.popRenew.src =
            this.imgListIOS.ygxf + "?time=" + this.getTimestamp();
        } else {
          this.$refs.popRenew.src =
            this.imgList.ygxf + "?time=" + this.getTimestamp();
        }
      } else if (index == 3) {
        if (!this.system) {
          this.$refs.popRenew.src =
            this.imgListIOS.cwxf + "?time=" + this.getTimestamp();
        } else {
          this.$refs.popRenew.src =
            this.imgList.cwxf + "?time=" + this.getTimestamp();
        }
      } else if (index == 1) {
        if (!this.system) {
          this.$refs.popRenew.src =
            this.imgListIOS.hhrxf + "?time=" + this.getTimestamp();
        } else {
          this.$refs.popRenew.src =
            this.imgList.hhrxf + "?time=" + this.getTimestamp();
        }
      }
      this.$refs.popRenew.roleShow = true;
    },

<template>
	<view class="vw100 vh100 bg-gray" style="overflow: hidden;">
		...
		<view v-if="vuex_work  == 'Y'" class="ml24 mr24 pd24 mt24 bg-white" style="height: auto;border-radius: 12px">
			...
			<view class="flex-row justify-left" style="height: 90%;width: 100%;">
				<view class="flex-row justify-left items-center" style="width: 10%;">
					<u-image class="ml20 mt15" width="80rpx" height="80rpx" shape="circle" :show-menu-by-longpress="false"
						:src="vuex_user.workData.img||img" @click="addStaff(item)"></u-image>
					<text class="ml10">{{vuex_user.workData.bossNumber}}</text>
				</view>
				<view class="flex-row justify-end" style="width: 90%;">
				</view>
			</view>
		</view>
		...
		<view v-if="vuex_work  == 'N'" class="ml24 mr24 pt25 pd24 bg-white mt30" @click="clear()"
			...
			<view class="flex-row justify-left" style="height: 10%;width: 100%;">
				...
				<view class="absolute" style="width: 66vw;right: 48rpx;">
					<button class="flex-col justify-center items-center" @click.stop="addRoleSet(1)" style="background-color: #00B578;width: 60px;height: 25px;
							border-radius: 254px;font-size: 12px;color: white;float: right;
							border: 1px solid transparent;border: none; border-style: none;
							">邀请</button>

					<view class="flex-col justify-center items-center pd15" v-show="showAl==1" style="background-color: #666666;width: 143px;height: 100px;position: absolute;right: 18px;top: 51px;z-index: 999;color: #ffffff;border-radius: 4px;
					           opacity: 1;transition: opacity 10s ease-in-out;">
						<view class=""
							style="width: 0;height: 0;border-left: 10px solid transparent;border-right: 10px solid transparent;border-bottom: 10px solid #666666;position: absolute;right: 5px;top: -8px;">
						</view>
			...
			<view class="flex-row justify-left" v-if="userUrl['1'].length==0" style="height: 90%;width: 100%;">
				<view class="flex-row justify-center items-center" style="width: 10%;">
					<u-icon class="mt20" name="https://res-oss.elist.com.cn/wxImg/vip/init.svg" :label="item.name"
						shape="circle" size="80" @click=""></u-icon>
				</view>
			</view>
			<view class="flex-col justify-left pt20 pb20" v-for="(item,index) in userUrl['1']" :key="index"
				...
				<view class="flex-row justify-left items-center mt20">
					...
					<view v-if="item.name==''" class="flex-row justify-left items-center" style="width: 10%;">
						<u-image class="" width="80rpx" height="80rpx" shape="circle" :src="item.img||img" :show-menu-by-longpress="false"
							@click="addStaff(item)"></u-image>
						<text class="ml10">{{item.name}}</text>
						<text class="ml15 jzzh"
							:style="{color:expireDate(item.endTime)?'':'#FA5151'}">账号截止到{{$u.timeFormat(item.endTime, 'yyyy-mm-dd')}}</text>
						<view v-if="item.staffNumber==0" @click="RenewPeople(item,1)"
							class="xf flex-col justify-center items-center ml24" style="">
							续费
						</view>
					</view>

					<view v-else class="flex-row justify-left items-center" style="width: 10%;">
						<u-image class="" width="50px" height="50px" shape="circle" :src="item.img||img" :show-menu-by-longpress="false"
							@click="addStaff(item)"></u-image>
						<view class="flex-col ml20">
							<view class="flex-row items-center">
								<text class="ml10 ft16">{{item.name}}</text>
								<view @click="RenewPeople(item,1)" class="ml24 xf flex-col justify-center items-center">
									续费
								</view>
							</view>
							<text class="ml15 jzzh mt20"
								:style="{color:expireDate(item.endTime)?'':'#FA5151'}">账号截止到{{$u.timeFormat(item.endTime, 'yyyy-mm-dd')}}</text>
						</view>
					</view>

					<view class="flex-row justify-end items-center" style="width: 95%;">
						<u-icon v-if="item.name!=''" @click="deleteRole(item)" name="trash" size="35" color="#333333"
							class="mr15"></u-icon>

						<u-icon @click="changeUser(1)" name="" :custom-style="{'margin':'0 0 0 30rpx'}" size="38"
							class="mr30"></u-icon>
					</view>
				</view>
		...
		<view v-if="vuex_work  == 'N'" class="ml24 mr24 pt25 pd24 bg-white mt30" @click="clear()"
			...
			<view class="flex-row justify-left" style="height: 10%;width: 100%;">
				...
				<view class="absolute" style="width: 66vw;right: 48rpx;">
					<button class="flex-col justify-center items-center" @click.stop="addRoleSet(3)" style="background-color: #00B578;width: 60px;height: 25px;
							border-radius: 254px;font-size: 12px;color: white;float: right;
							border: 1px solid transparent;border: none; border-style: none;
							">邀请</button>

					<view class="flex-col justify-center items-center pd15" v-show="showAl==3" style="background-color: #666666;width: 143px;height: 100px;position: absolute;right: 18px;top: 51px;z-index: 999;color: #ffffff;border-radius: 4px;
					           opacity: 1;transition: opacity 10s ease-in-out;">
						<view class=""
							style="width: 0;height: 0;border-left: 10px solid transparent;border-right: 10px solid transparent;border-bottom: 10px solid #666666;position: absolute;right: 5px;top: -8px;">
						</view>
			...
			<view class="flex-col justify-left pt20 pb20" v-for="(item,index) in userUrl['3']" :key="index"
				style="height: 90%;width: 100%;">
				<view class="flex-row justify-left items-center mt20">
					...
					<view v-if="item.name==''" class="flex-row justify-left items-center" style="width: 10%;">
						...
						<text class="ml15 jzzh"
							:style="{color:expireDate(item.endTime)?'':'#FA5151'}">账号截止到{{$u.timeFormat(item.endTime, 'yyyy-mm-dd')}}</text>
						<view v-if="item.staffNumber==0" @click="RenewPeople(item,3)"
							class="xf flex-col justify-center items-center ml24" style="">
							续费
						</view>
					</view>
					...
					<view v-else class="flex-row justify-left items-center" style="width: 10%;">
						...
						<view class="flex-col ml20">
							<view class="flex-row items-center">
								<text class="ml10 ft16">{{item.name}}</text>
								<view @click="RenewPeople(item,3)" class="ml24 xf flex-col justify-center items-center">
									续费
								</view>
							</view>
							<text class="ml15 jzzh mt20"
					...
					<view class="flex-row justify-end items-center" style="width: 95%;">
						...
						<u-icon @click="changeUser(3)" name="" :custom-style="{'margin':'0 0 0 30rpx'}" size="38"
							class="mr30"></u-icon>
					</view>
				</view>
				<!-- <u-divider class="mt20" border-color="#F9F9F9" half-width="290" :use-slot="false"></u-divider> -->
		...
		<view v-if="vuex_work  == 'N'" class="ml24 mr24 pt25 pd24 bg-white mt30" @click="clear()"
			...
			<view class="flex-row justify-left" style="height: 10%;width: 100%;">
				...
				<view class="absolute" style="width: 66vw;right: 48rpx;">
					<button class="flex-col justify-center items-center" @click.stop="addRoleSet(4)" style="background-color: #00B578;width: 60px;height: 25px;
							border-radius: 254px;font-size: 12px;color: white;float: right;
							border: 1px solid transparent;border: none; border-style: none;
							">邀请</button>
					<view class="flex-col justify-center items-center pd15" v-show="showAl==4" style="background-color: #666666;width: 143px;height: 100px;position: absolute;right: 18px;top: 51px;z-index: 999;color: #ffffff;border-radius: 4px;
					           opacity: 1;transition: opacity 10s ease-in-out;">
						<view class=""
							style="width: 0;height: 0;  border-left: 10px solid transparent;  border-right: 10px solid transparent;  border-bottom: 10px solid #666666;position: absolute;right: 5px;top: -8px;">
						</view>
			...
			<view class="flex-col justify-left mt14 pt20 pb20" v-for="(item,index) in userUrl['4']" :key="index"
				style="height: 90%;width: 100%;">
				...
				<view class="flex-row justify-left items-center mt20">
					...
					<view v-if="item.name==''" class="flex-row justify-left items-center" style="width: 10%;">
						...
						<text class="ml15 jzzh"
							:style="{color:expireDate(item.endTime)?'':'#FA5151'}">账号截止到{{$u.timeFormat(item.endTime, 'yyyy-mm-dd')}}</text>
						<view v-if="item.staffNumber==0" @click="RenewPeople(item,4)"
							class="xf flex-col justify-center items-center ml24" style="">
							续费
						</view>
					</view>
					...
					<view v-else class="flex-row justify-left items-center" style="width: 10%;">
						...
						<view class="flex-col ml20">
							<view class="flex-row items-center">
								<text class="ml10 ft16">{{item.name}}</text>
								<view @click="RenewPeople(item,4)" class="ml24 xf flex-col justify-center items-center">
									续费
								</view>
							</view>
							<text class="ml15 jzzh mt20"
					...
					<view class="flex-row justify-end" style="width: 95%;">
						...
						<u-icon @click="changeUser(4)" name="" :custom-style="{'margin':'0 0 0 30rpx'}" size="38"
							class="mr30"></u-icon>
					</view>
				</view>
				<!-- <u-divider class="mt20" border-color="#F9F9F9" half-width="290" :use-slot="false"></u-divider> -->
		...
		<u-popup border-radius="14" v-model="show" mode="center" width="600rpx" height="150px" :closeable="false"
			style="overflow:inherit;">
			<view class="u-border-top flex-col justify-center items-center pt15"
				style="height: 20%;color: #01BB74;font-size: 18px;font-weight: 500;color: #333333;">
				提示
			</view>
			<view v-if="gs!=undefined&&gs!=''" class="flex-col justify-center items-center pl24 pr24"
				style="height: 50%;width: 100%;font-family: Source Han Sans;
			font-size: 18px;font-weight: normal;line-height: 25.18px;text-align: center;letter-spacing: 0px;font-feature-settings: 'kern' on;color: #666666;">
				该用户已在 {{gs}}，您无法对该用户发起邀请
			</view>
			<view v-if="gs==undefined||gs==''" class="flex-col justify-center items-center pl24 pr24"
				style="height: 40%;width: 100%;font-family: Source Han Sans;
			font-size: 18px;font-weight: normal;line-height: 25.18px;text-align: center;letter-spacing: 0px;font-feature-settings: 'kern' on;color: #666666;">
				该用户已进入其他账户，您无法对该用户发起邀请
			</view>
			<view @click="show=false;gs=''" class="u-border-top flex-col justify-center items-center pb25"
		...
		methods: {
			...
			CrearOrderBuy(json, index) {
				...
				let order = {
					...
					payment: (parseFloat(jsonI.priceDiscount) * 100).toFixed(
			...
			getPricePeopleAll(index) {
				this.$u.post('edo/jurisdiction/people').then(res => {
					console.log(res);
			...
			getPriceAll(id, phone, index, type) {
				...
				this.$u.post('edo/jurisdiction/getJOpenJurisdiction', dx).then(res => {
			...
			loadDataPeop() {
				var that = this;
				this.$u.post('/edo/relation/get', {
					"bossNumber": that.vuex_user.phone
				}).
				then(res => {
			...
			deleteRole(val) {
				...
				uni.showModal({
					...
					success: (res) => {
						if (res.confirm) {
							console.log(val);
							that.$u.post('/edo/relation/del', val).
							then(res => {
								console.log(res);
								this.loadDataPeop()
							}).catch(res => {
			...
			verification(phone, json, e) {
				this.$u.post('edo/user/search?phone=' + phone + '&state=1').then(res => {
			...
			addYg(json) {
				...
				this.$u.post('edo/user/search?phone=' + addPhone + '&state=1').then(res => {
			...
			addFriendYG(e, img, invite) {
				if (e == this.vuex_user.phone) {
					...
					this.$u.post('edo/apply/add', JSON.stringify(yg)).then(res => {

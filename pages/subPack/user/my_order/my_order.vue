<template>
	<view class="Myorder">
		...
		<view class="handNavbarBackground">
			<view class="handTab">
				<view class="Tab">
					<u-tabs bg-color="linear-gradient(270deg, #323243 0%, #47506C 72%)"
						:active-item-style="{backgroundColor: 'transparent'}" inactive-color="#D6AE83"
						:itemStyle="{backgroundColor:'#323243'}" active-color="#D6AE83" :list="TabList"
		...
		<view class="BodyQBQY" v-show="current==0">
			<scroll-view class="BodyQBQYScrollView" :show-scrollbar="true" scroll-y="true" height="100%">
				...
				<view class="BodyQBQYScrollView_CardOne">
					<view class="card_people" v-for="(item,index) in c_product_statistics" :key="index" :style="{border:(index==0?check.A.checkA:check.A.checkB)?'2rpx solid #D6AE83':'',
						...
						<view class="card_people_title">
							<view class="card_people_title_one">
								<view class="relative">
									{{item.product||''}}
									<!-- #ifdef MP-WEIXIN -->
									<u-icon @tap.stop @click="popDetails(1,item)" class="mb10 ml5 absolute" size="23"
										name="question-circle" color="#AAAAAA"></u-icon>
									<!-- #endif -->
									<!-- #ifdef APP -->
									<u-icon @click="popDetails(1,item)" class="mb10 ml5 absolute" size="23"
										name="question-circle" color="#AAAAAA"></u-icon>
									<!-- #endif -->
								</view>
							</view>
						...
						<view class="card_people_price">
							...
							<view class="grandsonPriceOriginal ml10">
								￥{{item.price||0}}
							</view>
						</view>
						...
						<view class="width40 flex-col mt20" @click="dropdownIcon"
							...
							<liu-data-select elementId="data-select1" :dataList="dataListAll" @change="dropdown">
								<view id="data-select1" class="btn-info flex-row justify-center">
									<u-icon class="mr10" v-show="selectionIcon" name="arrow-down-fill" color="#333333"
										size="15"></u-icon>
									<u-icon class="mr10" v-show="!selectionIcon" name="arrow-up-fill" color="#333333"
										size="15"></u-icon>
							...
							<view class="">
								<view class="flex-row justify-left items-center pb20 pt10" @click="checkboxClickdr(0)">
									...
									<view class="ml15 relative mr48">
										...
										<u-icon @tap.stop @click="popDetails(1,c_product_old_data[0])"
											class="mb10 ml5 absolute" size="23" name="question-circle"
											color="#AAAAAA"></u-icon>
									</view>
									<u-text class="grandsonPriceSign">￥</u-text>
									<u-text
										class="grandsonPrice">{{c_product_old_data[0]?c_product_old_data[0].priceDiscount:0}}/年</u-text>
									<view class="grandsonPriceOriginal ml10">
										￥{{c_product_old_data[0]?c_product_old_data[0].price:0}}
									</view>
								</view>
								...
								<view class="flex-row justify-left items-center pt20 pb10" @click="checkboxClickdr(1)">
									...
									<view class="ml15 relative mr48">
										...
										<u-icon @tap.stop @click="popDetails(1,c_product_old_data[1])"
											class="mb10 ml5 absolute" size="23" name="question-circle"
											color="#AAAAAA"></u-icon>
									</view>
									<u-text class="grandsonPriceSign">￥</u-text>
									<u-text
										class="grandsonPrice">{{c_product_old_data[1]?c_product_old_data[1].priceDiscount:0}}/年</u-text>
									<view class="grandsonPriceOriginal ml10">
										￥{{c_product_old_data[1]?c_product_old_data[1].price:0}}
									</view>
								</view>
							</view>
						</view>
				...
				<view class="PeopleTab" :style="{height:currentVip==0?'26vh':'auto'}">
					<view class="TabRY" :style="{backgroundImage:currentVip==0?tabUrlImg.l:tabUrlImg.r}">
						<!-- <view class="TabRY" :style="{backgroundImage:currentVip==0?tabUrlImg.bgUrlOne:tabUrlImg.bgUrlTow}"> -->
						<view class="" style="width: 100%;">
							<u-tabs :show-bar="false" bg-color="transparent"
								:active-item-style="{backgroundColor: 'transparent'}" inactive-color="#D6AE83"
								:itemStyle="{backgroundColor:'#323243'}" active-color="#333333"
								:list="administrationManagement" :is-scroll="false" :current="currentVip"
								@change="changeVip">
							</u-tabs>
						</view>
					</view>
					...
					<view class="tabBodyOne" v-if="currentVip==0">
						...
						<view class="tabBodyOne_card_body">
							...
							<view class="tabBodyOne_card_body_sum">
								<u-number-box :min="0" bgColorJ="#F7C6CD" bgColorI="#ffffff" colorJ="#FFFFFF"
									colorA="#FFFFFF" bgColorA="#EA7183" v-model="playMoneyListSend.c_product_people.c"
									@change="valChange"></u-number-box>

							</view>
							<view class="tabBodyOne_card_body_price">
								<u-text class="grandsonPriceSign">￥</u-text>
								<u-text class="grandsonPrice"><text
										class="grandsonPriceBold">{{c_product_people[2]?c_product_people[2].priceDiscount:0}}</text>/年</u-text>
								<view class="grandsonPriceOriginal ml10">
									￥{{c_product_people[2]?c_product_people[2].price:0}}
						...
						<view class="tabBodyOne_card_body">
							...
							<view class="tabBodyOne_card_body_sum">
								<u-number-box :min="0" bgColorJ="#F7C6CD" bgColorI="#ffffff" colorJ="#FFFFFF"
									colorA="#FFFFFF" bgColorA="#EA7183" v-model="playMoneyListSend.c_product_people.a"
									@change="valChange"></u-number-box>
							</view>
							<view class="tabBodyOne_card_body_price">
								<u-text class="grandsonPriceSign">￥</u-text>
								<u-text class="grandsonPrice"><text
										class="grandsonPriceBold">{{c_product_people[0]?c_product_people[0].priceDiscount:0}}</text>/年</u-text>
								<view class="grandsonPriceOriginal ml10">
									￥{{c_product_people[0]?c_product_people[0].price:0}}
								</view>
							</view>
							<view class="tabBodyOne_card_body_check" @click="alertTitle">
								<view class="flex-col justify-center items-center"
						...
						<view class="tabBodyOne_card_body">
							...
							<view class="tabBodyOne_card_body_sum">
								<u-number-box :min="0" bgColorJ="#F7C6CD" bgColorI="#ffffff" colorJ="#FFFFFF"
									colorA="#FFFFFF" bgColorA="#EA7183" v-model="playMoneyListSend.c_product_people.b"
									@change="valChange"></u-number-box>
							...
							<view class="tabBodyOne_card_body_price">
								<u-text class="grandsonPriceSign">￥</u-text>
								<u-text class="grandsonPrice"><text
										class="grandsonPriceBold">{{c_product_people[1]?c_product_people[1].priceDiscount:0}}</text>/年</u-text>
								<view class="grandsonPriceOriginal ml10">
									￥{{c_product_people[1]?c_product_people[1].price:0}}
					...
					<view class="tabBodyTwo" v-if="currentVip==1">
						...
						<view class="tabBodyTwoBody" v-for="(item,index) in renew['C3']" :key="index"
							@click="renewCheck(index,renew['C3'])">
							<view class="tabBodyTwoBody_hand">
								<u-avatar :src="item.right||img" mode="circle"></u-avatar>
							</view>
							<view class="tabBodyTwoBody_time">
								<text class="ml15 time"
									:style="{color:expireDate(item.left)?'':'#FA5151'}">{{$u.timeFormat(item.left, 'yyyy-mm-dd')}}到期</text>

							</view>
							<view class="tabBodyTwoBody_price">
								<u-text
									class="grandsonPrice ml30">￥{{c_product_people[2]?c_product_people[2].priceDiscount:0}}/年</u-text>
								<view class="grandsonPriceOriginal ml10">
									￥{{c_product_people[2]?c_product_people[2].price:0}}
								</view>
							</view>
						...
						<view class="tabBodyTwoBody" v-for="(item,index) in renew['C1']" :key="index"
							@click="renewCheck(index,renew['C1'])">
							<view class="tabBodyTwoBody_hand">
								<u-avatar :src="item.right||img" mode="circle"></u-avatar>
							</view>
							<view class="tabBodyTwoBody_time">
							...
							<view class="tabBodyTwoBody_price">
								<u-text
									class="grandsonPrice ml30">￥{{c_product_people[0]?c_product_people[0].priceDiscount:0}}/年</u-text>
								<view class="grandsonPriceOriginal ml10">
									￥{{c_product_people[0]?c_product_people[0].price:0}}
								</view>
							</view>
						...
						<view class="tabBodyTwoBody" v-for="(item,index) in renew['C2']" :key="index"
							@click="renewCheck(index,renew['C2'])">
							<view class="tabBodyTwoBody_hand">
								<u-avatar :src="item.right||img" mode="circle"></u-avatar>
							</view>
							<view class="tabBodyTwoBody_time">
							...
							<view class="tabBodyTwoBody_price">
								<u-text
									class="grandsonPrice ml30">￥{{c_product_people[1]?c_product_people[1].priceDiscount:0}}/年</u-text>
								<view class="grandsonPriceOriginal ml10">
									￥{{c_product_people[1]?c_product_people[1].price:0}}
								</view>
							</view>
			...
			<scroll-view scroll-y="true" class="BodyWDDDScrollView">
				<view v-for="(item,index) in jurisdictionOrdder" :key="index" class="orderCard ml24 mr24 mt48"
					...
					<view class=" pd20 mt25">
						<view v-for="(item2,index2) in item.peopleRenew" v-if="item2.jurisdictionCode=='C1'"
							:key="index2" class="flex-row justify-left relative mt30" style="width: 100%;">
							<view class="mr15" style="width: 15%;">
								<u-avatar :src="img" mode="circle"></u-avatar>
							</view>
							<view class="items-center">
								<view class="flex-col justify-center ddbt" style="height: 50%;">
									人员续费(财务) 一年
								</view>
								<view class="flex-col justify-center dqsj" style="height: 50%;">
						...
						<view v-for="(item2,index2) in item.peopleRenew" v-if="item2.jurisdictionCode=='C2'"
							:key="index2" class="flex-row justify-left relative mt30" style="width: 100%;">
							<view class="mr15" style="width: 15%;">
								<u-avatar :src="img" mode="circle"></u-avatar>
							</view>
						...
						<view v-for="(item2,index2) in item.peopleRenew" v-if="item2.jurisdictionCode=='C3'"
							:key="index2" class="flex-row justify-left relative mt30" style="width: 100%;">
							<view class="mr15" style="width: 15%;">
								<u-avatar :src="img" mode="circle"></u-avatar>
							</view>
						...
						<view class="mt30" style="width: 100%;"
							v-if="IFJCODE(item.peopleRenew,'C1')||IFJCODE(item.peopleRenew,'C1')">
							<u-divider class="mt20" border-color="#F4F4F4" half-width="290"
								:use-slot="false"></u-divider>
						</view>

						<view v-for="(item3,index3) in item.product" :key="index3" v-if="getProductIFC(index3,'B1')"
							class="flex-row justify-left relative mt30" style="width: 100%;">
							<view class="items-center">
								<view class="flex-col justify-center ddbt" style="height: 50%;">
									往年数据{{getProductC(index3)}}年(发货端) 一年
								</view>
								<view class="flex-col justify-center dqsj" style="height: 50%;">
									{{item3}}到期
								</view>
						...
						<view v-for="(item3,index3) in item.product" :key="index3" v-if="getProductIFC(index3,'B2')"
							class="flex-row justify-left relative mt30" style="width: 100%;">
							<view class="items-center">
								<view class="flex-col justify-center ddbt" style="height: 50%;">
									往年数据{{getProductC(index3)}}年(收货端) 一年
								</view>
								<view class="flex-col justify-center dqsj" style="height: 50%;">
									{{item3}}到期
								</view>
						...
						<view v-for="(item3,index3) in item.product" :key="index3" v-if="getProductIFC(index3,'A1')"
							class="flex-row justify-left relative mt30" style="width: 100%;">
							<view class="items-center">
								<view class="flex-col justify-center ddbt" style="height: 50%;">
									订单统计(近三月数据)一年
						...
						<view v-for="(item3,index3) in item.product" :key="index3" v-if="getProductIFC(index3,'A2')"
							class="flex-row justify-left relative mt30" style="width: 100%;">
							<view class="items-center">
								<view class="flex-col justify-center ddbt" style="height: 50%;">
									订单统计(近一年数据)一年
						...
						<view v-for="(item3,index3) in item.product" :key="index3" v-if="getProductIFC(index3,'C1')"
							class="flex-row justify-left relative mt30" style="width: 100%;">
							<view class="items-center">
								<view class="flex-col justify-center ddbt" style="height: 50%;">
									新增账号(财务) 一年
						...
						<view v-for="(item3,index3) in item.product" :key="index3" v-if="getProductIFC(index3,'C2')"
							class="flex-row justify-left relative mt30" style="width: 100%;">
							<view class="items-center">
								<view class="flex-col justify-center ddbt" style="height: 50%;">
									新增账号(员工) 一年
						...
						<view v-for="(item3,index3) in item.product" :key="index3" v-if="getProductIFC(index3,'C3')"
							class="flex-row justify-left relative mt30" style="width: 100%;">
							<view class="items-center">
								<view class="flex-col justify-center ddbt" style="height: 50%;">
									新增账号(合伙人) 一年
						...
						<view class="mt30" style="width: 100%;"
							v-if="IFKEY(item.product,'C1')||IFKEY(item.product,'C2')">
							<u-divider class="mt20" border-color="#F4F4F4" half-width="290"
								:use-slot="false"></u-divider>
						</view>


						<view v-for="(item4,index4) in item.product" :key="index4" v-if="getProductIFC(index4,'O1')"
							class="flex-row justify-left relative mt30" style="width: 100%;">
							<view class="items-center">
								<view class="flex-col justify-center ddbt" style="height: 50%;">
									一键开单 半年
						...
						<view v-for="(item4,index4) in item.product" :key="index4" v-if="getProductIFC(index4,'O2')"
							class="flex-row justify-left relative mt30" style="width: 100%;">
							<view class="items-center">
								<view class="flex-col justify-center ddbt" style="height: 50%;">
									一键开单 一年
						...
						<view class="mt30" style="width: 100%;"
							v-if="IFKEY(item.product,'O1')||IFKEY(item.product,'O2')">
							<u-divider class="mt20" border-color="#F4F4F4" half-width="290"
								:use-slot="false"></u-divider>
						</view>
				...
				<view class="BodyWDDDScrollViewEmpty">
	...
	export default {
		...
		methods: {
			...
			Play() {
				...
				let order = {
					orderNumber: "", // 订单编号  
					price: this.playMoneyListSend
						.totalPrice, // 支付总价（注：通常价格应该是一个数字或BigDecimal的字符串表示，但这里按照要求设置为空字符串）  
					payment: (this.playMoneyListSend.totalPrice * 100).toFixed(
						0), // 支付金额（同样，支付金额通常应为数字或字符串形式的金额，这里设为空字符串）  
			...
			add() {
				...
				var statistics = statisticsIndex == 9 ? (a[1].priceDiscount) : (
					statisticsIndex == -
					1 ? 0 : a[statisticsIndex].priceDiscount);


				var old = oldIndex == 3 ? (b[0].priceDiscount + b[1].priceDiscount) : (oldIndex == -1 ? 0 : b[
						oldIndex]
					.priceDiscount)
				var people = (peopleIndexA * c[0].priceDiscount) + (peopleIndexB * c[1].priceDiscount) + (peopleIndexC * c[
					2].priceDiscount)
				...
				this.playMoneyListSend.totalPrice = parseFloat(statistics) + parseFloat(old) + parseFloat(
					people) + parseFloat(c1RenewPrice) + parseFloat(c2RenewPrice) + parseFloat(c3RenewPrice);
			...
			checkboxClickdr(i) {
				...
				this.play.old = !this.check.B.checkA && !this.check.B.checkB ? [0, 1] : (!this.check.B.checkA ? [
					0
				] : (!
					this.check.B.checkB ? [0] : []))

<template>
  <view style="position: relative; height: 100vh">
    <view v-if="show == '0'" class="ml20 mr48 pd30 bg-white mt10">
      ...
      <view
        class="flex-row pt30 pb30 u-border-bottom items-center justify-between"
      >
      ...
      >
        ...
        <view class="flex-row justify-center items-center flex-1">
          <input
            v-model="userInfo.accountSubject.simpleNameQ"
            type="text"
            maxlength="50"
            class="text-left ft29"
            placeholder="请输入企业名称"
          />
        </view>
      </view>
      <view class="flex-row pt30 pb30">
        ...
        <view class="flex-row justify-center items-center flex-1">
          <input
            v-model="userInfo.accountSubject.address"
            type="text"
            maxlength="50"
            class="text-left ft29"
            placeholder="请输入企业地址"
    ...
    <view
      v-if="show == 0"
      class="flex-col justify-center pl60 pr60 pb60 pt60 vw100"
    ...
    >
      <u-button
        ...
        type="primary"
        @click="updateInfo"
        >{{ up ? "提交信息" : "更新信息" }}</u-button
      >
    </view>

    <view v-if="show != 0" class="flex-col mt60 ml48 mr48">
      <view
        class="flex-row justify-between items-center pl20 pr20 pt30 pb30 radius mb10"
      >
        <view class="flex-row items-center">
          ...
          <view
            class="ml30"
            style="max-width: 100px; font-weight: bold; font-size: 16px"
          >
            {{ vuex_user.data.name }}
          </view>
          <u-image
      ...
      <view class="flex-row items-center pd20">
        ...
        <text class="flex-1">{{
          vuex_user.ac ? vuex_user.ac.enterpriseName : ""
        }}</text>
      </view>
      <view class="flex-row items-center pd20">
      ...
      <view class="flex-row items-center pd20">
        ...
        <text class="flex-1">{{
          vuex_user.ac ? vuex_user.ac.businessSite : "未填写"
        }}</text>
      </view>
    </view>
  </view>
...
export default {
  data() {
    return {
      ...
      license:
        "https://ydj-lsy.oss-cn-shanghai.aliyuncs.com/applet-img/img/auth/upload.svg",
    };
  },
  onShow() {
    this.loadData();
  ...
  methods: {
    authRefresh() {
      ...
      this.userInfo.accountSubject.simpleNameQ =
        this.vuex_user.ac.enterpriseName; //企业全称
    ...
    upOrderLogo() {
      ...
      uni.chooseMedia({
        ...
        success(res) {
          ...
          var bossNumber =
            that.vuex_work == "Y"
              ? that.vuex_user.workData.bossNumber
              : that.vuex_user.phone || that.vuex_user.data.phoneNumber;
          uni.uploadFile({

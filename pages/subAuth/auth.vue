<template>
	<view class="attestation" :style="{ backgroundImage: 'url(' + ImgUrl + '/wxImg/user/back.svg' + ')' }">
		<u-navbar
			...
			title="公司资料"
			title-size="34"
			:background="{ background: 'transparent', color: 'white', fontSize: '28rpx' }"
		></u-navbar>
		...
		<view class="card" style="">
			<view class="" v-if="true" style="border-radius: border-radius: 12rpx;">
				...
				<view
					...
					style="height: 10vh; background: #fffef9; border-radius: 12rpx; position: relative; box-shadow: 0rpx 2rpx 6rpx 0rpx rgba(205, 202, 186, 0.3);padding-left: 20rpx"
				>
	...
	methods: {
		...
		jump(i) {
			if (i == 1) {
				...
				let url = this.btnqy == 0 ? '/pages/subAuth/qiye?btn=0' : '/pages/subAuth/qiye?btn=3';
				uni.navigateTo({
					url: url
				});
			}

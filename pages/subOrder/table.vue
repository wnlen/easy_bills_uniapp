		...
		<view class="anchorText">
			<view class="SearchTablePage">
				<view class="SearchTablePageIcon">
					<u-icon name="search" color="#01BB74" size="30"></u-icon>
				</view>
				<view class="SearchTablePageInput">
					<u-input placeholder="请输入关键字进行查找" @input="search" />
				</view>
			</view>
		</view>


		<u-empty v-if="isEmptyObject(client)" src="https://res-oss.elist.com.cn/wxImg/order/empty.svg" icon-size="400"
			text="暂无好友~" mode="search" margin-top="-200"></u-empty>
				...
				<u-collapse arrow-color="#ffffff">
					<u-collapse-item v-if="!ifZX(index)" :title="getCompanyName(item)" arrow=false>
						<view v-for="(item2,index2) in item" :key="index2"
							...
							class="anchor-text ml48" :style="{color:item.state=='2'?'#FA5151':'#333333'}">
							<view class="" style="display: flex;flex-direction: row;padding-top: 5px;color: black;"
								...
								<view class="custom-style ml10"
									:style="{backgroundColor: item2.identity=='0'?'#648AFF':(item2.identity=='1'?'#FF8C8C':'#FFE387'),color:'#ffffff',width:item2.identity=='1'||item2.identity=='0'?'50px':'40px'}">

    ...
    >
      <template #top>
        ...
        <view class="ml24 mr24 flex-row items-center justify-center pb30">
          <view
            ...
            style="background: #ffffff; border-radius: 254rpx; width: 75%"
          >
            <u-icon
              class="ml6"
              name="search"
        ...
        <view class="invCard" @click="jumpAddCommodity">
          <view class="">
            <u-image
              ...
              src="https://res-oss.elist.com.cn/wxImg/order/SlView.png"
            ></u-image>
          </view>
          <view class="invText">
            <text>示例商品 1</text>
      ...
      <view class="invCard" v-for="(item, index) in orderList" :key="index">
        ...
        <view
          class="invText"
          style="width: 300rpx"
          @click="jumpCommodityDetails(item)"
        >
      ...
      <view slot="bottom">
        <view class="bottomCard">
          <view class="relative">
            <u-icon
              name="https://res-oss.elist.com.cn/wxImg/order/merchandiseInventory.png"
              size="110"
              @click="AlertCard"
            ></u-icon>
            <u-badge
              class="absolute"
              type="error"
              :count="orderItemList.length"
              style="right: -20rpx; top: 0"
            ></u-badge>
          </view>
          ...
          <u-button
            shape="circle"
            :custom-style="bottomCustomStyle"
            type="success"
            @click="save"
            >保存</u-button
          >
  ...
  methods: {
      ...
      if (this.orderItemList.length > 0 && this.update == false) {
        uni.showModal({
          ...
          success: (res) => {
            var okif = res.confirm;
            if (okif) {
              var nullNot = this.orderItemList.filter(
                (res) => res.unitPrice == null || res.unitPrice == "",
              );
              console.log(nullNot);
    ...
    EmpytSL(list, type) {
      if (type) {
        ...
        if (list.length == 0 && this.orderList.length <= 0) {
          if (
            this.merchandiseInventory.description == "" &&
            this.merchandiseInventory.specification == ""
          ) {
    ...
    addOrderBill(item) {
      ...
      if (idList.length <= 0) {
        ...
        if (itemToModify) {
          ...
          this.$u.toast(
            itemToModify.description + " + " + itemToModify.quantity,
          );
        }
      }
      this.add();
    ...
    add() {
      ...
      this.orderItemList.forEach((res) => {
        this.totalPrices =
          this.totalPrices + res.quantity * (res.unitPrice ? res.unitPrice : 0);
      });

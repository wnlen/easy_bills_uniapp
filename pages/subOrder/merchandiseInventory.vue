	...
	<view class="modification">
		...
		<z-paging ref="paging" use-virtual-list :force-close-inner-list="true" :showEmpty="true"
				...
				<view class="invCard" @click="jumpAddCommodity">
					<view class="">
						<u-image border-radius="6px" width="200rpx" height="200rpx" :show-menu-by-longpress="false"
							src="https://res-oss.elist.com.cn/wxImg/order/SlView.png"></u-image>
					</view>
					<view class="invText">
						<text>示例商品 1</text>
						<text>规格：00A1</text>
					...
					<u-icon @tab.stop class="absolute" style="bottom: 24rpx;right: 24rpx;" name="plus-circle"
						color="#01BB74" size="50"></u-icon>
				</view>
				<view class="flex-row justify-center items-center mt80 pb24">
					<view class="flex-row justify-center items-center" @click="jumpAddCommodity"
						style="width: 140px;height: 40px;border-radius: 169.2px;box-sizing: border-box;border: 1.5px solid #01BB74;">
						<u-icon class="absolute" label="添加商品" label-color="#01BB74" name="plus" color="#01BB74"
							size="30"></u-icon>
					</view>
				</view>
			</view>
			...
			<view class="invCard" v-for="(item,index) in orderList" :key="index">
				...
				<u-icon @tab.stop class="absolute" style="bottom: 24rpx;right: 24rpx;" name="plus-circle"
					color="#01BB74" size="50" @click="addOrderBill(item)"></u-icon>
			</view>

			<view slot="bottom">
				<view class="bottomCard">
					<view class="relative">
						<u-icon name="https://res-oss.elist.com.cn/wxImg/order/merchandiseInventory.png" size="110"
							@click="AlertCard"></u-icon>
						<u-badge class="absolute" type="error" :count="orderItemList.length"
							style="right: -10px;top:0"></u-badge>
					</view>

					<text class="ml12">
						合计:<text style="color: #01BB74;">￥{{totalPrices}}</text>
					</text>
					<u-button shape="circle" :custom-style="bottomCustomStyle" type="success"
		...
		<u-popup v-model="shoppingTrolley" mode="bottom" border-radius="15">
			<view class="" style="width: 100%;height: 70vh;padding-top: 24rpx;">
				<z-paging v-if="shoppingTrolley" ref="pagingCheck" :fixed="false" v-model="addList"
								...
								<u-tr>
									<u-td>
										<view class="u-border-bottom">
											<input type="digit" v-model="item.quantity" maxlength="10" placeholder="请输入"
												@input="calculate" :custom-style="uploadingCommodityInputStyle" />
										</view>
									</u-td>
									<u-td>{{item.unit}}</u-td>
									<u-td>
										<view class="u-border-bottom">
											<input type="digit" v-model="item.unitPrice" maxlength="10"
												@input="calculate" placeholder="请输入"
					...
					<view slot="bottom">
						<view class="bottomCard">
							<view class="relative">
								<u-icon name="https://res-oss.elist.com.cn/wxImg/order/merchandiseInventory.png"
									size="110" @click="closeOpen"></u-icon>
								<u-badge class="absolute" type="error" :count="orderItemList.length"
									style="right: -10px;top:0"></u-badge>
							</view>
							<text class="ml12">
	...
	export default {
		...
		onLoad(option) {
			...
			if (option.update == 1) {
				...
				if (uni.getStorageSync("updInventoryStockpile") != null && uni.getStorageSync("updInventoryStockpile") !=
					undefined && uni.getStorageSync("updInventoryStockpile") != "") {
				...
				if (uni.getStorageSync("inventoryStockpile") != null && uni.getStorageSync("inventoryStockpile") !=
					undefined && uni.getStorageSync("inventoryStockpile") != "") {
		...
		methods: {
				...
				if (this.orderItemList.length > 0 && this.update == false) {
					uni.showModal({
						...
						success: res => {
							var okif = res.confirm;
							if (okif) {
								var nullNot = this.orderItemList.filter(res => res.unitPrice == null || res
									.unitPrice == "");
								console.log(nullNot);
			...
			addOrderBill(item) {
				...
				var idList = this.orderItemList.filter(
					res => res.id == id && res.description === description && res.specification === specification &&
					res.unit === unit && res.unitPrice == unitPrice
				);
				...
				if (idList.length <= 0) {
					...
					console.log(idList);
					const itemToModify = this.orderItemList.find(res => res.id == id && res.description === description &&
						res.specification === specification && res.unit === unit && res.unitPrice == unitPrice); // 替换为你的条件
					if (itemToModify) {

<template>
  <view class="root">
    ...
    >
      ...
      >
        <view
          class="width100 pb25 text-left flex-row items-center justify-left"
        >
          <view
...
export default {
  ...
  methods: {
    ...
    draftsDel() {
      var checkTrue = this.orderList
        .filter((res) => res.check)
        .map((rew) => rew.id);
    ...
    checkboxGroupChange(event, index) {
      ...
      this.checked =
        check == 0 && this.DraftsAmount ? false : check == this.DraftsAmount;
    },
    jumpVideo() {
      uni.navigateTo({
    ...
    Init() {
      ...
      if (timeEmp) {
        ...
        this.realTimeSel.endDate = this.$u.timeFormat(
          currentDate,
          "yyyy-mm-dd",
        );
    ...
    filterSubmit() {
      ...
      this.realTimeSel.endDate =
        this.date2 != ""
          ? this.date2
          : this.$u.timeFormat(currentDate, "yyyy-mm-dd");
      this.$refs.paging.reload();
    },
  },
};

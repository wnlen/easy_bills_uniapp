		...
		<z-paging ref="paging" use-virtual-list :force-close-inner-list="true"
			...
			<view class="Card cardShow">
				<view class="InputCard">
					...
					<view class="InputOne">
						...
						<view class="flex-col justify-center items-center" style="height: 5vh;">
							<u-icon class="ml40" name="/static/img/list/ss.svg" size="45"></u-icon>
						</view>
					</view>
				</view>
			</view>
			...
			<view v-for="(item,index) in orderList" :key="item.id" :index="index" :id="`zp-id-${item.id}`"
				...
				<view class="width100 pb25 text-left flex-row items-center justify-left">
					<view class="flex-col justify-center items-center mr10"
						:style="{backgroundColor:item.check?'#01BB74':'#ffffff'}"
						@click="checkboxGroupChange(item,index)" @tap.stop
					...
					<view class="flex-col text-left" style="width: 90%;" @tap.stop @click="jumpDraftsOrder(item.id)">
						<text v-if="vuex_userRole=='D'" :style="{color:(ifZX(item.bossNumberE)?'#AAAAAA':'#3D3D3D')}"
							class="ft34 u-line-bt width100" style="font-weight: 500;">
				...
				<view class="flex-row justify-between items-center" style="height: 10vh;">
					<view class="flex-row items-center vw100">
						<view class="items-center flex-row"
							...
							<view :disabled="!hasCheck" @click="draftsDel" class="flex-row justify-center items-center"
	...
	export default {
		...
		methods: {
			...
			draftsDel() {
				...
				this.realTimeSel.bossNumberS = this.vuex_user.data.work != "0" ?

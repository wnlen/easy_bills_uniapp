<template>
	<view class="vh100 pb60 flex-col justify-center" style="background-color: #ffffff;">
		...
		<u-navbar :custom-back="navBack" :border-bottom="false" :titleBold="true" title-color="#000000" title="订单修改"
		...
		<view class="hand relative" style="background-color: #01BB74;width: 100vw;height: 60px;"
			...
			<view class="pd20 syst absolute" style="width:100vw;background-color: transparent;
				...
				( vuex_user.ac?vuex_user.ac.enterpriseName||vuex_user.ac.phone: vuex_user.phone)
				:( vuex_user.ac?vuex_user.ac.enterpriseName :vuex_user.workData.bossNumber)}}发货单
			</view>
		</view>
		...
		<view class="form-wrap  pt20 absolute" style="background-color: #ffffff;"
			...
			<view class="form-inner flex-col" style="font-size: 14px;background-color: #ffffff;">
				<view class="flex-col justify-left">
					...
					<view class="flex-row items-center width100 u-border-bottom">
						...
						<input disabled placeholder-class="placeholder_class" type="text"
							v-model="receipts.organizationE" placeholder="请输入手机号" class="flex-1 endcolor" />
					</view>
					<view class="flex-row items-center width100 pt20 pb20 u-border-bottom">
						...
						<input disabled placeholder-class="placeholder_class" type="text" v-model="staffNumberEName"
							maxlength="20" placeholder="请选择收货方" class="u-line-1 ml15 endcolor" />
					</view>
					<view class="flex-row items-center width100 pt20 pb20 u-border-bottom">
						<text class="textcolor"><text style="color: #FA3534;">*</text>发货日期:</text>
						<input disabled placeholder-class="placeholder_class" type="text"
							v-model="receipts.creationTime" placeholder="发货日期" class="flex-1  ml15 endcolor" />
			...
			<view class="form-inner-card flex-col" style="font-size: 14px;background-color: #ffffff;">
				...
				<view class="recently-cat flex-row flex-wrap mt40" style="width: 95%;">
					<u-upload :custom-btn="true" :action="action" :show-retry="false" :file-list="fileList"
						...
						@on-remove="onRemoveImg" del-bg-color="#e9e9e9">
						...
						<view class="flex-col justify-between ft24 pt20 pb10 mr24">
							...
							<view class="textcolor pt20 pb20 u-border-bottom">
								...
								<text class="ml15 endcolor" :style="{color:'#333333'}"
									@click="callPhone(receipts.bossNumberS)">{{ receipts.bossNumberS }}</text>
							</view>
						</view>
					</view>
				</view>
			...
			<view class="mt60 pl20 pr20 pb30">
				<u-button type="primary" class="form-btn-big" hover-class="none"
					:custom-style="{backgroundColor:'#01BB74'}" @click="sendOrder" shape="circle">修改订单</u-button>
			</view>
		</view>
...
<script>
	export default {
		...
		onShow() {
			...
			if (uni.getStorageSync("updInventoryStockpile") != undefined && uni.getStorageSync("updInventoryStockpile") !=
				null && uni.getStorageSync("updInventoryStockpile") != "") {
		...
		onLoad(options) {
			uni.removeStorageSync("updInventoryStockpile")
			this.$u.post('/edo/order/getById/' + options.orderId).
			then(res => {
				...
				this.orderItemList = [...this.orderItemList, ...res.data.data.orderItemList.map(item => ({
					...item,
					'color': {
		...
		methods: {
			...
			sendOrder() {
				...
				if (this.limitingCondition) {
					...
					this.$u.post('/edo/order/edit', this.receipts).
					then(res => {
						console.log(res);
			...
			searchIFNumber(phone) {
				this.$u.post('edo/user/searchAddOrder?phone=' + phone + '&state=1').then(res => {

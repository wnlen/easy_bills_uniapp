<template>
  ...
  >
    ...
    >
      ...
      >
        <view class="relative" style="height: 100%; width: 100%">
          ...
          <view
            ...
            style="height: 40%; font-size: 18px; font-weight: 600"
            >请选择转发版本</view
          >
          <view
      ...
      >
        ...
        <view
          ...
          style="color: #aaaaaa; font-size: 13px; font-weight: normal"
          >电子单据已发送给收货人</view
        >
    ...
    >
      ...
      <view
        class="absolute"
        style="bottom: -40px; color: #aaaaaa; font-size: 14px"
        >无开单权限~</view
      >
    </view>
    ...
    <view
      class="hand relative"
      style="background-color: #01bb74; width: 100vw; height: 60px"
      ...
      <view class="mt60 pl20 pr20 pb30 flex-row">
        ...
        <view class="" style="width: 40%; padding: 12rpx">
          <u-button
            plain
            hover-class="none"
            :custom-style="{ border: '1px solid #01BB74', color: '#01BB74' }"
            @click="draftOrder"
            shape="circle"
            >存草稿</u-button
          >
  ...
  onUnload() {
    if (!this.backHomepageClick) {
      if (
        ...
        (this.orderItemList.length > 0 && this.orderTotal > 0)
      ) {
  ...
  methods: {
    ...
    draftInit() {
      return new Promise((resolve) => {
        ...
        if (this.orderItemList[0].quantity == "-") {
          this.orderItemList = [];
        } else {
          this.orderItemList = this.orderItemList.map((item) => {
            const { id, ...rest } = item;
    ...
    async draftOrderConceal() {
      ...
      if (!result) {
        ...
        const resultOrder = await this.sendOrderResDraftNoAlert();
        console.log("添加结果======>", resultOrder);
        if (resultOrder) {
          const img = await this.sendOrderImg();
          this.$u.setPinia({

	...
	<view class="uploadingCommodity">
		...
		<view class="uploadingCommodityFrom">
			<view class="uploadingCommodityFromCard">
				...
				<u-input type="text" v-model="uploadingCommodity.description" placeholder="请您填写商品名称" />
			</view>
			...
			<view class="uploadingCommodityFromCard">
				...
				<u-input v-model="uploadingCommodity.explainRemark" type="textarea" placeholder="请输入" />
			</view>
	...
	export default {
		...
		methods: {
			...
			getCommodityDetails(id) {
				...
				this.$api.library.getCommodityById(this.getByID)
					.then(res => {
						...
						if (this.uploadingCommodity.img != null && this.uploadingCommodity.img != undefined && this
							.uploadingCommodity.img != "definde") {
							...
							this.fileList.push({
								url: this.uploadingCommodity.img === "definde" ?
									"https://res-oss.elist.com.cn/wxImg/order/emptyView.png" : this
									.uploadingCommodity.img,
								uploading: false
							})
						}
			...
			updMerchandiseInventory() {
				...
				if (this.imgList.uploading == undefined && this.imgList.length > 0 || this.imgList.uploading == null &&
					this.imgList.length > 0) {
					this.updMerchandiseInventoryYes(that)
				} else {

					if (this.imgList.length <= 0 && this.uploadingCommodity.img != "definde" && this.fileList.length <=
						0) {

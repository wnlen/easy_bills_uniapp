			...
			<view class="mt45 flex-col justify-center"
				...
				<view class="flex-row justify-center items-center mt40">
					...
					<button
						...
						class="ml10" @click="ContinueBilling">返回草稿箱</button>
				</view>
			</view>
	...
	export default {
		...
		methods: {
			authenticationSynchronization(receipts) {
				...
				this.$u.post('/edo/order/authentication', receiptsVer)
					.then((res) => {
			...
			getOrderParticular(id) {
				...
				this.$u.post('edo/draft/getById/' + id).then(res => {
			...
			sendOrderResDraft() {
				return new Promise((resolve) => {
					if (this.limitingCondition) {
						this.limitingCondition = false
						this.$u.post('/edo/draft/edit', this.receipts).
						then(res => {
							console.log(res);
							this.$u.toast(res.data.message);
							resolve(true);

...
<script>
	export default {
		...
		methods: {
			...
			// 			// 你可以在这里处理选择到的联系人数据  
			// 			that.receipts.organizationE = res.phoneNumber
			// 			var iten = {
			// 				target: {
			// 					value: res.phoneNumber.replace(/-/g, '').length == 11 ? res.phoneNumber
			// 						.replace(/-/g, '') : res.phoneNumber.replace(/\s/g, '')
			// 				}
			// 			}
			// 			that.receipts.organizationE = ""
			// 			that.searchIFNumber(iten)
			...
			loadRrecently() {
				...
				var phone = this.vuex_user.phone
				if (work) {
					phone = this.vuex_user.workData.bossNumber
				}
				this.$u.post('/edo/inventory/get?phone=' + phone).
				then(res => {
					console.log(res.data.data);

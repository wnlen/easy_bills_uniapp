<template>
	<view class="uploadingCommodity">
		...
		<view class="uploadingCommodityImg">
			<u-upload :custom-btn="true" :action="action" :show-retry="false" :file-list="fileList" :show-tips="false"
				:before-upload="handleUpload" max-size="9999999" :max-count="1" multiple del-bg-color="#565C62">
		...
		<view class="uploadingCommodityFrom">
			<view class="uploadingCommodityFromCardRow">
				...
				<view class="uploadingCommodityFromCardRowInput" style="width: 300px;">
					<u-input type="text" v-model="uploadingCommodity.description" :custom-style="customStyle"
			...
			<view class="uploadingCommodityFromCardRow">
				...
				<view class="uploadingCommodityFromCardRowInput" style="width: 200px;">
					<u-input v-model="uploadingCommodity.unitPrice" type="digit" maxlength="10" placeholder="请输入"
						:custom-style="customStyle" />
				</view>
			</view>
		...
		methods: {
			...
			addMerchandiseInventory(type) {
				...
				if (this.imgList.length > 0) {
					...
					if (this.check) {
						this.check = false
						this.$u.post('/edo/library/add', this.uploadingCommodity)
							.then(res => {
								console.log(res);
			...
			addMerchandiseInventoryYes(that) {
				...
				uni.uploadFile({
					...
					success: (uploadFileRes) => {
						...
						that.$u.post('/edo/library/add', that.uploadingCommodity)
							.then(res => {
								console.log(res);
								that.$u.toast(res.data.message);
								if (res.data.data > 0) {

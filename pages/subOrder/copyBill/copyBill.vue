		...
		<view class="form-wrap  pt20  absolute" style="background-color:#ffffff;"
			...
			<view class="form-inner flex-col" style="font-size: 14px;background-color: #ffffff;">
				<view class="flex-col justify-left">
					...
					<view class="flex-row items-center width100 u-border-bottom">
						<text class="pt20 pb20 pr12 textcolor"><text style="color: #FA3534;">*</text>客户手机号:</text>
						<input maxlength="20" placeholder-class="placeholder_class" style="color:#333333;"
							@input="searchIFNumber" @blur="searchIFNumberBlur" :focus="PhoneFocus"
							@focus="searchIFNumberFocus" type="number" v-model="khPhone" placeholder="请输入客户手机号"
			...
			<scroll-view enhanced :show-scrollbar="false" scroll-y="true"
				...
				<view style="width: 100%;background-color: #ffffff;" v-for="(item, index) in orderItemList" :key="index"
					@click="merchandiseInventory(false)" class="mb12 mt12">
			...
			<view class="form-inner-card flex-col" style="font-size: 14px;background-color: #ffffff;">
				...
				<view class="recently-cat flex-row flex-wrap mt40" style="width: 95%;">
					<u-upload :custom-btn="true" :action="action" :show-retry="false" :file-list="fileList"
						:show-tips="false" @on-remove="onRemoveImg" :before-upload="handleUpload" max-size="5242880"
						max-count="3" multiple del-bg-color="#e9e9e9">
				...
				<view class="flex-col mt45" style="width: 95%;">
					...
					<view class="mt40"
						...
						<input v-model="receipts.signatureDescr" type="text"
							class="line24 height80 table-input ml30 endcolor" placeholder="请填写备注内容"
...
<script>
	export default {
		...
		onLoad(options) {
			...
			// let currentDate = new Date(this.receipts.creationTime);
			...
			this.khPhone = this.receipts.organizationE != "" ? this.receipts.organizationE : this.receipts
				.bossNumberE
			this.staffNumberEName = this.receipts.takeE ? this.receipts.takeE : this.receipts
				.bossNumberE
		...
		methods: {
			authenticationSynchronization() {
				...
				this.$u.post('/edo/order/authentication', this.receipts)
					.then((res) => {
			...
			addOrderIfOk() {
				...
				const monthsDiff = (endDate.getFullYear() - startDate.getFullYear()) * 12 + endDate
					.getMonth() - startDate
					.getMonth();
			...
			onShareAppMessage(ops) {
				if (ops.from === 'button') {
					...
					return {
						...
						title: `您有一张订单待确认~`,
						path: '/pages/subOrder/detailsShare?share_id=' + pid + "&&type=1" + "&&phone=" + phone +
							"&&port=" +
							port +
							"&&versions=" + versions,
						imageUrl: pThumb
			...
			searchIFNumber(e) {
				...
				if (ifok) {
					...
					this.$u.post('edo/user/searchAddOrder?phone=' + phone + '&state=1&boss=' + boss + "&port=" + port)
						.then(res => {
							...
							if (userIfOk) {
								...
								if (verification != null && verification != undefined) {
									...
									if (data.remark) {
										this.khPhone = data.remark
										this.receipts.organizationE = data.remark
									}
								}


								if (cRelation != undefined && cRelation != null) {
									this.receipts.bossNumberE = user.work != "0" ? cRelation.bossNumber : user
										.phoneNumber
									this.receipts.staffNumberE = user.work != "0" ?
										cRelation.staffNumber : user.phoneNumber
				...
				// 	this.searchCopy = phone;
				// 	this.$u.post('edo/user/searchAddOrder?phone=' + phone + '&state=1&boss=' + boss +
				// 		"&port=" + this
				...
				// 				if (end.user.name) {
				// 					this.receipts.takeE = end.user.name
				// 					this.staffNumberEName = end.user.name
				// 				}

				// 				if (end.verification.enterpriseName) {
				// 					this.receipts.organizationE = end.verification.enterpriseName
				// 				} else {
				// 					this.receipts.takeE = phone
				// 					this.receipts.organizationE = phone
				// 				}
				...
				// 					this.receipts.bossNumberE = end.user.work != "0" ?
				// 						end.cRelation.bossNumber : end.user.phoneNumber
				// 					this.receipts.staffNumberE = end.user.work != "0" ?
				// 						end.cRelation.staffNumber : end.user.phoneNumber
				...
				// 				if (end.user.phoneNumber && end.user.id == null) {
				// 					console.log("===未注册用户===>");
				// 					this.staffNumberEName = end.user.phoneNumber
				// 				} else {
				// 					console.log("===注册用户===>");
				// 					this.staffNumberEName = end.user.name
				// 				}
				...
				// 					//查询的当前老板是否为当前老板 公司同事
				// 					var c2 = this.vuex_user.workData.bossNumber === end.cRelation
				// 						.bossNumber
				// 					if (c2) {
				// 						this.$u.toast("此人员不支持开单~");
				// 						this.clear()
			...
			searchIFNumberBlur(e) {
				...
				// 	if (this.searchDomain.remark && this.searchDomain.verification.enterpriseName ==
				// 		null) {
				// 		this.receipts.organizationE = this.searchDomain.remark
				// 		this.khPhone = this.searchDomain.remark;
				// 	}
			...
			loadOrderNo() {
				...
				this.receipts.bossNumberS = this.vuex_user.data.work != "0" ?
					this.vuex_user.workData.bossNumber : this.vuex_user.phone;

				this.receipts.staffNumberS = this.vuex_user.phone;

				this.receipts.jobNumberS = this.vuex_user.data.work != "0" ?
					this.vuex_user.workData.jobNumber : "";

				this.receipts.contactsS = this.vuex_user.data.work != "0" ?
					this.vuex_user.data.name || this.vuex_user.data.phoneNumber : this.vuex_user.phone;

				try {
					var res = JSON.parse(uni.getStorageSync('companyNameJSON'))
					console.log("companyNameJSON======>", res);
					this.receipts.bossNumberE = res.data.identity == "0" ? res.data.staffNumber : res.data
						.bossNumber;
					...
					this.searchCopy = res.data.identity == "0" ? res.data.staffNumber : res.data
						.bossNumber;
					...
					var dx = {
						user: {
							phoneNumber: res.data.identity == "0" ? res.data.staffNumber : res.data
								.bossNumber,
			...
			sendOrderRes() {
				return new Promise((resolve) => {
					if (this.limitingCondition) {
						...
						this.$u.post('/edo/order/add', this.receipts).
						then(res => {
							console.log(res);
							...
							if (code == 1) {
								...
								this.backHomepageClick = true
								this.$u.post('/edo/order/getByOrderNumber/' + this.receipts
									.orderNumber).
								then(res => {
			...
			sendOrderResDraft() {
				return new Promise((resolve) => {
					if (this.limitingCondition) {
						this.limitingCondition = false
						this.$u.post('/edo/draft/add', this.receipts).
						then(res => {
							console.log(res);
							var code = res.data.data
							if (code == 1) {
								...
								if (res.statusCode === 200) {
									const result = {
										...
										file: {
											...
											size: res.tempFilePath
												.length, // 这里只是简单示例，实际大小应该从响应头获取
											__proto__: Object
										},
										progress: 100,
										response: "1",
									...
									console.error(
										`图片 ${url} 下载失败，状态码: ${res.statusCode}`
									);
								}
							},
							fail: (err) => {
			...
			sendOrderImg() {
				return new Promise((resolve) => {
					var that = this;
					var listImg = []
					var bossNumber = this.vuex_work == "Y" ?
						this.vuex_user.workData.bossNumber : this.vuex_user.phone || this.vuex_user
						.data
						.phoneNumber
					var jobNumber = this.vuex_work == "Y" ?
						that.vuex_user.workData.jobNumber : that.vuex_user.phone;
					...
					var imgListO = this.fileList.filter(res => res.id && !imgRemove.includes(res
						.id)).map(res =>
						res.url);
					...
					this.downloadImages(imgListO)
						.then((tempUrls) => {
							...
							for (let key in tempUrls) {
								...
								uni.uploadFile({
									url: that.$u.http.config.baseUrl +
										'/edo/order/img',
									header: {
										phone: bossNumber,
										orderNumber: that.receipts.orderNumber,
										jobNumber: that.receipts.jobNumberS ||
											jobNumber,
			...
			getOrderNumber() {
				var ifphon = this.vuex_work == "Y" ? this.vuex_user.workData.bossNumber : this
					.vuex_user.phone;

			...
			<view class="form-inner flex-col" style="font-size: 28rpx; background-color: #ffffff">
				<view class="flex-col justify-left">
					...
					<view class="flex-row justify-between items-center mt20">
						...
						<text class="line34 handcolorClear pt24" @click="ClearOrderItemList">一键清空</text>
					</view>
				</view>
			</view>
...
export default {
	...
	methods: {
		...
		searchIFNumber(e) {
			...
			// 				//判断是否工作
			// 				var b = this.pinia_user.user.work === "0";
			// 				if (b) {
			// 					//没有工作 出来了自己其他人都可以开单

			// 					//不能给员工开单
			// 					var b1 = this.pinia_user.phone === end.cRelation.bossNumber
			// 					if (b1) {
			// 						this.$u.toast("此人员不支持开单~");
			// 						this.clear()
			// 						return;
			...
			// 					//当前老板是否为当前输入号码
			// 					var c1 = this.pinia_user.workData.bossNumber === phone
			// 					if (c1) {
			// 						this.$u.toast("此人员不支持开单~");
			// 						this.clear()
			// 						return;
			...
			// 					//查询的当前老板是否为当前老板 公司同事
			// 					var c2 = this.pinia_user.workData.bossNumber === end.cRelation
			// 						.bossNumber
			// 					if (c2) {
			// 						this.$u.toast("此人员不支持开单~");
			// 						this.clear()
		...
		loadOrderNo() {
			this.receipts.bossNumberS = this.pinia_user.user.work != '0' ? this.pinia_user.workData.bossNumber : this.pinia_user.phone;

			this.receipts.staffNumberS = this.pinia_user.phone;

			this.receipts.jobNumberS = this.pinia_user.user.work != '0' ? this.pinia_user.workData.jobNumber : '';

			this.receipts.contactsS = this.pinia_user.user.work != '0' ? this.pinia_user.data.name || this.pinia_user.data.phoneNumber : this.pinia_user.phone;

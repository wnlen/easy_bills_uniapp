      ...
      <uv-cell-group :border="false">
        <uv-cell title="头像" :center="true" :cellStyle="cellStyle">
          <template v-slot:value>
            <uv-avatar
              :src="fixAvatar(userData.headPortrait)"
              :default-url="headimg()"
              shape="circle"
              size="56"
            ></uv-avatar>
        ...
        <view class="box_l">
          ...
          <text style="color: #ffc300"
            >￥<uv-count-to
              ...
              decimals="2"
              separator=","
              :endVal="Statisticsdata.totalAmount"
            ></uv-count-to
          ></text>
        </view>
        <view class="box_r">
          <text>总开单数：</text>
          <text style="color: #01bb74"
            ><uv-count-to
              color="#01BB74"
              fontSize="15"
              :startVal="0"
              separator=","
              :endVal="Statisticsdata.totalOrders"
            ></uv-count-to
          ></text>
        </view>
      </view>
...
export default {
  ...
  methods: {
    ...
    getOrderStatistics() {
      ...
      this.$api.dashboard
        .getDashboardOrderStatistics(this.phone, portType)
        .then((res) => {
    ...
    getUserData(phone) {
      ...
      this.$u
        .post(url + phone + "&boss=" + boss + "&port=" + this.vuex_userRole)
        .then((res) => {

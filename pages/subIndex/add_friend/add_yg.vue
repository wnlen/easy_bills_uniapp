  ...
  <view class="bg-gray vh100 vw100" style="overflow: hidden">
    ...
    <view class="vw100" style="height: 80vh">
      ...
      >
        ...
        <view
          class="ml10 flex-col justify-left items-baseline"
        ...
        >
          ...
          >
            <text class="ml5" style="color: #01bb74">{{
              item.phoneNumber
            }}</text>
            <text class="ml10" style="">{{
              userName(item.name) || "用户"
            }}</text>
          </view>
          <view
            v-show="item.map != '' && item.map.boss"
            class="flex-row justify-start items-center"
            style="width: 60vw; font-size: 12px; height: 50%"
          >
            <u-icon
              name="https://res-oss.elist.com.cn/wxImg/user/yrz-s.svg"
              size="100"
            ></u-icon>
            <text class="ml10" style="color: #333333">
              {{
                item.map.enterpriseName != undefined
                  ? item.map.enterpriseName
                  : "该用户已进入其他账户"
              }}
            </text>
          </view>

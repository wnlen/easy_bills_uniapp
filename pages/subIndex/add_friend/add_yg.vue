<template>
	<view class="bg-gray vh100 vw100" style="overflow: hidden;">
		<view class="ml24 mr24" style="padding-top: 20px;">
			<u-search search-icon-color="#01bb74" :show-action="false" placeholder="请输入电话号码" :animation="true"
				@change="SearchFriend" @search="SearchFriend" maxlength="11" shape="square" class="u-search" height="90"
		...
		<view class="vw100" style="height: 80vh;">
			...
			<view v-for="(item,index) in searchUser" :key="index"
				...
				<view class="flex-col justify-center items-center" style="width: 13vw;">
					<u-image class="ml20" width="50px" height="50px" shape="circle"
						:src="item.headPortrait||defaultImg"></u-image>
				</view>
		...
		<!-- 			<view class="flex-row" style="height: 50%;"
					...
					<view class="flex-row" style="height: 50%;"
						:style="{height:item.map.boss?'50%':'100%',alignItems: item.map.boss?'flex-end':'center'}">
						<text class="ml5" style="color:#01BB74;">{{item.phoneNumber}}</text> <text class="ml10"
							style="">{{userName(item.name)||"用户"}}</text>
					</view>
					<view v-show="item.map!=''&&item.map.boss" class="flex-row justify-start items-center"
...
<!-- 					<view v-show="item.work=='1'" class="flex-row justify-start items-center"
				...
				<view class="mr48" style="float: right;width: 10vw;">
					<button :style="{display:state==0?'none':'block'}" v-if="item.work=='0'&&item.work!='1'||!showYQZ"
					...
					<view v-if="showYQZ" style="width: 100px;font-size: 13px;color: #01BB74;" class="flex-row">邀请中
					</view>
				</view>
		...
		methods: {
			...
			SearchFriend() {
				...
				if (this.searchNumber.length == 11) {
					this.$u.post('edo/user/search?phone=' + this.searchNumber + '&state=1'+'&port='+this.vuex_userRole).then(res => {

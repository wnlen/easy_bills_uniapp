		...
		<view class="vw100" style="height: 80vh">
			...
			>
				...
				<view class="ml10 flex-col justify-left items-baseline" style="width: 60vw; height: 100%">
					...
					<view class="flex-row" style="height: 50%" :style="{ height: item.map.boss ? '50%' : '100%', alignItems: item.map.boss ? 'flex-end' : 'center' }">
	...
	methods: {
		...
		SearchFriend() {
			...
			if (this.searchNumber.length == 11) {
				this.$api.user.searchUser({ phone: this.searchNumber, state: 1, port: this.vuex_userRole }).then((res) => {

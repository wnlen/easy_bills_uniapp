		...
		<view v-for="(item,index) in searchUser" :key="index"
			...
			<view class="ml10 flex-col justify-center items-baseline" style="width: 60vw;height: 100%;">
				<view class="flex-row" style="height: 50%;"
					:style="{height:item.map.boss?'50%':'100%',alignItems: item.map.boss?'center':'center'}">
			...
			<view class="mr48" style="float: right;width: 10vw;">
				<button v-if="!showYQZ" :style="{display:state==1?'none':'block'}"
					class="items-center flex-col justify-left pr35" @click="add"
		...
		methods: {
			...
			SearchFriend() {
				...
				if (this.searchNumber.length == 11) {
					this.$u.post('edo/user/search?phone=' + this.searchNumber + '&state=1').then(res => {

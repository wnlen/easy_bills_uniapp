<template>
  <view class="bg-white" style="overflow-x: hidden; padding-bottom: 200rpx">
    ...
    <view class="flex-row ml24 mr24 mt24" style="display: flex">
      ...
      <!-- <view class="ml5 mr5" style="flex: 1">
				<u-button :custom-style="SearchCustomStyle" hover-class="none" shape="circle" @click="goPath('/pages/subIndex/add_friend/add_friend')">
    ...
    <view
      class="mt24 flex-row ml24 mr24"
      style="background-color: #f8f8f8; border-radius: 12.24rpx"
    >
    ...
    <up-overlay :show="showSF" @click="showSF = false">
      ...
      >
        <view
          ...
          style="font-size: 32rpx; height: 5vh"
          >添加好友</view
        >

        <view
          class="flex-row text-center justify-center items-center"
          style="width: 100%; height: 8vh"
        >
          <view
            class="flex-col text-center justify-center items-center"
            style="color: #999999; font-size: 32rpx"
            >申请添加对方为</view
          >
          <view
            @click="
        ...
        >
          ...
          <view
            @click="addFriend(sm)"
            class="oksm flex-col text-center justify-center items-center"
            style="width: 50%; height: 100%"
            >确认</view
          >
        </view>
        ...
        <view
          v-if="showChecked"
          class="u-border-top flex-col justify-center items-center"
        ...
        >
          ...
          >
            ...
            <view v-if="role == 0" class="absolute" style="right: 20rpx">
              <u-icon
                name="https://res-oss.elist.com.cn/wxImg/code/check.svg"
                size="28rpx"
              ></u-icon>
            </view>
          </view>
        ...
        >
          <view class="" style="width: 10%">
            <u-image
              ...
              :src="
                item2.img == 'zx'
                  ? '/static/img/obj/zx.svg'
                  : item2.img == undefined
                    ? headimg()
                    : item2.img == 'wsz'
                      ? '/static/img/obj/defind.svg'
                      : item2.img
              "
              width="90rpx"
              height="90rpx"
          ...
          <view class="ml30" style="width: 60%">
            ...
            <view class="ft-gray">
              ...
              <text
                class="ft-bold"
                size="mini"
                :style="{ color: item2.total > 0 ? '#01BB74' : '#999999' }"
                >￥{{ formatAmount(item2.total) }}</text
              >
            </view>
          </view>
          <view
    ...
    <view class="">
      ...
      <view class="" :style="{ display: show != 0 ? 'none' : 'block' }">
        <view
          ...
          :key="index"
          @click="particulars(item, true)"
        >
          <view
            class="ml20 mt15"
            style="border-bottom: 2rpx solid #f7f7f7; width: 110vw"
          >
            <u-collapse arrow-color="#ffffff">
              <view
                class="flex-col justify-center items-baseline"
                style="height: 80rpx"
  ...
  methods: {
    ...
    collection(item2) {
      ...
      if (this.vuex_userRole == "D") {
        uni.navigateTo({
          url:
            "/pages/subStatistics/receipt/receipt?tid=开收款单&searchData=" +
            JSON.stringify(dx),
        });
      } else {
        uni.navigateTo({
          url:
            "/pages/subStatistics/receipt/receipt?tid=开付款单&searchData=" +
            JSON.stringify(dx),
        });
      }
    },
    particulars(particulars, type) {
      if (type) {
        ...
        if (!boss[0].staffNumber.includes("zx")) {
          uni.navigateTo({
            url:
              "/pages/subIndex/details/details?phone=" +
              boss[0].staffNumber +
              "&select=" +
              boss[0].state +
              "&&type=" +
        ...
        var state =
          particulars.img == "wzc" || particulars.img == undefined ? "9" : "0";

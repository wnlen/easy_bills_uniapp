<template>
  <view class="bg-gray vh100">
    ...
    >
      ...
      <view
        class="ml24 mr24 mt24"
        style="background-color: white; border-radius: 24rpx"
        :index="index"
        ...
        >
          <view class="item flex-col justify-left">
            ...
            <view class="u-body-item u-flex u-row-between u-p-b-0">
              ...
              >
                <u-icon
                  name="https://res-oss.elist.com.cn/wxImg/index/del.svg"
                  size="40"
                ></u-icon>
                <text class="ml20" style="font-size: 32rpx; font-weight: bold"
                  >删除申请</text
                >
              </view>
              <view
              ...
              >
                <u-icon
                  name="https://res-oss.elist.com.cn/wxImg/index/play.svg"
                  size="40"
                ></u-icon>
                <text class="ml20" style="font-size: 32rpx; font-weight: bold"
                  >确认付款申请</text
            ...
            <view class="">
              ...
              <view
                ...
                v-if="item.port == 'D' || item.port == 'R'"
              >
                <text style="color: #666666" v-if="item.genre == 'D'"
                  >{{ item.aName || item.aUser }}申请删除订单</text
                >
                <text style="color: #666666" v-if="item.genre == 'P'"
                  >{{ item.aName || item.aUser }}申请确认付款订单</text
                >
                <text style="color: #666666" class="mt10"
                  >订单编号：<text style="color: #01bb74">{{
                    item.orderNumber
                  }}</text></text
                >
              </view>
              <view
                class="flex-col mt25"
                style="font-size: 28rpx"
                v-if="item.port == 'E' || item.port == 'S'"
              >
                <text style="color: #666666" v-if="item.genre == 'D'"
                  >{{ item.aName || item.aUser }}申请删除{{
                    item.port == "E" ? "收款单" : "付款单"
                  }}</text
                >
                <text style="color: #666666" v-if="item.genre == 'P'"
                  >{{ item.aName || item.aUser }}申请确认付款收款单</text
                >
                <text style="color: #666666" class="mt10"
                  >订单编号：<text style="color: #01bb74">{{
                    item.orderNumber
              ...
              >
                ...
                <text style="color: #aaaaaa"
                  >￥{{ parseFloat(item.totalPrices).toFixed(2) }}</text
                >
              </view>
            ...
            <view
              class="u-body-item u-flex u-row-between u-p-b-0 u-border-top"
              @click="jumpDetails(item)"
            >
              <view
                class="u-body-item-title u-line-0 mt15"
                style="color: #01bb74"
              >
                查看订单</view
              >
...
export default {
  ...
  methods: {
    ...
    queryList(start, end) {
      ...
      if (
        this.vuex_user.workData.identity != "3" ||
        (this.tab == 1 && this.vuex_user.workData.identity == "3")
      ) {
    ...
    jumpDetails(item) {
      ...
      if (item.port == "E" || item.port == "S") {
        uni.navigateTo({
          url:
            "/pages/subStatistics/receipt/bill_particulars?id=" + item.orderId,
        });
      } else {
        uni.navigateTo({
    ...
    refresh(start, end) {
      ...
      this.$api.order.getOrderDraftLimit(dx).then((res) => {
        ...
        if (filer) {
          this.$refs.paging.complete(
            getList.filter((res) => res.port == "R" || res.port == "E"),
          );
        } else {
          this.$refs.paging.complete(
            getList.filter((res) => res.port == "D" || res.port == "S"),
          );
        }

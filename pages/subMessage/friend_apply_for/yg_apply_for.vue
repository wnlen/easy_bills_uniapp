...
export default {
	...
	methods: {
		...
		click1(item, index) {
			...
			this.$api.user.deleteRelationApply(item).then((res) => {
		...
		loadDataYG() {
			...
			this.$api.apply.getApplyList(dx).then((res) => {
		...
		getPhoneNumberT(item) {
			...
			if (this.ClickSafe) {
				...
				console.log('员工统一');
				this.$api.apply.confirmApply(JSON.parse(JSON.stringify(dx))).then((res) => {

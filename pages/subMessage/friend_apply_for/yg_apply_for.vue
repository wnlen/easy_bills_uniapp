<template>
	<view>
		<view class="ml24 mr24" style="height: 10vh;width: 93vw;border-radius: 6px;align-items: center;"
			v-show="SHOW==1">
			<u-swipe-action :btn-width="180" :index="index2" :show="item2.show" v-for="(item2,index2) in resApiT"
				@open="open1" :options="options" :key="index2" v-show="SHOW==1" @click="click1(item2,index2)">
					...
					<view class="flex-col justify-evenly items-end" style="height: 70px;width: 40%;">
						<view class="flex-col relative  items-end" style="height: 100%;">
							...
							<view class="absolute flex-row justify-center items-center" style="bottom: 8px;right: 0;">
								...
								<view v-if="item2.state==0" @click="getPhoneNumberT(item2)"
	...
	export default {
		...
		methods: {
			...
			click1(item, index) {
				...
				this.$u.post('/edo/relationApply/del', item).
				then(res => {
			...
			getPhoneNumberT(item) {
				...
				if (this.ClickSafe) {
					...
					console.log("员工统一");
					this.$u.post('/edo/apply/ok', JSON.parse(JSON.stringify(dx))).
					then(res => {

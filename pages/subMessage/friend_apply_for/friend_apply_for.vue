<template>
	<view>
		<view class="ml24 mr24" style="height: 10vh;width: 93vw;border-radius: 6px;align-items: center;"
			v-show="SHOW==0">
			<u-swipe-action :btn-width="180" :show="item.show" :index="index" v-for="(item,index) in resApiO"
				:key="index" v-show="SHOW==0" @open="open2" :options="options" @click="click2(item)">
				<view style="width: 96vw;" class="flex-row u-border-bottom mt20 pd15">
					...
					<view class="flex-col justify-center ml26" style="height: 70px;width: 40%;">
						...
						<text class="title "
							style="font-size: 27rpx;color: #999999;letter-spacing: 1rpx">申请成为您的{{item.port=="D"?"供应商":"客户"}}</text>
					</view>
					<view class="flex-col justify-evenly items-end" style="height: 70px;width: 40%;">
						<view class="flex-col relative  items-end" style="height: 100%;">
							...
							<view class="absolute flex-row justify-center items-center" style="bottom: 10px;right: 0;">
								<view v-if="item.state==0" @click="getPhoneNumber(item)"
	...
	export default {
		...
		methods: {
			...
			click2(item) {
				...
				this.$u.post('/edo/client/del', item).
				then(res => {
			...
			loadDataHY() {
				let that = this;
				that.$u.post('/edo/client/get', {
					"bNumber": that.vuex_user.phone,
					"bBossNumber": that.vuex_user.data.work == "1" ? that.vuex_user.workData.bossNumber : that
						.vuex_user.phone,
					"state": "0",
				}).
				then(res => {
			...
			getPhoneNumber(s) {
				let that = this;
				let item = s;
				this.$u.post('/edo/client/ok', item).
				then(res => {

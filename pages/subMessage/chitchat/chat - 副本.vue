<template>
	<view>
		<y-chat :userId="userId" :value="val" :messageList="messageList" :updateList="updateList"
			:defaultOptions="defaultOptions" :tagOptions="tagOptions" :sheetList="sheetList" @onRefresh="getNext"
			@playPhoto="playPhoto" @playCamera="playCamera" @send="send" @tapAvator="tapAvator" @redPacket="redPacket"
			:OperatingSystem="OperatingSystem">
			<view slot="hander">
				<u-navbar :custom-back="customBack" :background="{background:'transparent'}" :border-bottom="false"
		...
		methods: {
			...
			getCurrentDate(str) {
				...
				const month = ("0" + (date.getMonth() + 1)).slice(-
					2); // Months are zero-based, add leading zero if necessary  
			...
			getChatMes() {
				...
				this.$u.post('edo/chatting/get', dx).
				then(res => {
			...
			sendMessage(mes) {
				...
				//存入数据库
				this.$u.post('/edo/chatting/add', chatRecord).
				then(res => {
				...
				uni.uploadFile({
					...
					success: (uploadFileRes) => {
						...
						const chatRecord = {
							...
							readTime: "", // 读取时间，可以为空（但通常应该是日期时间格式）  
							withdraw: "1", // 撤回状态，可以为空  
							avator: that.vuex_user.data.headPortrait ? that.vuex_user.data.headPortrait :
								this.defAtr, // 发起者头像，可以为空  

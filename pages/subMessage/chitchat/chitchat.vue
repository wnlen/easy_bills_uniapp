<template>
	<view class="bg-gray vh100 vw100" style="background-color: #F4F4F4;">
		<scroll-view scroll-y="true" :scroll-top="scrollTop" style="height: 85vh;">
			<view class="chatMessage ml30 mt30" style="position: relative;;height: 10%;"
				:style="{height:item.message.length>20?10+(item.message.length/5)+'%':'10%'}"
				...
				<view class="" v-if="item.formUserId!=vuex_user.phone">
					...
					<view class="mt30" style="font-size: 12px;height: 100%;margin-left: 12vw;">
						...
						<view class="mt15 pd10 pl10 pr10 pb10 pt10"
							style="background-color: #ffffff;width: auto;float: left;border-radius: 0px 6px 6px 6px;max-width: 50vw;white-space: normal; word-wrap: break-word;font-size: 14px;">
							{{item.message}}
						</view>
				...
				<view class="mt45 mr30" v-if="item.formUserId==vuex_user.phone">
					...
					<view class="" style="font-size: 12px;height: 100%;margin-right: 12vw;">
					</view>
					<view class="pd10 pr10 pb10 pt10 mt40" v-if="item.message.img==undefined"
						style="background-color: #01BB74;color:white;width: auto;float: right;border-radius: 6px 0px 6px 6px;max-width: 50vw;white-space: normal; word-wrap: break-word;margin-right: 12vw;font-size: 14px;">
						{{item.message}}
					</view>
					<view class="pd10 pr10 pb10 pt10 mt40" v-if="item.message.img"
						style="background-color: #ffffff;color:white;width: auto;float: right;border-radius: 6px 0px 6px 6px;max-width: 50vw;white-space: normal; word-wrap: break-word;margin-right: 12vw;font-size: 14px;">
			...
			<view class="flex-row justify-center items-center" style="width: 100%;" v-if="messageList.length>20">
				...
				<u-icon name="close-circle" label="清空" color="#4c4c4c" size="28" label-siz="20"
					@click="clearMessage"></u-icon>
		...
		<view class="inpMess bg-white vw100 pt25" style="height: 14vh;background-color: #ffffff;">
			<view class="flex-row ml20 bg-white" style="width: 95%;height: 40%;" v-if="showTag==1">
				<view class="ml10" v-for="(item,index) in tagList" :key="index" @click="copyIteam(item)">
					<u-tag :text="item" bg-color="#ffffff" border-color="#ccc" color="#ccc" mode="light" shape="circle"
						closeable :show="show" @close="tagClick(item)" />
				</view>
			</view>
			...
			<view class="flex-row mt10 mb20 ml20" style="height: 60%;">
				...
				<u-input class="" type="text" border='true' placeholde="输入您想咨询的问题"
					:style="{width:showFile==1?'70%':'80%'}" @input="messageChange" v-model="message" />
				<view class="mt10 ml10" style="height: 100%;align-items: center;">
					<u-icon :style="{display:showFile==0?'block':'none'}" class="mr10 ml10" name="plus-circle" size="50"
						@click="sendImg"></u-icon>

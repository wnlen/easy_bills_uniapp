	...
	<view class="root">
		<z-paging ref="paging" use-virtual-list :force-close-inner-list="true" cell-height-mode="dynamic"
			...
			<template #top>
				...
				<!-- #endif -->
				<!-- #ifdef APP -->
				<up-navbar title="查询订单" :placeholder="true" leftIconColor="#fff" :titleBold="true"
				...
				<view style="width: 100%;">
					...
					<up-tabs :list="tabsList" lineWidth="30" lineHeight="7" lineColor="#0FB076" :scrollable="false"
						:activeStyle="{
							color: '#0FB076',
							fontWeight: 'bold',
			...
			<view class="Card cardShow">
				<view class="priceCard">
					...
					<view class="">
						...
						<u-count-to :end-val="OrderQuantitySum" separator="," color="#000000" font-size="20"
				...
				<view class="InputCard">
					<view class="InputOne">
						<u-input :modelValue="customer" :custom-style="{ backgroundColor: 'transparent' }"
							:placeholder="userStore.userRole === 'R' ? '请选择供应商' : '请选择客户'" :clearable="true"
							border="none" @change="CustomerGetChange">
							<template #prefix>
							...
							<template #suffix>
								<view class="flex-col justify-center items-center">
									<u-icon class="ml40" name="/static/img/list/lxr.svg" size="23"
					...
					<view class="InputOne" v-if="showTage !== '1'">
						<u-input :modelValue="field" :custom-style="{ backgroundColor: 'transparent' }"
							placeholder="输入关键字进行检索" :clearable="true" border="none" @change="searchListenner">
							<template #prefix>
								<u-text :text="Title" margin="0 0 0 20rpx" suffixIcon="arrow-down-fill"
									iconStyle="color: #666;margin:0 10rpx;fontSize:30rpx" @click="filtrateGet"></u-text>
							</template>
							<template #suffix>
					...
					<view class="InputOne" v-if="showTage === '1'">
						<u-input :modelValue="field" maxlength="11" :custom-style="{ backgroundColor: 'transparent' }"
							placeholder="输入号码进行检索" :clearable="true" border="none" @change="searchListenner">
							<template #prefix>
								<u-text class="ft11 ft-gray ml20" :text="Title" suffixIcon="arrow-down-fill"
									suffixIconStyle="color: #909399;marginLeft:20rpx;" @click="filtrateGet"></u-text>
							</template>
							<template #suffix>
			...
			<view slot="empty" style="padding-bottom: 200rpx;">
				<u-icon margin-top="22rpx" label-pos="bottom" :name="'/wxImg/list/empty.svg'"
			...
			<view v-for="(item,index) in orderList" :key="item.id" :index="index" :id="`zp-id-${item.id}`"
				...
				<view class="OrderCardHand" @tap.stop>
					...
					<view class="ml20" style="width: 30%;">
						<u-image :show-menu-by-longpress="false" v-if="userStore.userRole=='D'"
							:style="{display:item.paymentState == '0'?'inline':'none'}" class="u-img" width="120rpx"
							height="50rpx" :src="bat64.dqs"></u-image>
						<u-image :show-menu-by-longpress="false" v-if="userStore.userRole=='R'"
							:style="{display:item.paymentState == '0'?'inline':'none'}" class="u-img" width="120rpx"
				...
				<view class="width100 pb25 text-left">
					<text class="flex-col text-left">
						<text v-if="userStore.userRole=='D'"
						...
						<text v-if="userStore.userRole=='R'"
						...
						<view class="flex-row items-center justify-center">
							<button class="hl-btn flex-row items-center justify-center" @click="shareNY(item, 1)"
								...
								<u-icon v-show="OperatingSystem" name="zhuanfa" size="25"
									v-if="userStore.userRole === 'D' || userStore.userRole === 'R'" color="#666666"
							...
							<button
								v-if="item.paymentState !== '2'&&item.paymentState !== '1'&&item.lockOrder!=1&&userStore.userRole=='D'&&vuex_user.workData.identity !== '3'"
								class="hl-btn ml20 flex-row items-center justify-center" type="default"
...
<script setup>
	...
	const startX = ref(0)
	const startY = ref(0)
	const uNoticeBarlist = ref([
		'请及时完成2024年订单收款，逾期将无法处理；跨年后，可前往更多功能-往年数据处查看往年订单。'
	])

	watch(() => systemStore.flush, (newVal) => {
	...
	function refreshDataNew() {
		...
		if (refresh.value) {
			...
			uni.$api.order_order.postOrderFilter(realTimeSel.value)
				.then(res => {
			...
			uni.$api.order_order.postOrderQuantity('/edo/order/Quantity', realTimeSel.value)
				.then(res => {
	...
	function filterSubmit() {
		...
		const date = new Date()
		date.setDate(date.getDate() + 15)
		realTimeSel.value.startDate = date1.value || uni.$u.timeFormat(new Date(new Date().getFullYear(), 0, 1),
			'yyyy-mm-dd')
	...
	function VerifyAdd(item, index, type) {
		...
		if (type === 2 || type === 3) {
			...
			const content = type === 2 ?
				(isReceiver ? '是否向发货方申请确认付款该单据' : '是否确认收货方已支付该单据') :
				'是否确认修改该单据？'
			uni.showModal({
				title,
				content,
	...
	function applyUpdatePlay(order) {
		...
		uni.$u.post('/edo/orderDel/add', dx)
			.then(res => finallyAlertDel(res.data, order))
			.catch(() => {})
	}

	function updateOrder(order) {
		uni.$u.post('/edo/order/signFor', order)
			.then(res => {

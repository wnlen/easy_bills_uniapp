    ...
    >
      <template #top>
        ...
        >
          <template #left>
            <view
              class="flex-row items-center justify-center ml96"
              style="width: 100%"
            >
      ...
      <view class="Card cardShow">
        <view class="priceCard">
          <text
            class="ft-gray mb18 ml10"
            style="color: #999999; font-size: 20rpx"
            >累计金额</text
          >
          <view class="">
        ...
        <view class="InputCard">
          <view class="InputOne">
            <u-input
              ...
              :placeholder="
                userStore.userRole === 'R' ? '请选择供应商' : '请选择客户'
              "
              :clearable="true"
              border="none"
      ...
      >
        <view class="OrderCardHand" @tap.stop>
          <view class="title ml1" style="" @tap.stop>
            ...
            <u-icon
              class="ml15"
              size="28"
              v-if="item.lockOrder == 1 && item.paymentState != 2"
              :name="bat64.lock"
              color="#666666"
            ></u-icon>
          </view>
        ...
        <view
          class="flex-row u-border-top pd10 pt20 justify-between text-right items-center mt17"
          @tap.stop
        >
          <!-- 分享按钮区域 -->
          <view
          ...
          >
            ...
            >
              ...
              >
                ...
                >
                  <u-icon
                    ...
                    label-size="22"
                    :label="
                      vuex_user.data.work !== '1' && vuex_user.workDate == null
                        ? '有金额转发'
                        : '有金额转发'
                    "
                  ></u-icon>
              ...
              <view
                class="flex-row justify-center items-center u-border-left u-border-right"
                style="width: 40%; color: #cccccc"
              >
                <button
                  class="hl-btn NY flex-row items-center justify-center"
            ...
            <view class="flex-row items-center justify-center">
              ...
              >
                <u-icon
                  ...
                  size="25"
                  v-if="
                    userStore.userRole === 'D' || userStore.userRole === 'R'
                  "
                  color="#666666"
                  label-color="#333333"
                  label-size="22"
      ...
      <view
        class="NullView"
        style="height: 5vh; background-color: transparent"
      ></view>
      <!-- 自定义tab -->
      ...
      <view class="pl30 pr30">
        ...
        <view class="flex-row justify-end">
          ...
          <u-button
            ...
            shape="circle"
            size="medium"
            :custom-style="{ width: '154rpx', margin: 0, height: '60rpx' }"
            >确定</u-button
          >
        </view>
        <!-- 日历选择器 -->
...
function useInitPage(
  ...
  // 设置默认时间
  if (timeEmp) {
    const start = proxy.$u.timeFormat(
  ...
  if (refresh.value) {
    ...
    uni.$api.order
      ...
      .then((res) => {
        ...
        totalMoney.value = orderListData.reduce(
...
function filterSubmit() {
  ...
  const date = new Date();
  date.setDate(date.getDate() + 15);
  realTimeSel.value.startDate =
    date1.value ||
    uni.$u.timeFormat(new Date(new Date().getFullYear(), 0, 1), "yyyy-mm-dd");
  realTimeSel.value.endDate =
    date2.value || uni.$u.timeFormat(date, "yyyy-mm-dd");
  paging.value?.reload();
}
...
function VerifyAdd(item, index, type) {
  ...
  if (type === 2 || type === 3) {
    ...
    const title =
      type === 2 ? (isReceiver ? "确认付款提示" : "确认收款提示") : "修改提醒";
    const content =
      type === 2
        ? isReceiver
          ? "是否向发货方申请确认付款该单据"
...
function deleteOrder(order, index) {
  ...
  totalMoney.value = orderList.value.reduce(
    (total, obj) => total + obj.price,
    0,
  );
}
...
function applyUpdatePlay(order) {
  ...
  if (workif) {
    ...
    dx.aBoss = boss;
    dx.bBoss =
      order.bossNumberS === order.staffNumberS
        ? order.bossNumberS
        : order.staffNumberS;
  }

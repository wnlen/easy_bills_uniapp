	...
	<view class="root">
		<z-paging ref="paging" use-virtual-list :force-close-inner-list="true"
			...
			<view slot="top" style="padding-bottom: 60rpx;">
				<view class="Card cardShow">
					<view class="HnadCard">
						<view class="HnadCardBtn" @click="change(0)"
							:style="{color:current==0?'#01BB74':'#999999',border:'1px solid '+(current==0?'#01BB74':'#999999')}"
							style="width: 20%;height: 27px;">
							全部
						</view>
						<view class="HnadCardBtn" @click="change(1)"
							:style="{color:current==1?'#01BB74':'#999999',border:'1px solid '+(current==1?'#01BB74':'#999999')}"
							style="width: 20%;height: 27px;">
							待签收
						</view>
						<view class="HnadCardBtn" @click="change(2)"
							:style="{color:current==2?'#01BB74':'#999999',border:'1px solid '+(current==2?'#01BB74':'#999999')}"
							style="width: 20%;height: 27px;">
							已签收
						</view>
					</view>
					...
					<view class="InputCard">
						...
						<view class="InputOne">
							...
							<u-input class="my-input" style="width: 100%;" @input="CustomerGetChange" v-model="customer"
								:custom-style="{ backgroundColor: 'transparent' }" disabled="true"
						...
						<view class="InputOne">
							...
							<view class="flex-col justify-center items-center" style="height: 5vh;">
								<u-icon class="ml40" name="/static/img/list/ss.svg" size="45"
									@click="SearchBtn"></u-icon>
							</view>
						</view>
					</view>
				</view>
				<view class="absolute" style="text-align: left;font-size: 12px;color: #01BB74;left: 0;"
					:disabled="false">
			...
			<view :index="index" v-for="(item,index) in orderList" :key="item.id"
				...
				<view class="OrderCardHand" @tap.stop>
					<view class="title ml1" style="" @tap.stop>
						...
						<view class="flex-col justify-center items-center mr15" @tap.stop
						...
						<u-icon size="28" v-if="item.paymentState!=2" :name="bat64.copy"
							@click="copyBtn(item.orderNumber)"></u-icon>

					</view>
					<view class="ml20" style="width: 30%;">
						<u-image v-if="vuex_userRole=='D'" :style="{display:item.paymentState == '0'?'inline':'none'}" :show-menu-by-longpress="false"
							class="u-img" width="60px" height="25px" :src="bat64.dqs"></u-image>
						<u-image v-if="vuex_userRole=='R'" :style="{display:item.paymentState == '0'?'inline':'none'}" :show-menu-by-longpress="false"
							class="u-img" width="60px" height="25px" :src="bat64.dqss"></u-image>
						<u-image v-if="vuex_userRole=='R'" :style="{display:item.paymentState == '2'?'inline':'none'}" :show-menu-by-longpress="false"
							class="u-img" width="60px" height="25px" :src="bat64.yfk"></u-image>
						<u-image :style="{display:item.paymentState == '1'?'inline':'none'}" width="60px" height="25px" :show-menu-by-longpress="false"
							class="u-img" :src="bat64.yqs"></u-image>
						<u-image v-if="vuex_userRole!='R'" :style="{display:item.paymentState == '2'?'inline':'none'}" :show-menu-by-longpress="false"
							width="60px" height="25px" class="u-img" :src="bat64.ysk"></u-image>
					</view>
				</view>
					...
					<view class="flex-row items-center vw100">
						<view class="items-center flex-row"
							...
							<view class="relative" @click="showCheckClick">
								<u-badge :offset="[20,0]" bgColor="#FF8F1F" :absolute="true"
									:count="realTimeSel.ids.length" type="green" class="badge"
									color="#ffffff"></u-badge>
								<u-icon size="100"
									name="https://res-oss.elist.com.cn/wxImg/order/checkOrder.svg"></u-icon>
							</view>
							...
							<view :disabled="!hasCheck" @click="OpenReceipt"
								class="flex-row justify-center items-center" style="width: 140px;height: 40px;border-radius: 45px;opacity: 1;background-color: #01BB74;color: white;float: right;
								font-weight: 600;
								">
								修改
							</view>
		...
		<u-popup v-model="showCheck" mode="bottom" border-radius="15">
			<view class="warp"
				...
				<z-paging v-if="showCheck" ref="pagingCheck" :fixed="false" v-model="selectList"
					@query="queryListCheck">
					<!-- 如果希望其他view跟着页面滚动，可以放在z-paging标签内 -->
					<view solt="top" style="height: 24rpx;">

					</view>
					<view class="item">
						<u-swipe-action :show="item.show" :index="index" v-for="(item, index) in selectList"
							:key="item.id" @click="click" @open="open" :options="options" btn-width="140">
							<view @click="goPath('/pages/subOrder/details?id='+item.id);hide=false" class="OrderCard">
								<view class="OrderCardHand" @tap.stop>
									<view class="title ml1" style="" @tap.stop>

										<text class="ft30 ft-lighgray pr30" style="color: #666666;" @tap.stop>
											订单编号:
											<text class="ml15" @click="copyBtn(item.orderNumber)"
												style="color: #F76565;">
												{{item.orderNumber}}
											</text>
										</text>
									...
									<view class="ml20" style="width: 30%;">
										<u-image v-if="vuex_userRole=='D'"
											:style="{display:item.paymentState == '0'?'inline':'none'}" class="u-img" :show-menu-by-longpress="false"
											width="60px" height="25px" :src="bat64.dqs"></u-image>
										<u-image v-if="vuex_userRole=='R'"
											:style="{display:item.paymentState == '0'?'inline':'none'}" class="u-img" :show-menu-by-longpress="false"
											width="60px" height="25px" :src="bat64.dqss"></u-image>
										<u-image v-if="vuex_userRole=='R'"
											:style="{display:item.paymentState == '2'?'inline':'none'}" class="u-img" :show-menu-by-longpress="false"
											width="60px" height="25px" :src="bat64.yfk"></u-image>
										<u-image :style="{display:item.paymentState == '1'?'inline':'none'}" :show-menu-by-longpress="false"
											width="60px" height="25px" class="u-img" :src="bat64.yqs"></u-image>
										<u-image v-if="vuex_userRole!='R'" :show-menu-by-longpress="false"
											:style="{display:item.paymentState == '2'?'inline':'none'}" width="60px"
											height="25px" class="u-img" :src="bat64.ysk"></u-image>
									</view>
								</view>
								<view class="width100 pb25 text-left">
									<text class="flex-col text-left">
								...
								<text class="ft-lighgray mt10 line25 flex-row items-center justify-end">
									...
									<text style="color: black;font-size: 12px;">￥</text>
									<text class="ft35"
										style="color: black;font-weight: 500;">{{item.price.toFixed(2)}}</text>
								</text>
...
<script>
	export default {
		...
		onLoad(option) {
			...
			this.field = this.searchJson.kTakeE != "" ? this.searchJson.kTakeE : (this.searchJson.kSiteE != "" ? this
				.searchJson.kSiteE : (this.searchJson.kPhoneE != "" ? this.searchJson.kPhoneE : ""));
			this.showTage = this.searchJson.kTakeE != "" ? 0 : (this.searchJson.kPhoneE != "" ? 1 : (this.searchJson
				.kSiteE != "" ? 2 : 3));
		...
		methods: {
			...
			OpenReceipt() {
				...
				if (this.checked) {
					...
					console.log("全选");
					//获取单据id
					this.$u.post("/edo/bills/checkAllId", this.realTimeSel).then(res => {
						console.log(res);
					...
					//计算价格
					this.$u.post("/edo/bills/getFilterLockPrice", this.realTimeSel).then(res => {
			...
			checkedAll() {
				...
				if (this.checked) {
					...
					this.$u.post("/edo/bills/checkAll", this.realTimeSel).then(res => {
						console.log(res);
						if (res.data.data == "0") {
			...
			Init() {
				...
				if (this.vuex_user.data.work == "0") {
					if (this.vuex_userRole == "D") {
						...
						if (this.vuex_user.phone == this.start) {
					...
					if (this.vuex_userRole == "D") {
						this.realTimeSel.bossNumberS = this.vuex_user.workData.bossNumber
					} else {
						this.realTimeSel.bossNumberE = this.vuex_user.workData.bossNumber
					}
				}
				this.$refs.paging.refresh();
			},
			...
			getCheck() {
				// getFilterLockCheck
				this.$u.post('/edo/bills/getFilterLockCheck', this.realTimeSel)
					.then(res => {
						...
						var orderList = res.data.data.map(obj => {
							...
							return {
								...obj,
								check: this.checked ? true : this.ids.includes(obj.id) || this.realTimeSel.ids
									.includes(obj.id),
								show: false
							};
						});
			...
			refreshDataNew() {
				...
				if (this.refresh) {
					...
					this.realTimeSel.role = this.vuex_userRole == "R" ? "1" : "0"
					this.$u.post('/edo/bills/getFilterLock', this.realTimeSel)
						.then(res => {
							//console.log(res.data.data);

							var orderList = res.data.data.map(obj => {
								console.log(this.ids, obj.id);
								console.log(this.ids.includes(obj.id) || this.realTimeSel.ids.includes(obj
									.id));
								return {
									...obj,
									check: this.checked ? true : this.ids.includes(obj.id) || this.realTimeSel
										.ids.includes(obj.id),
									show: false
								};
							});
					...
					this.$u.post('/edo/bills/QuantityLock', this.realTimeSel)
						.then(res => {

<template>
	<view class="billReceipt">
		...
		<u-calendar btn-type="success" v-model="time.endShow" active-bg-color="#01BB74" range-bg-color="#DFF9EF"
			range-color="#333333" mode="date" :min-date="getCurrentYearFirstDay()" :max-date="getCurrentDate()"
		...
		<z-paging :paging-style="{'marginTop': '0'}" ref="paging" use-virtual-list :force-close-inner-list="true"
			...
			<view slot="top">
				...
				<view class="billCard cardShow">
					...
					<u-input class="my-input" style="width: 100%;" @input="InputTextOne" v-model="InputOneText"
			...
			<view class="OrderCard" v-for="(item, index) in CBills" :key="item.id" @click="clickJump(item)">
				...
				<view class="OrderCard_body" v-if="item.billState==0">
					<text v-if="vuex_userRole=='R'"
						class="OrderCard_body_text ml24">{{vuex_user.data.work=="0"?(item.sourcePhone==vuex_user.phone?item.billEnterpriseE:item.billEnterpriseS):(item.sourcePhone==vuex_user.workData.bossNumber?item.billEnterpriseE:item.billEnterpriseS)}}</text>
					<text v-if="vuex_userRole=='D'"
						class="OrderCard_body_text ml24">{{vuex_user.data.work=="0"?(item.sourcePhone==vuex_user.phone?item.billEnterpriseE:item.billEnterpriseS):(item.sourcePhone==vuex_user.workData.bossNumber?item.billEnterpriseE:item.billEnterpriseS)}}</text>
				</view>
				<view class="OrderCard_body_ply" v-if="item.billState==1">
				...
				<view class="OrderCard_end" @tap.stop>
					...
					<u-button class="ml30 bg-white"
						v-if="vuex_user.data.work=='1'?(vuex_user.workData.identity==3||vuex_user.workData.identity==1?(item.sourcePhone==vuex_user.workData.bossNumber)&&item.billState!=1:false):(item.sourcePhone==vuex_user.phone)&&item.billState!=1"
						hover-class="none" plain shape="circle" @click="click(index,0)"
						:custom-style="SearchCustomStyleWechat">
						<u-icon name="order" size="25" color="#666666" label-size="22" label-color="#333333"
							label="修改"></u-icon>
					</u-button>

					<u-button class="ml30" hover-class="none" plain shape="circle" @click="viewDetails(item)"
						:custom-style="SearchCustomStyleWechat">
						<u-icon name="list-dot" size="25" color="#666666" label-size="22" label-color="#333333"
							label="订单列表"></u-icon>
					</u-button>
					<!-- v-if="vuex_user.data.work=='1'?vuex_user.workData.identity!=3:true&&item.billState!=1" -->
					<u-button class="ml30" hover-class="none" plain shape="circle" @click="click(index,1)"
						v-if="item.billState!=1" :custom-style="SearchCustomStyleWechat">
						<u-icon name="trash" size="25" color="#666666" label-size="22" label-color="#333333"
							label="删除"></u-icon>
					</u-button>
			...
			<view slot="bottom" :style="{display:current==0?'block':'none'}">
				<view class="fixedBar pd10" style="">
					...
					<view class="fixedBarRight">
						<u-button shape="circle" @click="okUpdate" hover-class="none"
							:custom-style="customStyleBill">{{billTitle}}</u-button>
					</view>
				</view>
			</view>
		</z-paging>
	...
	export default {
		...
		methods: {
			...
			delBill(bill) {
				...
				if (billdel) {
					...
					if (this.vuex_userRole == "D") {
						...
						this.defaultValues.aName = this.vuex_user.ac == null ? this.vuex_user.phone : this
			...
			OpenBillDetails(id, type) {
				uni.navigateTo({
					url: "/pages/subStatistics/receipt/bill_particulars_up?id=" + id + "&port=" + this
						.vuex_userRole + "&edit=" + type
				})
			},
			ChangeTimeStart(e) {
			...
			getSin(type, index) {
				...
				if (Bill.sourcePhone == phone || Bill.sourcePhone == this.vuex_user.workData.bossNumber) {
					...
					content = "是否确认向" + (port ? "收" : "发") + "货方申请" + (operator ? "修改" : "删除") + "该" + billPort +
						"款单，需要对方同意后" + billPort +
						"款单才会被删除"
				}
				...
				uni.showModal({
					...
					success: res => {
						if (res.confirm) {
							if (operator) {
								var Bill = this.CBills[index];
								if (Bill.sourcePhone == this.vuex_user.phone || Bill.sourcePhone == this
			...
			okList() {
				...
				this.billTitle = this.vuex_userRole == "D" ? (okList.length > 1 ? "批量收款" : "收款") : (okList.length > 1 ?
					"批量付款" : "付款")
			},
			okUpdate() {
				var bill = this.CBills.filter(res => res.check == true);
			...
			chargeBut() {
				...
				if (port) {
					...
					bill.forEach(res => {
						this.defaultValues = Object.assign({}, ...Object.keys(this.defaultValues).map(key => ({
							[key]: ""
						})));
						this.defaultValues.port = this.vuex_userRole == "D" ? "S" : "E";
						...
						if (this.vuex_userRole == "D") {
							...
							if (this.defaultValues.aUser == res.receptionPhone || this.vuex_user.workData
								.bossNumber == res.receptionPhone) {
								this.defaultValues.bUser = res.sourcePhone;
								this.defaultValues.bBoss = res.sourcePhone;
							} else if (this.vuex_user.phone == res.sourcePhone || this.vuex_user.workData
								.bossNumber == res.sourcePhone) {
								this.defaultValues.bUser = res.receptionPhone;
								this.defaultValues.bBoss = res.receptionPhone;
							}

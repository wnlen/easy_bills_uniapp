      ...
      >
        ...
        <view class="OrderCard_card">
          ...
          <view class="OrderCard_card_b">
            <view class="c_t">
              订单金额：
              <text style="color: #01bb74; font-weight: 500"
                >￥{{ formatAmount(item.billPrice) }}</text
              >
            </view>
          </view>
        </view>
          ...
          <view class="fixedBarLeft">
            <view class="fixedBarLeftTop">
              <view class="LeftTop_text">
                ...
                <text
                  >￥{{
                    formatAmount(parseFloat(price) - parseFloat(discount))
                  }}</text
                >
              </view>
            </view>
...
export default {
  ...
  methods: {
    ...
    delBill(bill) {
      ...
      if (billdel) {
        ...
        if (this.vuex_userRole == "D") {
          ...
          this.defaultValues.aName =
            this.vuex_user.ac.enterpriseName || this.vuex_user.phone;
        } else {
          ...
          this.defaultValues.aName =
            this.vuex_user.ac == null
              ? this.vuex_user.phone
              : this.vuex_user.ac.enterpriseName;
        }
        ...
        this.$api.bills
          .revokeBillApplication(this.defaultValues)
          .then((res) => {
            console.log("删除：", bill.id, res);
            this.$u.toast(res.data.message);
            this.$refs.paging.refresh();
    ...
    OpenBillDetails(id, type) {
      uni.navigateTo({
        url:
          "/pages/subStatistics/receipt/bill_particulars_up?id=" +
          id +
          "&port=" +
          this.vuex_userRole +
          "&edit=" +
    ...
    getSin(type, index) {
      ...
      var content = "";
      if (
        Bill.sourcePhone == phone ||
        Bill.sourcePhone == this.vuex_user.workData.bossNumber
      ) {
        content =
          "是否确认" + (operator ? "修改" : "删除") + "该" + billPort + "款单?";
      } else {
        content =
          "是否确认向" +
          (port ? "收" : "发") +
        ...
        success: (res) => {
          if (res.confirm) {
            if (operator) {
              var Bill = this.CBills[index];
              if (
                Bill.sourcePhone == this.vuex_user.phone ||
                Bill.sourcePhone == this.vuex_user.workData.bossNumber
              ) {
      ...
      if (port) {
        ...
        bill.forEach((res) => {
          ...
          if (this.vuex_userRole == "D") {
            ...
            if (
              this.defaultValues.aUser == res.receptionPhone ||
              this.vuex_user.workData.bossNumber == res.receptionPhone
            ) {
              this.defaultValues.bUser = res.sourcePhone;
              this.defaultValues.bBoss = res.sourcePhone;
            } else if (
              this.vuex_user.phone == res.sourcePhone ||
              this.vuex_user.workData.bossNumber == res.sourcePhone
            ) {
              this.defaultValues.bUser = res.receptionPhone;
              this.defaultValues.bBoss = res.receptionPhone;
            }
        ...
        // batchBillApply
        this.$api.bills
          .batchApplyBills({ cDelOrderApplyList: cDelOrderApplyList })
          .then((res) => {
            this.$u.toast(res.data.message);
            this.$refs.paging.reload().catch(() => {});
            this.checkFalse();

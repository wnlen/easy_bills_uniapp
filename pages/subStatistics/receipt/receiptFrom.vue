	...
	<view class="receiptFrom">
		<view class="FROMBill" v-if="FROMBShow">
			...
			<view class="FromPrice">
				...
				<view class="FromInput u-border-bottom" v-if="vuex_userRole == 'D'">
					...
					<input placeholder-class="placeholder_class" @input="billAfterPriceCount" type="number"
						v-model="billFrom.billDiscountPrice" :style="{color:'#333333'}" maxlength="3"
				...
				<view class="FromInput">
					...
					<input placeholder-class="placeholder_class" type="number" disabled
						v-model="billFrom.billAfterPrice" :style="{color:'#333333'}" maxlength="11"
						placeholder="请输入折扣计算金额" class="ml15  flex-1 u-line-1 endcolor" />
					<text>元</text>
				</view>
			</view>
			...
			<view class="FromFile">
				...
				<view class="recently-cat flex-row flex-wrap" style="width: 95%;">
					<u-upload :custom-btn="true" :action="action" :show-retry="false" :file-list="imgFileList"
						:show-tips="false" :before-upload="handleUpload" max-size="10485760" max-count="3" multiple
						@on-remove="moveImgFileList" del-bg-color="#e9e9e9">
			...
			<view class="sendBill">
				...
				<u-button type="primary" class="form-btn-big" hover-class="none"
					...
					shape="circle">{{vuex_userRole == 'D'?"发送收款单":"发送付款单"}}</u-button>
				<!-- #endif -->
				<!-- #ifdef APP -->
				<u-button type="primary" class="form-btn-big" hover-class="none"
		...
		<view class="BillSuccess" v-if="!FROMBShow">
			<view class="" style="width: 60vw;height: 21vh;">
				<u-image style="" width="100%" height="100%" :show-menu-by-longpress="false"
			...
			async sendOrder(app) {
				...
				if (this.checkSend) {
					...
					for (const res of this.imgFileList) {
						const url = await this.UploadFilePdf(res.url, this.billFrom.billNumber, false, res.file.size,
							app);

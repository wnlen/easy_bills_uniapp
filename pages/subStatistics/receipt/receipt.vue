...
<template>
	<view class="root">
		...
		<z-paging ref="paging" use-virtual-list :force-close-inner-list="true" v-show="identity"
			...
			<view v-for="(item,index) in orderList" :key="index" :index="index"
				...
				<view class="OrderCardHand" @tap.stop>
					<view class="title ml1" style="" @tap.stop>
						...
						<!-- #endif -->
						<!-- #ifdef APP -->
						<view class="flex-col justify-center items-center mr15"
							:style="{backgroundColor:item.check?'#01BB74':'#ffffff'}"
							@click="checkboxGroupChange(item,index)"
							style="border-radius: 50px;height: 20px;width: 20px;border: 1px solid #AAAAAA;">
							<u-icon name="checkbox-mark" color="#ffffff" size="28"></u-icon>
...
<script>
	export default {
		data() {
			return {
				...
				bat64: {
					...
					bz: "data:image/png;base64,",
		...
		onLoad(option) {
			...
			if (option.searchData != undefined && option.searchData != "" && option.searchData != null) {
				var json = JSON.parse(option.searchData);
		...
		methods: {
			OpenReceipt() {
				...
				var checkList = this.orderList.filter(res => res.check == true)
				...
				this.searchJson.bossNumberE = checkList[0].bossNumberE;
				this.searchJson.staffNumberS = checkList[0].staffNumberS;
				this.searchJson.staffNumberE = checkList[0].staffNumberE;
				...
				if (this.checked) {
					...
					this.$u.post("/edo/bills/getFilterIdAll", this.realTimeSel).then(res => {
						console.log(res);
						...
						if (key) {
							...
							console.log(res);
							uni.navigateTo({
								url: "/pages/subStatistics/receipt/receiptFrom?data=" + JSON.stringify(res
									.data.data) + "&searchJson=" + JSON.stringify(this.searchJson)
							})
						} else {
							this.$u.toast("验证失败~")
						}
					...
					console.log("开收款单 NO全选", num);
					if (num <= 0) {
						this.$u.toast("您没有勾选订单~")
					} else {
						...
						this.$u.post("/edo/bills/lock", {
							...
							if (lock) {
								...
								console.log(res);
								uni.navigateTo({
									url: "/pages/subStatistics/receipt/receiptFrom?data=" + JSON.stringify(
										res.data.data) + "&searchJson=" + JSON.stringify(this
										.searchJson)
								})
							} else {
			...
			Init() {
				...
				if (this.searchData != null) {
					this.customer = this.searchData.company;

					if (ifWorkPort) {
						this.realTimeSel.bossNumberS = this.searchData.phone;
					} else {
						this.realTimeSel.bossNumberE = this.searchData.phone;

					}
					// this.field = this.searchData.phone;

					// this.showTage = 1
					// this.TitleFun(this.showTage)
				}
			...
			refreshDataNew() {
				...
				if (this.refresh) {
					...
					this.realTimeSel.role = this.vuex_userRole == "R" ? "1" : "0"
					this.$u.post('/edo/bills/getFilter', this.realTimeSel)
						.then(res => {
					...
					this.$u.post('/edo/bills/Quantity', this.realTimeSel)
						.then(res => {

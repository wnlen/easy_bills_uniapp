					...
					<view class="flex-col mt40">
						...
						<view class="flex-row items-center justify-between mt10" style="width: 100%">
							<view class="flex-row items-center" style="width: 50%">
								...
								<view @click="calendar1Show = true" class="ml24" style="border-box;border: 0.5px solid #999999;padding: 12rpx;border-radius: 3px;">
									{{ date1 }}
								</view>
							</view>
							<view class="flex-row items-center" style="width: 50%">
								...
								<u-icon name="arrow-down-fill" size="10"></u-icon>
								<view @click="calendar2Show = true" class="ml24" style="border-box;border: 0.5px solid #999999;padding: 12rpx;border-radius: 3px;">
									{{ date2 }}
								</view>
							</view>
						</view>
			...
			if (this.checked) {
				...
				this.$api.bills.getAllFilteredLockedIds(this.realTimeSel).then((res) => {
					...
					if (key) {
						...
						console.log(res);
						uni.navigateTo({
							url: '/pages/subStatistics/receipt/receiptFrom?data=' + JSON.stringify(res.data.data) + '&searchJson=' + JSON.stringify(this.searchJson)
						});
					} else {
						this.$u.toast('验证失败~');
					}

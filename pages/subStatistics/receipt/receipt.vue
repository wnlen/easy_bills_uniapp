<template>
  <view class="root">
    <u-empty
      icon="https://res-oss.elist.com.cn/wxImg/order/cw.svg"
      iconSize="400rpx"
      text="无查看权限~"
      mode="search"
      margin-top="-200"
        ...
        <view class="OrderCardHand" @tap.stop>
          <view class="title ml1" style="" @tap.stop>
            ...
            >
              ...
              <text
                class="ml15"
                @click="copyBtn(item.orderNumber)"
                style="color: #f76565"
              >
                {{ item.orderNumber }}
              </text>
...
export default {
  ...
  onLoad(option) {
    ...
    if (
      option.searchData != undefined &&
      option.searchData != "" &&
      option.searchData != null
    ) {
  ...
  methods: {
    OpenReceipt() {
      ...
      if (this.checked) {
        ...
        this.$api.bills
          .getAllFilteredLockedIds(this.realTimeSel)
          .then((res) => {
            console.log(res);
            ...
            if (key) {
              ...
              uni.navigateTo({
                url:
                  "/pages/subStatistics/receipt/receiptFrom?data=" +
                  JSON.stringify(res.data.data) +
                  "&searchJson=" +
                  JSON.stringify(this.searchJson),
              });
            } else {
              this.$u.toast("验证失败~");
            }
        ...
        if (num <= 0) {
          ...
          var orderListCheck = this.orderList.filter(
            (res) => res.check == true,
          );
    ...
    checkboxGroupChange(event, index) {
      ...
      if (port) {
        filteredOrders = checkTrue.filter(
          (order) => order.bossNumberS != checkTrue[0].bossNumberS,
        );
        zxList = filteredOrders.filter((order) =>
          order.bossNumberS.includes("zx"),
        );
      } else {
        filteredOrders = checkTrue.filter(
          (order) => order.bossNumberE != checkTrue[0].bossNumberE,
        );
        zxList = filteredOrders.filter((order) =>
          order.bossNumberE.includes("zx"),
        );
      }
    ...
    filterSubmit() {
      ...
      this.realTimeSel.endDate =
        this.date2 != "" ? this.date2 : this.$u.timeFormat(date, "yyyy-mm-dd");
      this.refreshDataNew();
    },
  },
};

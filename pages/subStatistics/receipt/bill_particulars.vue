		...
		<view class="FROMBill">
			...
			<view class="FromOwn">
				<view class="OwnText">
					...
					<text class="OwnTextFromText">
						{{billFrom.billNumber }}</text>
				</view>
				<view class="OwnText">
					<view class="OwnTextFromTitle" v-if="vuex_userRole == 'D'">
						客户名称
					</view>
					<view class="OwnTextFromTitle" v-if="vuex_userRole == 'R'">
						供应商名称
					</view>

					<text class="OwnTextFromText"
						v-if="vuex_user.data.work=='0'?vuex_user.phone== billFrom.sourcePhone:vuex_user.workData.bossNumber== billFrom.sourcePhone">{{ billFrom.billEnterpriseE}}</text>
			...
			<view class="FromOwn">
				<view class="OwnText">
					...
					<text class="OwnTextFromText">
						￥{{billFrom.billPrice.toFixed(2)}}</text>
				</view>
				<view class="OwnText"
					v-if="(vuex_user.data.work=='0'?(billFrom.sourcePhone==vuex_user.phone&&vuex_userRole == 'D'):(billFrom.sourcePhone==vuex_user.workData.bossNumber&&vuex_userRole == 'D'))||(vuex_user.data.work=='0'?(billFrom.receptionPhone==vuex_user.phone&&vuex_userRole == 'R'):(billFrom.receptionPhone==vuex_user.workData.bossNumber&&vuex_userRole == 'R'))">
					<view class="OwnTextFromTitle">
						折扣率
					</view>
			...
			<view class="FromFile">
				<view class="FromFileTitle" v-if="billFrom.cBillFolderList.length>0">
					图片
				</view>
				<view class="recently-cat flex-row flex-wrap" style="width: 95%;">
					<view class="ImgView">
						<u-image class="ml24" v-for="(item,index) in billFrom.cBillFolderList" :key="index"
							v-if="item.type=='0'" width="150rpx" height="150rpx" :src="item.file"
							@click="LookImg(billFrom.cBillFolderList)"></u-image>
					</view>
				</view>
				<view class="FromFileTitle" v-if="FileList.length>0">
					附件
				</view>
				...
				<view class="FromFileTitleRemark" style="">
					<u-input disabled type="textarea" class="" placeholder="(无备注)" v-model="billFrom.billRemark"
			...
			<view class="FromOwn">
				...
				<view class="OwnText">
					...
					<text
						class="OwnTextFromText">{{ vuex_user.data.work=='0'?(vuex_user.phone || vuex_user.data.phone):vuex_user.workData.bossNumber }}</text>
				</view>
			</view>
		</view>
	</view>

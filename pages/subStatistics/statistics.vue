	...
	<view class="vh100 vw100 fixed-bar-height bg-gray">
			...
			<view slot="top">
				...
				<view class="fliter-wrap">
					<view class="fliter-wrap-view">
						<view @click="tagClick(index,item)" v-for="(item,index) in filterTags" :key="index"
							v-if="item.show" class="filter-btn flex-row justify-center items-center"
							:class="{'active':index==tagIndex}">
							<u-icon v-if="item.key==false" size="35"
								name="https://ydj-lsy.oss-cn-shanghai.aliyuncs.com/applet-img/img/vip/vip.svg"></u-icon>
							{{item.value}}
						</view>
					</view>
				</view>
				...
				<div class="bg-white pd20 mt20 radius flex-col justify-center items-center cardShow">
					...
					<div class="flex-row items-center radius pr20 mr10 mt20"
						...
						<div class="bg-white flex-row items-center justify-left radius"
							...
							<u-input class="ml24 width100" @input="changeCustomer" v-model="customer"
								:placeholder="vuex_userRole=='R'?'请选择供应商':'请选择客户'" />
							<div class="flex-col justify-center items-center" style="height: 5vh;">
								<u-icon class="ml48" name="/static/img/list/lxr.svg" size="45"
		...
		<view class="order-simple-list pl30 pr30">
				...
				<view class="flex-col pl30 pr30 pb30 justify-between height100" style="width: 100%;">
					...
					<view class="flex-row justify-end mt25 vw100" style="">
						...
						<view class="mr48" style="float: right;">
							<u-button @click="filterReset" shape="circle" size="medium" type="info"
			...
			<u-calendar btn-type="success" v-model="calendar1Show" active-bg-color="#01BB74" range-bg-color="#DFF9EF"
				range-color="#333333" mode="date" :min-date="getMin()" :max-date="getMax()"
							...
							if (downloadResult.statusCode === 200) {
								...
								uni.saveFile({
									...
									success: (saveResult) => {
										...
										// #ifdef APP-PLUS
										plus.runtime.openFile(saveResult.savedFilePath, {},
											function(e) {
												uni.showToast({

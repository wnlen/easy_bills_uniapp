<template>
	<view class="y-wrap" :style="'--bottom-height:' + bottomHeight">
		...
		<view class="flex-col justify-center items-center pb15"
		...
		<scroll-view class="y-wrap_message_content" id="scrollBox" scroll-y :scroll-into-view="scrollToId"
			...
			<view v-for="(item, index) in list" :key="index" :id="'y-chat-' + item[defaultOptions['msgId']]"
				...
				<view>
					<u-image width="100rpx" height="100rpx" :src="item[defaultOptions['avator']] || defaultAvator"
						@click="tapAvator(item)" radius="10rpx" bgColor="transparent" shape="circle"></u-image>
					<view
						:class="['y-wrap_message_content_box_msg', { 'y-wrap_message_content_box_my' : item[defaultOptions['userId']] == userId }]">
						<view style="position: relative;width: 200px;"
							:class="['y-wrap_message_content_box_msg__name', { 'y-wrap_message_content_box_msg__me' : item[defaultOptions['userId']] == userId }]">
							...
							<view v-if="item.userId=='0'" class="" style="position: absolute;left: 0;font-size: 12px;">
								<text>{{ item[defaultOptions['name']]  }} <text class="ml10"
										style="color: #F1BF7B;">金牌客服</text></text>
							</view>
						...
						<view :class="[
							...
							<u-image @tap="lookImg(item[defaultOptions['img']], index)"
								...
								width="calc(60vw - 15rpx)" height="auto" mode="widthFix"></u-image>
						</view>
					</view>
		...
		<view class="y-wrap_footer">
			<view class="y-wrap_footer_show_box" id="show_box">
				...
				<view class="y-wrap_footer_show_box__ipt" style="background-color: #F4F4F4;">
					...
					<u-input :border="true" v-if="textShowFlag" v-model="sendVal"
						:placeholder="bannedToPost ? '禁言中' : ''" placeholderStyle="text-align:center"
						:disabled="bannedToPost" :focus="focus" @blur="blur"></u-input>
				...
				<view>
					...
					<button
						...
						type="primary" size="mini" :class="sendVal ? 'width_to_large' : 'fade_show'"
						@touchend.prevent="send" ref="send" v-else>发送</button>
					<!-- #endif -->
					<!-- #ifdef MP-WEIXIN -->
			...
			<view class="y-wrap_footer_hide_box" id="hide_box" >
				<view class="y-wrap_footer_hide_box_item" v-for="(item, index) in sheet" :key="index"
					...
					<view class="y-wrap_footer_hide_box_item__btn pb100">
						<u-image shape="circle" v-if="item.img&&footerFlag" :src="item.img" width="60rpx" height="60rpx"
							mode="aspectFill" />
		...
		<!-- readyCloseAudio -->
		<u-popup :show="audioFlag" :bgColor="readyCloseAudio ? 'rgba(255, 255, 255, 0.4)' : ''" mode="center"
			:zoom="false" round="20">

  ...
  <view class="y-wrap" :style="'--bottom-height:' + bottomHeight">
    ...
    >
      <view
        ...
        :class="[
          ...
          {
            'y-wrap_message_content_my':
              item[defaultOptions['userId']] == userId,
          },
        ]"
      >
        <view
          style="text-align: center; padding: 10rpx 0 20rpx"
          v-if="item.showTime"
        >
          {{ item.timeLabel }}
        </view>
          ...
          >
            ...
            >
              <u-tag
                ...
                :bgColor="
                  tagOptions[item[defaultOptions['tagLabel']]]
                    ? tagOptions[item[defaultOptions['tagLabel']]].bgColor
                    : ''
                "
                :borderColor="
                  tagOptions[item[defaultOptions['tagLabel']]]
                ...
                :color="
                  tagOptions[item[defaultOptions['tagLabel']]]
                    ? tagOptions[item[defaultOptions['tagLabel']]].color
                    : ''
                "
                :text="
                  tagOptions[item[defaultOptions['tagLabel']]]
                    ? tagOptions[item[defaultOptions['tagLabel']]].text
                    : ''
                "
                size="mini"
              ></u-tag>
              <view
                v-if="item.userId == '0'"
                class=""
                style="position: absolute; left: 0; font-size: 12px"
              >
                <text>
                  {{ item[defaultOptions["name"]] }}
                  <text class="ml10" style="color: #f1bf7b">金牌客服</text>
                </text>
              </view>
              <!-- 		<view v-if="item.userId!='0'" class="" style="position: absolute;right: 0;font-size: 12px;">
    ...
    <view class="y-wrap_footer">
      <view class="y-wrap_footer_show_box" id="show_box">
        ...
        <view
          class="y-wrap_footer_show_box__ipt"
        ...
        >
          ...
          <button
            v-else
            style="font-size: 32rpx; overflow: inherit"
      ...
      <view class="y-wrap_footer_hide_box" id="hide_box">
        <view
          class="y-wrap_footer_hide_box_item"
    ...
    <u-popup
      :show="audioFlag"
      :bgColor="readyCloseAudio ? 'rgba(255, 255, 255, 0.4)' : ''"
      mode="center"
      :zoom="false"
      round="20"
    >
      <view class="y-wrap_audio">
        <view
          class="y-wrap_audio__large"
          :style="'border-color:' + borderColor"
        >
          <view
            class="y-wrap_audio__small"
            :style="'border-color:' + borderColor"
          >
            <view
              class="y-wrap_audio__mini"
              :style="'border-color:' + borderColor"
            ></view>
          </view>
        </view>
        ...
        if (newVal.length > 0 && newVal.length != oldVal.length) {
          newVal.forEach((item, index) => {
            item.showTime =
              index == 0
                ? true
                : item.time - newVal[index - 1][timeOptions] >=
                  this.intervalTime;
          ...
          setTimeout(async () => {
            if (oldVal.length == 0) {
              ...
              const containerBox = query.selectAll(
                ".y-wrap_message_content_box",
              );
        ...
        if (Object.prototype.toString.call(newVal).includes("Array")) {
          ...
          newVal.forEach((item, index) => {
            item.showTime =
              index == 0
                ? true
                : item[timeOptions] - newVal[index - 1][timeOptions] >=
                  this.intervalTime;
          ...
          newVal.showTime =
            empty ||
            newVal[timeOptions] -
              this.list[this.list.length - 1][timeOptions] >=
              this.intervalTime;
  ...
  computed: {
    bottomHeight() {
      return this.footerFlag
        ? this.hideBoxHeight + this.showBoxHeight + "px"
        : this.showBoxHeight + "px";
    },
    sheet() {
  ...
  methods: {
    ...
    scrollBottom() {
      ...
      setTimeout(() => {
        this.scrollToId =
          "y-chat-" +
          this.list[this.list.length - 1][this.defaultOptions.msgId];
      }, duration);
    },
    showHideBox() {
...
.y-wrap {
  ...
  &_message_content {
    ...
    height: calc(86% - var(--bottom-height));
  ...
  &_footer {
    ...
    &_show_box {
      ...
      .fade_show {
        animation: fade_show 0.3s linear;
      }

      .width_to_mini {
        animation: width_animate_mini 0.3s linear;
      }

      .width_to_large {
        animation: width_animate_large 0.3s linear;
      }

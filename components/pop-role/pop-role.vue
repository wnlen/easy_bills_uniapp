    ...
    >
      <view class="flex-col" style="width: 100%">
        <text
          ...
          >设置角色</text
        >
        <text class="ft-gray pb48" style="#666666"
          >切换您的角色，快速收发单</text
        >
      </view>
      ...
      if (this.roleShowF) {
        ...
        if (this.vuex_user.phone != undefined) {
          if (this.$parent.indexSumList) {
            ...
            this.$parent.indexSumList = this.$parent.indexSumList.filter(
              (res) => res.port == filer,
            );
            console.log("发货端1");

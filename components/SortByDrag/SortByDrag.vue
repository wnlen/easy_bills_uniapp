<template>
	<movable-area :style="{width, height}" style="position: relative;">
		<movable-view direction="all" :disabled="!drag" :x="item.x" :y="item.y" class="box relative"
			:style="{width: itemWidth+'rpx', height: itemHeight+'rpx'}" style="position: absolute; top: 0; left: 0;"
			v-for="(item, index) in list" :key="index" @touchstart="moveIndex = index" @touchend="moveStop"
			...
			<div @tap.stop class="absolute" style="right: 0;top: 0;width: 20px;height: 20px;z-index: 99999;" @click="minusFun(item,list,index,minusName)">
				<u-icon v-show="minus" :color="minusColor"  :name="minusName"
					size="30rpx" class="absolute" style="right: 0;top: 0;"></u-icon>
			</div>
			<div>
	...
	export default {
		...
		methods: {
			...
			move(e, item, index) {
				...
				let x = e.detail.x
				let y = e.detail.y
				const my = (y - item.y) < 0 ? Math.ceil((y - item.y) / this.scaleY) : Math.floor((y - item.y) / this
					.scaleY)

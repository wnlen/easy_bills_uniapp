  ...
  <movable-area :style="{ width, height }" style="position: relative">
    ...
    >
      <div
        @tap.stop
        class="absolute"
        style="right: 0; top: 0; width: 20px; height: 20px; z-index: 99999"
        @click="minusFun(item, list, index, minusName)"
      >
        <u-icon
          v-show="minus"
          :color="minusColor"
...
export default {
  ...
  methods: {
          ...
          if (start > end) {
            ...
            if (index >= end && index < start) {
              ...
              this.$set(
                item,
                "y",
                Math.floor((index + 1) / 5) * this.scaleY + 1,
              );
            }
          }
          if (start < end) {
    ...
    move(e, item, index) {
      ...
      const my =
        y - item.y < 0
          ? Math.ceil((y - item.y) / this.scaleY)
          : Math.floor((y - item.y) / this.scaleY);

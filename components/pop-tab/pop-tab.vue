		...
		<u-tabbar :value="tabIndex" activeColor="#01BB74" @change="changeTab">
			<up-tabbar-item :text="item.text" v-for="(item,index) in tabbar" :key="index">
				<template #active-icon>
					<image style="width: 46rpx;" mode="widthFix" class="u-page__item__slot-icon"
						:src="item.selectedIconPath"></image>
				</template>
				<template #inactive-icon>
					<image :id="index==1?'box4':''" style="width: 46rpx;" mode="widthFix"
						class="u-page__item__slot-icon" :src="item.iconPath">
					</image>
				</template>
			</up-tabbar-item>
		</u-tabbar>

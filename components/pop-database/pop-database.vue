<template>
  <view
    @tap.stop
    v-show="roleShow"
    class="flex-col justify-center items-center pop-renew"
  >
    <view
      class="relative flex-col justify-center items-center"
    ...
    >
      ...
      <view
        class="absolute flex-row justify-center items-center mt30"
        style="width: 100%; height: 120px"
      >
        <view
          class="mr15 tab flex-col justify-center items-center"
          :style="{ backgroundColor: year == 1 ? '#FDF7ED' : '#FFFFFF' }"
          @click="orderInit('A1')"
        >
          <u-text class="t1">{{
            productCode[0] ? productCode[0].product : ""
          }}</u-text>
          <u-text class="t2">￥{{ orderPriceBDiscount }}</u-text>
          <u-text class="t3">￥{{ orderPriceB }}</u-text>
          <view class="iconPeople flex-col justify-center items-center mt10"
            >低至{{ (orderPriceBDiscount / 365).toFixed(2) }}元/天</view
          >
        </view>
        <view
          class="ml15 tab flex-col justify-center items-center"
          :style="{ backgroundColor: year == 2 ? '#FDF7ED' : '#FFFFFF' }"
          @click="orderInit('A2')"
        >
          <u-image
            :lazy-load="true"
          ...
          ></u-image>
          <u-text class="t1">{{
            productCode[1] ? productCode[1].product : ""
          }}</u-text>
          <u-text class="t2">￥{{ orderPriceQDiscount }}</u-text>
          <u-text class="t3">￥{{ orderPriceQ }}</u-text>
          <view class="iconPeople flex-col justify-center items-center mt10"
            >低至{{ (orderPriceQDiscount / 365).toFixed(2) }}元/天</view
          >
        </view>
      </view>
      ...
      <view
        @click="buyRenew"
        class="absolute flex-row justify-center items-center buttonRenew"
        style="bottom: 50px"
      >
        <u-text class="te"
          >￥{{ year == 1 ? orderPriceBDiscount : orderPriceQDiscount }}</u-text
        >
...
export default {
  ...
  methods: {
    ...
    orderInit(type) {
      ...
      this.order.price =
        type == "A1" ? this.orderPriceBDiscount : this.orderPriceQDiscount;
      this.order.payment =
        type == "A1"
          ? (this.orderPriceBDiscount * 100).toFixed(0)
          : (this.orderPriceQDiscount * 100).toFixed(0);

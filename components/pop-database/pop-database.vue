<template>
	<view @tap.stop v-show="roleShow" class="flex-col justify-center items-center pop-renew">
		<view class="relative flex-col justify-center items-center" style="height: 100%;width: 100%;">
			...
			<u-icon @click="close" class="absolute" style="top: 5%;right: 10%;" name="close-circle" color="#ffffff"
				size="40"></u-icon>

			<view class="absolute flex-row justify-center items-center mt30" style="width: 100%;height: 120px;">
				<view class="mr15 tab flex-col justify-center items-center"
					:style="{backgroundColor:year==1?'#FDF7ED':'#FFFFFF'}" @click="orderInit('A1')">
				...
				<view class="ml15 tab flex-col justify-center items-center"
					:style="{backgroundColor:year==2?'#FDF7ED':'#FFFFFF'}" @click="orderInit('A2')">
	...
	export default {
		...
		methods: {
			...
			orderInit(type) {
				...
				this.order.payment = type == "A1" ? (this.orderPriceBDiscount * 100).toFixed(0) : (this
					.orderPriceQDiscount * 100).toFixed(0);
